...
@_spi(Internal) public final class RuleRegistryGenerator: FileGenerator {
  ...
  public func generateContent() -> String {
    ...
    var result = """
      ...
      """
    for rule in sorted {
      result += "    \"\(rule.typeName)\": \(rule.isOptIn ? ".off" : ".warning"),\n"
    }
    result += "  ]\n\n"

    result += "  public static let formatRules: [String: RuleSeverity] = [\n"
    for rule in formatRules {
      result += "    \"\(rule.typeName)\": \(rule.isOptIn ? ".off" : ".warning"),\n"
    }
    result += "  ]\n\n"

    result += "  public static let lintRules: [String: RuleSeverity] = [\n"
