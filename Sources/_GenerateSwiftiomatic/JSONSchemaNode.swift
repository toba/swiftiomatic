  ...
  init(from decoder: any Decoder) throws {
    ...
    if let v = try? container.decode(Bool.self) { self = .bool(v) }
      ...
      throw DecodingError.dataCorrupted(
        .init(codingPath: decoder.codingPath, debugDescription: "Cannot decode JSONSchemaValue")
      )
    }
  }
}
...
extension JSONSchemaNode {
  ...
  static func integer(
    description: String, defaultValue: Int, minimum: Int? = nil
  ) -> JSONSchemaNode {
    var node = JSONSchemaNode()
    node.type = "integer"
    node.description = description
  ...
  static func stringEnum(
    description: String, values: [String], defaultValue: String
  ) -> JSONSchemaNode {
    var node = JSONSchemaNode()
    node.type = "string"
    node.description = description
  ...
  static func stringArray(
    description: String, defaultValue: [String]? = nil
  ) -> JSONSchemaNode {
    var node = JSONSchemaNode()
    node.type = "array"
    node.description = description
