...
enum Request {
  ...
  private var sourcekitObject: SourceKitObject {
    ...
    case .cursorInfoUSR(let file, let usr, let arguments, let cancelOnSubsequentRequest):
      return [
        "key.request": UID("source.request.cursorinfo"),
        "key.sourcefile": file,
        "key.usr": usr,
  ...
  func send() throws(Request.Error) -> [String: SourceKitValue] {
    ...
    let result: Result<[String: SourceKitValue], Request.Error> =
      sourceKitRequestGate
      .withLock {
        _ in
    ...
    if case .failure(let error) = result, case .connectionInterrupted = error {
  ...
  enum Error: Swift.Error, LocalizedError, CustomStringConvertible {
    ...
    private var message: String? {
      switch self {
      case .connectionInterrupted(let string): string
      case .invalid(let string): string
      case .failed(let string): string
      case .cancelled(let string): string
