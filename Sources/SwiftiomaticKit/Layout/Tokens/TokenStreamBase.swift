...
/// Visits the nodes of a syntax tree and constructs a linear stream of formatting tokens that tell
/// the pretty printer how the source text should be laid out.
///
/// The final `TokenStream` subclass with generated override stubs lives in
/// `TokenStream+Generated.swift` . Extension methods (the actual visit implementations) are in the
/// `TokenStream+*.swift` files.
class TokenStreamBase: SyntaxVisitor {
    ...
    /// Lists the expressions that have been visited, from the outermost expression, where
    /// contextual breaks and start/end contextual breaking tokens have been inserted.
    ...
    /// from the preceding token. When breaks are inserted around compound expressions, the breaks
    /// are moved past these tokens.
    ...
    /// token stream.
    func before(_ token: TokenSyntax?, tokens: Token...) { before(token, tokens: tokens) }
    ...
    /// token stream.
    func after(_ token: TokenSyntax?, tokens: Token...) { after(token, tokens: tokens) }
...
extension TokenStream {
    func makeStream(from node: Syntax) -> [Token] {
        ...
        if case .ranges = selection { appendToken(.enableFormatting(nil)) }
        defer { tokens = [] }
        return tokens
    }
    ...
    func verbatimToken(_ node: Syntax, indentingBehavior: IndentingBehavior = .allLines) {
        ...
        if let lastToken = node.lastToken(viewMode: .sourceAccurate) {
            ...
            // syntax token. Leading comments don't need special handling since they belong to the
            // current node, and will get printed.
...
extension AccessorBlockSyntax {
    ...
    /// return the code block items in that getter.
    var getterCodeBlockItems: CodeBlockItemListSyntax {
        guard case let .getter(codeBlockItemList) = accessors else {
