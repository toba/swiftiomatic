...
/// Position-aware JSON5 scanner used by `Configuration.applyUpdateText` to drive surgical edits on
/// a configuration file. Captures top-level object members, their value spans, trailing commas, and
/// any nested object's children — enough to insert or remove a key without disturbing adjacent
/// formatting or comments.
///
/// Design adapted from `croct-tech/json5-parser-js` — a token stream with
/// `peek/next/consume/expect/matches` and a small JSON5 lexicon (unquoted identifier keys,
/// single-quoted strings, hex/Infinity/NaN numbers, trailing commas, `// ...` and `/* ... */`
/// comments).
struct JSON5Scanner {
    ...
    /// Parse a member key — either a quoted string or a JSON5 unquoted identifier. Returns the
    /// decoded key name and the source range covering the key token (including any surrounding
    /// quotes).
    private mutating func parseKey() throws(Error) -> (String, Range<String.Index>) {
        ...
        switch tok.kind {
            ...
            default: throw Error.expected("object key", at: tok.range.lowerBound)
        }
    }

    /// Advances past one JSON5 value. Returns a nested object layout if the value is `{...}` ,
    /// otherwise `nil` .
    private mutating func parseValue() throws(Error) -> ObjectLayout? {
        ...
        switch tok.kind {
            ...
            case .string, .scalar, .identifier:
                try lexer.advance()
                return nil
            default: throw Error.expected("value", at: tok.range.lowerBound)
        }
    }
        ...
        mutating func advance() throws(JSON5Scanner.Error) {
            ...
            if isIdentifierStart(c) {
                ...
                switch text {
                    ...
                    default: current = Token(kind: .identifier, range: start..<cursor)
                }
                return
            }
