...
func rewriteSourceFile(
    ...
    // 4. UseAtEntryNotEnvironmentKey: rewrite `EnvironmentValues` extensions to use `@Entry` and
    //    remove the matched `EnvironmentKey` types.
    if context.shouldRewrite(UseAtEntryNotEnvironmentKey.self, at: Syntax(result)) {
        result = UseAtEntryNotEnvironmentKey.transform(
    ...
    if context.shouldRewrite(DropRedundantAccessControl.self, at: Syntax(result)) {
        result = DropRedundantAccessControl.transform(
            result, original: node, parent: nil, context: context)
    }

    // 6. UseURLMacroForURLLiterals: if any `URL(string:)!` was rewritten to `#URL(...)` during the
    //    descent (recorded via `ruleState.madeReplacements` ), insert the configured module import
