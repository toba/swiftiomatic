...
/// A rule that formats and/or lints, dispatched entirely through static `transform` / `willEnter` /
/// `didExit` hooks invoked by `CompactSyntaxRewriter` .
///
/// `StaticFormatRule` carries no per-file state — `Context` is threaded through each static call.
/// The base class exists only as a registration target so `RuleCollector` can detect the rule via
/// inheritance pattern, identical to the way `LintSyntaxRule` and `StructuralFormatRule` are
/// recognised.
///
