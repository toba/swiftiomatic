...
final class UnusedArguments: RewriteSyntaxRule<BasicRuleValue> {
  ...
  override func visit(_ node: ClosureExprSyntax) -> ExprSyntax {
    ...
    case .parameterClause(var clause):
  ...
  private func isNameUsed(_ name: String, in syntax: some SyntaxProtocol) -> Bool {
    ...
    for token in syntax.tokens(viewMode: .sourceAccurate) {
      guard matchesIdentifier(token, name: name) else { continue }
  ...
  private func isShadowed(
    ...
    while let parent = current.parent {
      ...
      if let forStmt = parent.as(ForStmtSyntax.self) {
        ...
        if (inBody || inWhere) && patternContains(name, in: forStmt.pattern) {
          return true
        }
      }
         ...
         let caseLabel = switchCase.label.as(SwitchCaseLabelSyntax.self)
      {
        for item in caseLabel.caseItems {
          if patternContains(name, in: item.pattern) { return true }
        }
      }
  ...
  private func statementDeclares(_ name: String, in stmt: CodeBlockItemSyntax) -> Bool {
    if let varDecl = stmt.item.as(VariableDeclSyntax.self) {
      for binding in varDecl.bindings {
        if patternContains(name, in: binding.pattern) { return true }
      }
    }
  ...
  private func closureBinds(_ name: String, in closure: ClosureExprSyntax) -> Bool {
    guard let signature = closure.signature,
          let paramClause = signature.parameterClause else { return false }
    ...
    case .parameterClause(let clause):
      return clause.parameters.contains { param in
        if let secondName = param.secondName {
          return secondName.text == name
        }
        return param.firstName.text != "_" && param.firstName.text == name
      }
    }
  }
  ...
  private func isSubscriptParamUsed(
    ...
    case .accessors(let accessors):
      for accessor in accessors {
        if let body = accessor.body, isNameUsed(name, in: body) {
          return true
        }
      }
      return false
