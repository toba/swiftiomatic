...
final class PreferTrailingClosures: RewriteSyntaxRule<BasicRuleValue> {
  ...
  override func visit(_ node: FunctionCallExprSyntax) -> ExprSyntax {
    ...
    if let funcName, Self.neverTrailing.contains(funcName) {
      return visited
    }

    // Don't convert in conditional contexts where trailing closure is ambiguous
    if isInConditionalContext(node) {
      return visited
    }
  ...
  private func convertSingle(
    ...
    return ExprSyntax(result.with(\.trailingClosure, trailingClosure))
  }
...
extension Finding.Message {
  fileprivate static let useTrailingClosure: Finding.Message =
