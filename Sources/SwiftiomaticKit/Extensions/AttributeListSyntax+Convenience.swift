...
extension AttributeListSyntax {
    ...
    ///
    /// Matching is by name shape, because the syntax tree carries no type information: an attribute
    /// qualifies when its simple name starts with an uppercase letter and ends with `Actor` . That
    /// covers `@MainActor` and the conventional custom form (`@DatabaseActor` ). `@globalActor` is
    /// excluded — it marks the *definition* of a global actor, and a `static` member of that type
    /// is nonisolated.
    func globalActorAttribute() -> AttributeSyntax? {
        for element in self {
            ...
            guard name.hasSuffix("Actor"), let first = name.first, first.isUppercase
            else { continue }
            return attr
        }
        return nil
    ...
    /// This mirrors `DeclModifierListSyntax.remove(anyOf:)` — see `DropRedundantAccessControl` for
    /// the modifier pattern and `DropRedundantViewBuilder` for the attribute pattern.
