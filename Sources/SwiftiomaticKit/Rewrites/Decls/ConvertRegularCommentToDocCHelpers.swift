...
/// Stateless helpers for the inlined `ConvertRegularCommentToDocC` rule.
/// Each `static transform` overload on the rule class delegates here.

/// Directive prefixes that should never be converted.
private let directivePrefixes = [
    "MARK:", "TODO:", "FIXME:", "HACK:", "WARNING:",
    "swiftformat:", "swiftlint:", "sourcery:",
    "sm-ignore",
]

func applyConvertRegularCommentToDocC(
...
private func convertRegularToDoc<N: SyntaxProtocol>(
    ...
    ConvertRegularCommentToDocC.diagnose(.useDocComment, on: node, context: context)
...
private func convertDocToRegular<N: SyntaxProtocol>(
    ...
    guard modified else { return node }
    ConvertRegularCommentToDocC.diagnose(.useRegularComment, on: node, context: context)
    return node.with(\.leadingTrivia, Trivia(pieces: newPieces))
}
