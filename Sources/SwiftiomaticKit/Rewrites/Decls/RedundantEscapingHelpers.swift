...
/// Stateless helpers for the inlined `RedundantEscaping` rule. The conservative
/// escape checker (`EscapeChecker` `SyntaxVisitor`) is encapsulated here. Each
/// `static transform` overload on the rule class delegates to
/// `applyRedundantEscaping(_:parent:context:)`.

func applyRedundantEscaping(
    _ node: FunctionDeclSyntax,
    parent: Syntax?,
    context: Context
) -> FunctionDeclSyntax {
    guard !redundantEscapingIsInsideProtocol(parent: parent),
        let body = node.body
    else { return node }
    guard let rewritten = redundantEscapingRewriteParameterClause(
...
private func redundantEscapingRewriteParameterClause(
    ...
    let newParams = clause.parameters.map { param -> FunctionParameterSyntax in
        guard let attributedType = param.type.as(AttributedTypeSyntax.self),
            let escapingAttr = redundantEscapingAttribute(in: attributedType.attributes)
        else { return param }
        let paramName = (param.secondName ?? param.firstName).text
        let isAutoclosure = redundantEscapingHasAttribute(
            named: "autoclosure", in: attributedType.attributes
        )
        ...
        let newAttributes = AttributeListSyntax(
            attributedType.attributes.compactMap { element -> AttributeListSyntax.Element? in
                if case .attribute(let attr) = element,
                    redundantEscapingAttributeName(of: attr) == "escaping"
                {
                    return nil
                }
                return element
...
private func redundantEscapingAttribute(in list: AttributeListSyntax) -> AttributeSyntax? {
    for element in list {
        guard case .attribute(let attr) = element else { continue }
        if redundantEscapingAttributeName(of: attr) == "escaping" { return attr }
    }
    return nil
}

private func redundantEscapingHasAttribute(
    named name: String, in list: AttributeListSyntax
) -> Bool {
    list.contains { element in
        guard case .attribute(let attr) = element else { return false }
        return redundantEscapingAttributeName(of: attr) == name
    }
}

private func redundantEscapingAttributeName(of attr: AttributeSyntax) -> String? {
    attr.attributeName.as(IdentifierTypeSyntax.self)?.name.text
}

private func redundantEscapingIsInsideProtocol(parent: Syntax?) -> Bool {
    var current = parent
