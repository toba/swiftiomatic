...
/// - `s.range(of: needle) [==/!=] nil` (skipped when `options:` is supplied, e.g.
///   `.regularExpression` )
final class UseContains: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: InfixOperatorExprSyntax) -> SyntaxVisitorContinueKind {
        ...
        if methodName == "first" || methodName == "firstIndex" {
            ...
            guard hasClosureArgument else { return .visitChildren }
            diagnose(
                .containsOverFirstNotNil(method: methodName, op: opText), on: leftMember.declName)
            return .visitChildren
        }
...
fileprivate extension Finding.Message {
    static let containsOverFilterCount: Finding.Message =
        "prefer 'contains(where:)' over comparing 'filter(_:).count' to 0"

    static let containsOverFilterIsEmpty: Finding.Message =
        "prefer 'contains(where:)' over 'filter(_:).isEmpty'"

    static func containsOverFirstNotNil(method: String, op: String) -> Finding.Message {
        "prefer 'contains(where:)' over '\(method)(where:) \(op) nil'"
    }

    static func containsOverRangeNilComparison(op: String) -> Finding.Message {
