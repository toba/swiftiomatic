...
final class UseLastWhere: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: MemberAccessExprSyntax) -> SyntaxVisitorContinueKind {
        ...
        diagnose(.useLastWhere, on: calledMember.declName)
        return .visitChildren
    }
}

fileprivate extension Finding.Message {
    static let useLastWhere: Finding.Message = "prefer 'last(where:)' over 'filter(_:).last'"
}
