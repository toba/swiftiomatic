...
///
/// `@_cdecl` was the experimental, underscore-prefixed spelling. SE-0407 promoted
/// it to the official `@c` attribute in Swift 6.3.
///
/// Lint: Using `@_cdecl` raises a warning.
///
/// Rewrite: The attribute is replaced with `@c`, preserving any argument list.
final class PreferOfficialCDecl: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        guard identType.name.text == "_cdecl" else {
            return node
        }
...
extension Finding.Message {
    fileprivate static var useCAttribute: Finding.Message {
        "replace '@_cdecl' with '@c'"
    }
}
