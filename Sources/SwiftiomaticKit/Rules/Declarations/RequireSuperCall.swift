...
/// ignored, view lifecycle skipped, test setup not run).
///
/// Configure the list of method names via `requireSuperCall.methodNames` . Defaults cover common
/// UIKit/AppKit/XCTest methods. Names use SwiftLint's resolved-name format: `viewDidLoad()` ,
/// `viewWillAppear(_:)` , `setEditing(_:animated:)` .
///
/// Lint: When an `override` of a configured method either omits the `super.<name>(...)` call or
/// calls it more than once, a warning is raised.
final class RequireSuperCall: LintSyntaxRule<RequireSuperCallConfiguration>, @unchecked Sendable {
    ...
    override func visit(_ node: FunctionDeclSyntax) -> SyntaxVisitorContinueKind {
        ...
        guard node.modifiers.contains(.override) else { return .visitChildren }
        if node.modifiers.contains(anyOf: [.static, .class]) { return .visitChildren }
    ...
    private func resolvedName(of node: FunctionDeclSyntax) -> String {
        ...
        return labels.isEmpty
            ? "\(node.name.text)()"
            : "\(node.name.text)(\(labels.map { "\($0):" }.joined()))"
    }
...
fileprivate extension Finding.Message {
    static func missingSuperCall(_ name: String) -> Finding.Message {
        "override of '\(name)' should call super"
    }

    static func multipleSuperCalls(_ name: String) -> Finding.Message {
...
package struct RequireSuperCallConfiguration: SyntaxRuleValue {
    ...
    /// Methods whose overrides must call `super` . Entries are full Swift selectors, e.g.
    /// `"viewDidLoad()"` or `"setEditing(_:animated:)"` . Replacing this list overrides the
    /// built-in UIKit/AppKit/XCTest defaults.
    ...
    package init(from decoder: any Decoder) throws {
        ...
        methodNames = try container.decodeIfPresent([String].self, forKey: .methodNames)
