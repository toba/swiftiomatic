...
final class OneDeclarationPerLine: RewriteSyntaxRule<BasicRuleValue>, @unchecked Sendable {
    ...
    /// A state machine that collects case elements encountered during visitation and allows new
    /// case declarations to be created with those elements.
    private struct CaseElementCollector {
        ...
        /// Indicates whether the full leading trivia of basis case declaration should be preserved
        /// by the next case declaration that will be created by copying the basis declaration.
        ///
        /// This is true for the first case (to preserve any leading comments on the original case
        /// declaration) and false for all subsequent cases (so that we don't repeat those
        /// comments).
        private var shouldKeepLeadingTrivia = true

        /// Creates a new case element collector based on the given case declaration.
        init(basedOn basis: EnumCaseDeclSyntax) { self.basis = basis }

        /// Adds a new case element to the collector.
        mutating func addElement(_ element: EnumCaseElementSyntax) { elements.append(element) }

        /// Creates a new case declaration with the elements collected so far, then resets the
        /// internal state to start a new empty declaration again.
        ///
        /// This will return nil if there are no elements collected since the last time this was
        /// called (or the collector was created).
        ...
        /// current basis declaration, and updates the comment preserving state if needed.
        mutating func makeCaseDeclFromBasis(
            ...
            if shouldKeepLeadingTrivia {
                ...
                // We don't bother preserving any indentation because the pretty printer will fix
                // that up. All we need to do here is ensure that there is a newline.
    ...
    static func transform(
        ...
        for member in node.memberBlock.members {
            ...
            // Collect the elements of the case declaration until we see one that has either an
            // associated value or a raw value.
...
fileprivate extension Finding.Message {
    static func moveAssociatedOrRawValueCase(name: String) -> Finding.Message {
        "move '\(name)' to its own 'case' declaration"
    }

    static func onlyOneVariableDeclaration(specifier: String) -> Finding.Message {
        "split this variable declaration to introduce only one variable per '\(specifier)'"
    }
}

/// Splits a variable declaration with multiple bindings into individual declarations.
///
/// Swift's grammar allows each identifier in a variable declaration to have a type annotation, an
/// initializer expression, both, or neither. Stricter checks occur after parsing, however; a lone
/// identifier may only be followed by zero or more other lone identifiers and then an identifier
/// with *only* a type annotation (and the type annotation is applied to all of them). If we have
/// something else, we should handle them gracefully (i.e., not destroy them) but we don't need to
/// try to fix them since they didn't compile in the first place so we can't guess what the user
/// intended.
///
/// So, this algorithm works by scanning forward and collecting lone identifiers in a queue until we
/// reach a binding that has an initializer or a type annotation. If we see a type annotation
/// (without an initializer), we can create individual variable declarations for each entry in the
/// queue by projecting that type annotation onto each of them. If we reach a case that isn't valid,
/// we just flush the queue contents as a single declaration, to effectively preserve what the user
/// originally had.
private struct VariableDeclSplitter<Node: SyntaxProtocol> {
    /// A function that takes a `VariableDeclSyntax` and returns a new node, such as a
    /// `CodeBlockItemSyntax` , that wraps it.
    ...
    ///
    /// This is an implicitly-unwrapped optional because it isn't initialized until
    /// `nodes(bySplitting:)` is called.
    private var varDecl: VariableDeclSyntax!

    /// The list of nodes generated by splitting the variable declaration into individual bindings.
    private var nodes = [Node]()

    /// Tracks whether the trivia of `varDecl` has already been fixed up for nodes after the first.
    private var fixedUpTrivia = false

    /// Creates a new variable declaration splitter.
    ///
    /// - Parameter generator: A function that takes a `VariableDeclSyntax` and returns a new node,
    ///   such as a `CodeBlockItemSyntax` , that wraps it.
    init(generator: @escaping (VariableDeclSyntax) -> Node) { self.generator = generator }

    /// Returns an array of nodes generated by splitting the given variable declaration into
    /// individual bindings.
    mutating func nodes(bySplitting varDecl: VariableDeclSyntax) -> [Node] {
        ...
        // on the previous token) so that we can re-associate it if we flush the bindings out as
        ...
        for binding in varDecl.bindings {
            if binding.initializer != nil {
                // If this is the only initializer in the queue so far, that's ok. If it's an
                // initializer following other un-flushed lone identifier bindings, that's not valid
                // Swift. But in either case, we'll flush them as a single decl.
    ...
    /// Replaces the original variable declaration with a copy of itself with updates trivia
    /// appropriate for subsequent declarations inserted by the rule.
    private mutating func fixOriginalVarDeclTrivia() {
        guard !fixedUpTrivia else { return }

        // We intentionally don't try to infer the indentation for subsequent lines because the
        // pretty printer will re-indent them correctly; we just need to ensure that a newline is
        // inserted before new decls.
    ...
    /// Flushes any remaining bindings as individual variable declarations where each has the given
    /// type annotation.
