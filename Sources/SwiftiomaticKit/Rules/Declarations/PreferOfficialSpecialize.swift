...
///
/// `@_specialize` was the experimental, underscore-prefixed spelling. The
/// official `@specialize` attribute is available in Swift 6.3.
///
/// Lint: Using `@_specialize` raises a warning.
///
/// Rewrite: The attribute is replaced with `@specialize`, preserving its
/// argument list (e.g. `where`, `exported:`, `kind:`).
final class PreferOfficialSpecialize: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        guard identType.name.text == "_specialize" else {
            return node
        }
...
extension Finding.Message {
    fileprivate static var useSpecializeAttribute: Finding.Message {
        "replace '@_specialize' with '@specialize'"
    }
}
