...
/// A computed-property or subscript setter that never reads its parameter
...
final class UnusedSetterValue: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: AccessorDeclSyntax) -> SyntaxVisitorContinueKind {
        ...
        guard let body = node.body else {
            // Protocol stubs `{ get set }` have no body.
            return .visitChildren
        }
...
extension Finding.Message {
    fileprivate static func unusedSetterValue(_ name: String) -> Finding.Message {
