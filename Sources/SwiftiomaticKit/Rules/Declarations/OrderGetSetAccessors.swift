...
///
/// Configure via `orderGetSetAccessors.order` :
...
///
/// Lint-only: this rule does not auto-fix because reordering accessors with non-trivial bodies
/// risks misplacing trailing comments and trivia.
final class OrderGetSetAccessors: LintSyntaxRule<AccessorOrderConfiguration>, @unchecked Sendable {
    ...
    private func order(of node: AccessorBlockSyntax) -> AccessorOrderConfiguration.Order? {
        guard case let .accessors(accessors) = node.accessors,
              accessors.count == 2,
              accessors.allSatisfy({ $0.body != nil }) else { return nil }
...
fileprivate extension Finding.Message {
    static func orderGetSetAccessors(
...
package struct AccessorOrderConfiguration: SyntaxRuleValue {
    ...
    package init(from decoder: any Decoder) throws {
        ...
        order = try container.decodeIfPresent(Order.self, forKey: .order) ?? .getSet
    }
}
