...
/// Use angle brackets ( `extension Array<Foo>` ) for generic type extensions instead of type
/// constraints ( `extension Array where Element == Foo` ).
///
/// Swift 5.7+ supports angle bracket syntax in extension declarations. When a `where` clause
/// constrains all generic parameters of a known type to concrete types, the angle bracket form is
/// more concise.
///
/// Known types: `Array` , `Set` , `Optional` , `Dictionary` , `Collection` , `Sequence` .
///
/// Lint: An extension with a `where` clause that can be replaced by angle brackets raises a
/// warning.
///
/// Rewrite: The `where` clause constraints are moved into angle bracket syntax on the extended
/// type.
final class PreferAngleBracketExtensions: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        let genericArgs = requiredParams.enumerated().map { i, param -> GenericArgumentSyntax in
            ...
            return GenericArgumentSyntax(
                ...
                trailingComma: isLast
                    ? nil
                    : TokenSyntax(.comma, trailingTrivia: .space, presence: .present)
            )
        }
        ...
        if remainingRequirements.isEmpty {
            ...
            for (i, req) in remainingRequirements.enumerated() {
                ...
                if i == remainingRequirements.count - 1 { modified.trailingComma = nil }
...
fileprivate extension Finding.Message {
    static func useAngleBracketSyntax(type: String) -> Finding.Message {
