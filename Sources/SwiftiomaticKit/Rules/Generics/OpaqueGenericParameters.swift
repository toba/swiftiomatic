...
/// Use opaque generic parameters ( `some Protocol` ) instead of named generic parameters with
/// constraints ( `<T: Protocol>` ) where equivalent.
///
/// This rule applies to `func` , `init` , and `subscript` declarations. A generic type parameter is
...
final class OpaqueGenericParameters: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        if result.genericWhereClause == nil, visited.genericWhereClause != nil {
            ...
            if let returnClause = result.signature.returnClause {
                ...
                result.signature.parameterClause.rightParen = result.signature.parameterClause
                    .rightParen.with(\.trailingTrivia, [])
            }
        }
    ...
    private struct TypeInfo {
        ...
        var sameTypeTarget: TypeSyntax?
        var whereRequirementIndices: [Int] = []
        var eligible = true

        func replacementType() -> TypeSyntax? {
            if let target = sameTypeTarget { return target.trimmed }
            if conformances.isEmpty {
                return someType(IdentifierTypeSyntax(name: .identifier("Any")))
            }
            if conformances.count == 1 { return someType(conformances[0].trimmed) }
...
fileprivate extension Finding.Message {
    static let useOpaqueGenericParameters: Finding.Message =
