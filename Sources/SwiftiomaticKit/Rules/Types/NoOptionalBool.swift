...
/// Optional booleans are confusing — three states ( `true` , `false` , `nil` ) where two are
/// usually enough. Prefer a non-optional `Bool` with a sensible default, or model the third state
/// with an enum so the cases are named.
///
...
fileprivate extension Finding.Message {
    static let noOptionalBool: Finding.Message =
