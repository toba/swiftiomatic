...
final class PreferShorthandTypeNames: StructuralFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    /// (which is `nil` because the post-recursion node is detached from the original tree). Set
    /// only by the static `transform` overloads invoked from `CompactSyntaxRewriter` via the
    ...
    override func visit(_ node: IdentifierTypeSyntax) -> TypeSyntax {
        ...
        // If the node is a direct child of a member type identifier (e.g., `Array<Int>.Index` ),
        // the type must be left in long form for the compiler. Fall back to the default visitation
        // logic, so that we don't skip children that may need to be rewritten. For example,
        // `Foo<Array<Int>>.Bar` can still be transformed to `Foo<[Int]>.Bar` because the member
        // reference is not directly attached to the type that will be transformed, but we need to
        // visit the children so that we don't skip this).
        ...
        // Ensure that all arguments in the clause are shortened and in the expected format by
        // visiting the argument list, first.
        ...
        switch node.name.text {
            case "Array":
                guard case let .type(typeArgument) = genericArgumentList.firstAndOnly?.argument
                else {
                    newNode = nil
                    break
                }
        ...
        if let newNode { return newNode }

        // Even if we don't shorten this specific type that we're visiting, we may have rewritten
        // something in the generic argument list that we recursively visited, so return the
    ...
    override func visit(_ node: GenericSpecializationExprSyntax) -> ExprSyntax {
        ...
        // nodes, so we may need to translate the arguments inside the generic argument list---which
        // are types---to the appropriate equivalent. Ignore nodes where the expression being
        // specialized isn't a simple identifier.
        guard let expression = node.expression.as(DeclReferenceExprSyntax.self) else {
            return super.visit(node)
        }

        // Member access after a shorthand type in an expression context appears to be fine in
        // Swift. For example, `let x: [Int].Index` is not permitted but `let x = [Int].Index()` is
        // allowed. To avoid introducing inconsistencies in users' code, we choose not to apply a
        // shorthand transform to these member accesses in expression contexts, even when they would
        // be valid. However, we still fall back to the default visitation logic, so that we don't
        // skip children that may need to be rewritten. For example, `Foo<Array<Int>>.Bar()` can
        // still be transformed to `Foo<[Int]>.Bar()` because the member reference is not directly
        // attached to the type that will be transformed, but we need to visit the children so that
        // we don't skip this).
        let effectiveParent = node.parent ?? compactPipelineParent
        ...
        // rewritten something in the generic argument list that we recursively visited, so return
        // the original node with that swapped out.
        var result = node
    ...
    /// Returns a `TypeSyntax` representing a shorthand dictionary type (e.g., `[Foo: Bar]` ) with
    /// the given key/value types and trivia.
    ...
    private func shorthandOptionalType(
        ...
        // Certain types must be wrapped in parentheses before using shorthand optional syntax to
        // avoid the "?" from binding incorrectly when re-parsed. Attach the leading trivia to the
        // left-paren that we're adding in these cases.
        switch Syntax(wrappedType).as(SyntaxEnum.self) {
            case let .attributedType(attributedType):
    ...
    /// given type, or nil if the conversion is not possible because the element type does not have
    /// a valid expression representation.
    private func makeArrayTypeExpression(
        elementType: TypeSyntax,
        leftSquare: TokenSyntax,
        rightSquare: TokenSyntax
    ) -> ArrayExprSyntax? {
        guard let elementTypeExpr = expressionRepresentation(of: elementType) else { return nil }
    ...
    /// Returns a `DictionaryExprSyntax` whose single key/value pair is the expression
    /// representations of the given key and value types, or nil if the conversion is not possible
    /// because either the key type or value type does not have a valid expression representation.
    private func makeDictionaryTypeExpression(
        ...
        guard let keyTypeExpr = expressionRepresentation(of: keyType),
              let valueTypeExpr = expressionRepresentation(of: valueType) else { return nil }
    ...
    /// representation of the given type, or nil if the conversion is not possible because either
    /// the key type or value type does not have a valid expression representation.
    ...
    /// Returns an `ExprSyntax` that is syntactically equivalent to the given `TypeSyntax` , or nil
    /// if it wouldn't be valid.
    private func expressionRepresentation(of type: TypeSyntax) -> ExprSyntax? {
        switch Syntax(type).as(SyntaxEnum.self) {
            case let .identifierType(simpleTypeIdentifier):
            ...
            case let .memberType(memberTypeIdentifier):
                guard let baseType = expressionRepresentation(of: memberTypeIdentifier.baseType)
                else { return nil }
            ...
            case let .dictionaryType(dictionaryType):
            ...
            case let .optionalType(optionalType):
    ...
    private func makeFunctionTypeExpression(
        ...
        guard let parameterExprs = expressionRepresentation(of: parameters),
              let returnTypeExpr = expressionRepresentation(of: returnType) else { return nil }
    ...
    /// Returns true if the given pattern binding represents a stored property/variable (as opposed
    /// to a computed property/variable).
    private func isStoredProperty(_ node: PatternBindingSyntax) -> Bool {
        guard let accessor = node.accessorBlock else { return true }

        guard case let .accessors(accessors) = accessor.accessors else { return false }

        for accessorDecl in accessors {
            // Look for accessors that indicate that this is a computed property. If none are found,
            // then it is a stored property (e.g., having only observers like `willSet/didSet` ).
    ...
    private func nearestAncestor<NodeType: SyntaxProtocol, AncestorType: SyntaxProtocol>(
        ...
        while let existingParent = parent {
            if existingParent.is(type) { return existingParent.as(type) }
    ...
    /// The transform fires post-recursion when actually rewriting; this hook only emits findings.
    static func willEnter(_ node: IdentifierTypeSyntax, context: Context) {
        ...
        switch name {
            case "Array", "Optional": guard arity == 1 else { return }
            case "Dictionary": guard arity == 2 else { return }
            default: return
        }
    ...
    static func willEnter(_ node: GenericSpecializationExprSyntax, context: Context) {
        ...
        // In an expression context, every type argument must have a valid expression representation
        // (e.g. `()` cannot become `()` as an expression because the parser interprets `()` as the
        // void *value*).
    ...
    /// Static counterpart to `expressionRepresentation(of:)` returning only success/failure. Empty
    /// tuple types ( `()` ) have no expression representation in an expression context.
    fileprivate static func hasValidExpressionRepresentation(_ type: TypeSyntax) -> Bool {
        switch Syntax(type).as(SyntaxEnum.self) {
            case let .tupleType(tupleType):
                ...
                for elem in tupleType.elements where !hasValidExpressionRepresentation(elem.type) {
                    return false
                }
                return true
            case let .arrayType(arr): return hasValidExpressionRepresentation(arr.element)
            case let .dictionaryType(dict):
                return hasValidExpressionRepresentation(dict.key)
                    && hasValidExpressionRepresentation(dict.value)
            case let .optionalType(opt): return hasValidExpressionRepresentation(opt.wrappedType)
            case let .implicitlyUnwrappedOptionalType(iuo):
                return hasValidExpressionRepresentation(iuo.wrappedType)
            case let .memberType(member): return hasValidExpressionRepresentation(member.baseType)
            case let .functionType(fnType):
                for elem in fnType.parameters where !hasValidExpressionRepresentation(elem.type) {
                    return false
                }
                return hasValidExpressionRepresentation(fnType.returnClause.type)
            case let .packElementType(pack): return hasValidExpressionRepresentation(pack.pack)
            case let .identifierType(ident):
                // The legacy pipeline recursively visits generic args first, shortening eligible
                // ones. We mirror that: an IdentifierType that *would* be shortened to a form with
                // no expression representation (e.g. `Optional<()>` → `()?` , which fails because
                // `()` has no expression representation) cannot itself be expressed.
                guard let clause = ident.genericArgumentClause else { return true }
                let arity = clause.arguments.count

                switch ident.name.text {
                    case "Optional", "Array":
                        guard arity == 1, case let .type(arg) = clause.arguments.first?.argument
                        else { return true }
                    ...
                    case "Dictionary":
                        ...
                        if case let .type(k) = args[0].argument,
                           case let .type(v) = args[1].argument
                        {
    ...
    /// Static counterpart of `isTypeOfUninitializedStoredVar` that uses the pre-traversal
    /// `node.parent` chain (which is intact during `willEnter` ).
    fileprivate static func isUninitializedStoredVar(node: IdentifierTypeSyntax) -> Bool {
        guard let typeAnnotation = node.parent?.as(TypeAnnotationSyntax.self),
              ...
              patternBinding.initializer == nil,
              let variableDecl = nearestAncestor(of: patternBinding, type: VariableDeclSyntax.self),
              variableDecl.bindingSpecifier.tokenKind == .keyword(.var) else { return false }
        return true
    }
...
fileprivate extension Finding.Message {
    static func useTypeShorthand(type: String) -> Finding.Message {
