...
/// `String(decoding: data, as: UTF8.self)` silently substitutes `U+FFFD` for invalid bytes, hiding
/// decoding errors. Prefer the failable `String(bytes: data, encoding: .utf8)` initializer so the
/// caller can handle invalid input explicitly.
///
...
final class UseFailableStringInit: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
...
fileprivate extension Finding.Message {
    static let useFailableStringInit: Finding.Message =
