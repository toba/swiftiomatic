...
/// Lint the `swap(&a, &b); a.removeAll(…)` (or `b.removeAll(…)`) pattern. It
/// almost always indicates a hand-rolled alternating-buffer parser that loses
/// `Array`/`Data` CoW guarantees and is brittle when refactored.
final class WarnSwapThenRemoveAll: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: CodeBlockItemListSyntax) -> SyntaxVisitorContinueKind {
        ...
        for i in 0..<items.count - 1 {
            guard let swapCall = swapInOutCall(items[i]),
                  ...
                  removalReceiver == a || removalReceiver == b
            else {
                continue
            }
    ...
    private func swappedNames(_ call: FunctionCallExprSyntax) -> (String, String)? {
        ...
        guard let aName = inoutIdentifier(args[0].expression),
              let bName = inoutIdentifier(args[1].expression)
        else {
            return nil
        }
        return (aName, bName)
    ...
    private func inoutIdentifier(_ expr: ExprSyntax) -> String? {
        guard let inout_ = expr.as(InOutExprSyntax.self),
              let ident = inout_.expression.as(DeclReferenceExprSyntax.self)
        else {
            return nil
        }
...
extension Finding.Message {
    fileprivate static func swapThenRemoveAll(a: String, b: String, removed: String) -> Finding.Message {
