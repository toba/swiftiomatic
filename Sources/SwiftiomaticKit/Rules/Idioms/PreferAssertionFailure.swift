...
/// Replace `assert(false, ...)` with `assertionFailure(...)` and `precondition(false, ...)` with
/// `preconditionFailure(...)` .
///
/// The `Failure` variants more clearly express intent: the code path should never be reached. They
/// also have `Never` return type, enabling the compiler to prove exhaustiveness.
///
...
final class PreferAssertionFailure: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        guard let firstArg = node.arguments.first,
              ...
              boolLiteral.literal.tokenKind == .keyword(.false) else { return ExprSyntax(node) }
...
fileprivate extension Finding.Message {
    static func useFailureVariant(name: String, replacement: String) -> Finding.Message {
