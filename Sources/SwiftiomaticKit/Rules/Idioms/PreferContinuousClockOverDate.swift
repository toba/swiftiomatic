...
///
/// `ContinuousClock.now` (paired with `start.duration(to: .now)` or
/// `clock.measure { … }`) is monotonic, allocation-free, and unaffected by
/// wall-clock adjustments.
///
/// Lint-only: autofixing requires rewriting the start-time site as well, which
/// is outside the rule's single-node scope.
final class PreferContinuousClockOverDate: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: MemberAccessExprSyntax) -> SyntaxVisitorContinueKind {
        // Only the property form: `Date().timeIntervalSinceNow`. Skip the
        // member when it's the callee of a function call (handled above).
...
extension Finding.Message {
    fileprivate static let preferContinuousClock: Finding.Message =
