...
/// `do { return .success(...) } catch { return .failure(...) }` . The pattern is mechanical —
/// `throws(E) -> T` expresses the same contract more directly.
final class UseTypedThrowsNotResult: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: FunctionDeclSyntax) -> SyntaxVisitorContinueKind {
        guard returnsResult(node),
              ...
              endsWithFailure(catchClause.body.statements) else { return .visitChildren }
    ...
    private func returnsResult(_ node: FunctionDeclSyntax) -> Bool {
        guard let returnType = node.signature.returnClause?.type,
              ...
              ident.name.text == "Result" else { return false }
        return true
    }
    ...
    private func endsWithReturnOfMember(
        ...
        guard let member = callee.as(MemberAccessExprSyntax.self),
              member.declName.baseName.text == name else { return false }
        return true
    }
}

fileprivate extension Finding.Message {
    static let preferTypedThrows: Finding.Message =
