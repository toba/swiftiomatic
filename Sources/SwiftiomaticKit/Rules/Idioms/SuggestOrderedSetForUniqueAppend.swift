...
///
/// Maintaining uniqueness in an `Array` via `contains` + `append` is O(n) per
/// insertion. `OrderedSet` (swift-collections) preserves insertion order with
/// O(1) average insertion and `contains`.
///
/// Lint-only: rewriting would change the variable's declared type, which is
/// outside the rule's structural scope.
final class SuggestOrderedSetForUniqueAppend: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    override class var group: ConfigurationGroup? { .idioms }

    override func visit(_ node: IfExprSyntax) -> SyntaxVisitorContinueKind {
        guard node.conditions.count == 1,
              let firstCondition = node.conditions.first,
              case .expression(let conditionExpr) = firstCondition.condition,
              ...
              let collectionExpr = containsMember.base
        else {
        ...
        guard node.body.statements.count == 1,
              ...
              let appendCollectionExpr = appendMember.base
        else {
            return .visitChildren
        }

        guard sameExpression(collectionExpr, appendCollectionExpr),
              sameExpression(containsArg, appendArg)
        else {
            return .visitChildren
        }
...
extension Finding.Message {
    fileprivate static let suggestOrderedSet: Finding.Message =
