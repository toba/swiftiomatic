...
/// Prefer `reduce(into:_:)` over `reduce(_:_:)` when the accumulator is a copy-on-write value
...
final class PreferReduceInto: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    override class var group: ConfigurationGroup? { .idioms }

    override func visit(_ node: FunctionCallExprSyntax) -> SyntaxVisitorContinueKind {
        guard
            let nameToken = node.reduceNameToken,
            ...
            firstArgument.expression.isCopyOnWriteSeed
        else {
            return .visitChildren
        }
        diagnose(.preferReduceInto, on: nameToken)
        return .visitChildren
    }
}

extension Finding.Message {
    fileprivate static let preferReduceInto: Finding.Message =
        "prefer 'reduce(into:_:)' over 'reduce(_:_:)' for copy-on-write seeds"
}

extension FunctionCallExprSyntax {
    fileprivate var reduceNameToken: TokenSyntax? {
        if let member = calledExpression.as(MemberAccessExprSyntax.self) {
            return member.declName.baseName
        }
        if let ref = calledExpression.as(DeclReferenceExprSyntax.self) {
            return ref.baseName
        }
        return nil
    }
...
extension ExprSyntax {
    fileprivate var isCopyOnWriteSeed: Bool {
        ...
        if let call = self.as(FunctionCallExprSyntax.self) {
            ...
            if call.calledExpression.isCopyOnWriteSeed {
                return true
            }
        }
        return false
    ...
    fileprivate var cowIdentifierExpr: DeclReferenceExprSyntax? {
...
extension DeclReferenceExprSyntax {
    fileprivate static let copyOnWriteTypeNames: Set<String> = ["Array", "Dictionary", "Set"]

    fileprivate var isCopyOnWriteTypeName: Bool {
        Self.copyOnWriteTypeNames.contains(baseName.text)
    }
}
