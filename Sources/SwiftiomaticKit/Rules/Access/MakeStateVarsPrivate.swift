...
///
/// SwiftUI `@State` and `@StateObject` properties should be `private` because they are owned by the
/// view and should not be set from outside. If no access control modifier is present, `private` is
/// added. Existing access modifiers (including `private(set)` ) and `@Previewable` properties are
/// left unchanged.
///
...
fileprivate extension Finding.Message {
    static let addPrivateToStateProperty: Finding.Message = "add 'private' to this @State property"
}
