...
/// Flag `AnyView(...)` constructed inside a `ForEach` body. Type-erasing each row defeats SwiftUI's
/// structural identity, forces extra invalidation, and almost always indicates a missing
/// `@ViewBuilder` helper or a `Group { ... }` with branches. Move the branching into a
/// `@ViewBuilder` function or use `if / switch` directly inside the closure.
final class NoAnyViewInForEach: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: FunctionCallExprSyntax) -> SyntaxVisitorContinueKind {
        ...
        if isInsideForEachClosure(node) { diagnose(.anyViewInForEach, on: node.calledExpression) }
        ...
        while let cur = current {
               ...
               callee.baseName.text == "ForEach"
            {
                ...
                if let trailing = call.trailingClosure,
                   ...
                   pos < trailing.endPosition { return true }

                for additional in call.additionalTrailingClosures {
                    if pos >= additional.position, pos < additional.endPosition { return true }
                }
                return false
            }
            current = cur.parent
