...
/// Flag uses of system notifications whose name has a typed
/// `NotificationCenter.MainActorMessage` adapter on macOS 26 / iOS 26+. Matches
/// `addObserver(forName:...)`, `.notifications(named:)`, `.messages(named:)`, and
/// `post(name:...)` whose name argument matches an entry in
...
///
/// Flag-only — the suggested replacement (`addObserver(of:for:)` / `messages(of:)`) reshapes
/// the surrounding call and is out of scope for an autofix.
    ...
    override func visit(_ node: FunctionCallExprSyntax) -> SyntaxVisitorContinueKind {
        for arg in node.arguments {
            guard let label = arg.label?.text,
                  ...
                  let replacement = NotificationAPI.adapter(for: arg.expression)
            else { continue }
...
extension Finding.Message {
    fileprivate static func adapted(name: String, replacement: String) -> Finding.Message {
