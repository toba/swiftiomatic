...
final class PreferEarlyExits: RewriteSyntaxRule<BasicRuleValue>, @unchecked Sendable {
    ...
    override func visit(_ node: CodeBlockItemListSyntax) -> CodeBlockItemListSyntax {
        ...
        for codeBlockItem in node {
            // The `elseBody` of an `IfExprSyntax` will be a `CodeBlockSyntax` if it's an `else` block, or
            // another `IfExprSyntax` if it's an `else if` block. We only want to handle the former.
            ...
            for trueStmt in trueBlock.statements { newItems.append(trueStmt) }
        }
