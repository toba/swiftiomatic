...
/// a warning is raised.
final class FlagIdenticalOperands: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: InfixOperatorExprSyntax) -> SyntaxVisitorContinueKind {
        ...
        if normalized(node.leftOperand) == normalized(node.rightOperand) {
            diagnose(.flagIdenticalOperands, on: node.leftOperand)
        }
...
fileprivate extension Finding.Message {
    static let flagIdenticalOperands: Finding.Message =
