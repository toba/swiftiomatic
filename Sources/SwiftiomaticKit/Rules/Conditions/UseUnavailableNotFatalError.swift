...
///
/// Inverting an availability check via an empty `if` -body and a non-empty `else` -body is harder
/// to read than the direct `#unavailable` form (Swift 5.6+). This rule rewrites the simple shape;
/// it does not touch chains where the `else` body has its own availability check (rewriting those
/// is not a simple inversion).
///
...
final class UseUnavailableNotFatalError: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        guard ifExpr.body.statements.isEmpty,
              ...
              case let .availability(availability) = onlyCondition.condition,
              let elseBody = ifExpr.elseBody,
              case let .codeBlock(elseBlock) = elseBody,
              !elseAvailabilityCheckChainInvolved(elseBlock) else { return ExprSyntax(ifExpr) }
        ...
        Self.diagnose(
            .useUnavailableNotFatalError(currentlyAvailable: isAvailable),
            on: availability,
            context: context
        )

        // Build the inverted availability keyword.
        let newKeyword: TokenSyntax = isAvailable
        ...
        // if-body's left brace had a trailing space before its empty body; keep the elseBlock's own
        // brace trivia (which already represents the proper indentation), and just preserve the
        // original `if {` 's left-brace leading trivia.
    ...
    private static func hasAvailabilityCondition(_ ifExpr: IfExprSyntax) -> Bool {
        ...
        if case let .ifExpr(nested) = ifExpr.elseBody { return hasAvailabilityCondition(nested) }
        return false
    }
}

fileprivate extension Finding.Message {
    static func useUnavailableNotFatalError(currentlyAvailable: Bool) -> Finding.Message {
        currentlyAvailable
            ? "use '#unavailable' instead of '#available' with an empty body"
            : "use '#available' instead of '#unavailable' with an empty body"
    }
}
