...
final class SortSwitchCases: StructuralFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    override func visit(_ node: SwitchCaseSyntax) -> SwitchCaseSyntax {
        guard case var .case(caseLabel) = node.label else { return node }
        ...
        for (i, item) in sorted.enumerated() {
            if item.whereClause != nil, i < sorted.count - 1 { return node }
        }
        ...
        for (i, sortedItem) in sorted.enumerated() {
            ...
            if i < sorted.count - 1 {
                ...
                if let comma = sortedItem.trailingComma {
                    ...
                    // Item was previously last (no comma) — use trivia from original position's
                    // comma
    ...
    private func sortKeys(for pattern: PatternSyntax) -> [String] {
        ...
        for token in pattern.tokens(viewMode: .sourceAccurate) {
            switch token.tokenKind {
                case let .identifier(name): parts.append(name)
                case let .stringSegment(text): parts.append(text)
                case let .integerLiteral(value): parts.append(normalizeNumericLiteral(value))
                case .keyword: parts.append(token.text)
                case .wildcard: parts.append("_")
                default: break
            }
        }
        return parts
...
fileprivate extension Finding.Message {
    static let sortSwitchCases: Finding.Message = "sort switch case items alphabetically"
}
