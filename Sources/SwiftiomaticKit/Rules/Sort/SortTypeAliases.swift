...
final class SortTypeAliases: RewriteSyntaxRule<BasicRuleValue>, @unchecked Sendable {
    ...
    override func visit(_ node: TypeAliasDeclSyntax) -> DeclSyntax {
        ...
        // Sort by type name (strip `any` prefix for comparison since it applies to entire composition)
        ...
        // Rebuild preserving positional trivia structure.
        // Each position has a "slot" with trivia from the original element at that index.
        ...
        for (i, sortedElem) in deduped.enumerated() {
            ...
            if i < elements.count {
                ...
                if i == deduped.count - 1 {
                    newType.trailingTrivia = elements[elements.count - 1].type.trailingTrivia
                } else {
                    newType.trailingTrivia = elements[i].type.trailingTrivia
                }
            }
            ...
            newElements.append(
                CompositionTypeElementSyntax(type: TypeSyntax(newType), ampersand: ampersand)
            )
        }
...
extension Finding.Message {
    fileprivate static let sortTypealiases: Finding.Message =
        "sort protocol composition types alphabetically"
}
