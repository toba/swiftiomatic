...
/// @\_implementationOnly imports, and 4) @testable imports. These groups are separated by a single
/// blank line. Blank lines in between the import declarations are removed.
///
/// Logical grouping is enabled by default but can be disabled via the
...
///
/// Lint: If an import appears anywhere other than the beginning of the file it resides in, not
/// lexicographically ordered, or (optionally) not in the appropriate import group, a lint error is
/// raised.
///
...
final class SortImports: StructuralFormatRule<SortImportsConfiguration>, @unchecked Sendable {
        ...
        for (index, line) in lines.enumerated() {
            ...
            if ruleConfig.shouldGroupImports {
                ...
                switch line.type {
                    ...
                    case .comment: commentBuffer.append(line)
                }
            } else {
                switch line.type {
    ...
    private func checkGrouping<C: Collection>(_ lines: C) where C.Element == Line {
        ...
        for line in lines {
            ...
            switch lineType {
                case .declImport: seenDeclImport = true
                case .implementationOnlyImport: seenImplementationOnlyImport = true
                case .testableImport: seenTestableImport = true
                case .codeBlock: seenCodeBlock = true
                default: ()
            }
            ...
            guard ruleConfig.shouldGroupImports else { continue }
            ...
            if seenImplementationOnlyImport {
                switch lineType {
                    case .regularImport, .declImport:
                        diagnose(
                            .groupImports(
                                before: lineType, after: LineType.implementationOnlyImport),
                            on: line.firstToken
                        )
                    default: ()
                }
                ...
                case .regularImport, .declImport, .implementationOnlyImport, .testableImport:
                    ...
                    if let previousMatchingImportIndex = visitedImports[fullyQualifiedImport] {
                        ...
                        guard !duplicateLine.import.trailingTrivia.isEmpty,
                            ...
                            if !line.trailingTrivia.isEmpty { duplicateLine.import = line }
    ...
    for block in codeBlockItemList {
        for piece in block.leadingTrivia {
            switch piece {
                ...
                case let .newlines(N): for _ in 0..<N { appendNewLine() }
                default: currentLine.leadingTrivia.append(piece)
            }
        }

        if block.item.is(ImportDeclSyntax.self) {
            // Always create a new `Line` for each import statement, so they can be reordered.
            if currentLine.syntaxNode != nil { appendNewLine() }
            let sortable = context.shouldFormat(SortImports.self, node: Syntax(block))
            var blockWithoutTrailingTrivia = block
            blockWithoutTrailingTrivia.trailingTrivia = []
            currentLine.syntaxNode = .importCodeBlock(
                blockWithoutTrailingTrivia, sortable: sortable)
            ...
            if let syntaxNode = currentLine.syntaxNode {
                ...
                switch syntaxNode {
                    ...
                    case let .nonImportCodeBlocks(existingCodeBlocks):
        ...
        for piece in block.trailingTrivia { currentLine.trailingTrivia.append(piece) }
    }
...
private func convertToCodeBlockItems(lines: [Line]) -> [CodeBlockItemSyntax] {
    ...
    for line in lines {
        ...
        if let syntaxNode = line.syntaxNode {
            switch syntaxNode {
                case .importCodeBlock(let codeBlock, _): append(codeBlockItem: codeBlock)
                case let .ifConfigCodeBlock(ifConfigCodeBlock):
                    append(codeBlockItem: ifConfigCodeBlock)
                case let .nonImportCodeBlocks(codeBlocks):
                    codeBlocks.forEach(append(codeBlockItem:))
            }
        }

        // Merge multiple newlines together into a single trivia piece by updating its count.
        if let lastPiece = pendingLeadingTrivia.last, case let .newlines(count) = lastPiece {
    ...
    if Trivia(pieces: pendingLeadingTrivia).hasAnyComments, !output.isEmpty {
        ...
        lastItem.trailingTrivia = Trivia(
            pieces: lastItem.trailingTrivia.pieces + pendingLeadingTrivia
        )
...
enum LineType: CustomStringConvertible {
    ...
    var description: String {
        switch self {
            case .regularImport: "regular"
            case .declImport: "declaration"
            case .implementationOnlyImport: "implementationOnly"
            case .testableImport: "testable"
            case .codeBlock: "code"
...
/// A Line more or less represents a literal printed line in a source file. A line can be a comment,
/// code, code with a trailing comment, or a blank line. For import statements, a Line will
...
private final class Line {
    ...
    enum SyntaxNode {
        ...
        /// A single code block item whose content must be an if config decl. This is used to sort
        /// conditional imports.
    ...
    var firstToken: TokenSyntax? {
        guard let syntaxNode else { return nil }
        switch syntaxNode {
            ...
            case let .ifConfigCodeBlock(codeBlock):
                return codeBlock.firstToken(viewMode: .sourceAccurate)
        }
    }
    ...
    private func importType(of importDecl: ImportDeclSyntax) -> LineType {
        let importIdentifierTypes = importDecl.attributes.compactMap {
            $0.as(AttributeSyntax.self)?.attributeName
        }
        let importAttributeNames = importIdentifierTypes.compactMap {
            $0.as(IdentifierTypeSyntax.self)?.name.text
        }
    ...
    var description: String {
        ...
        if let syntaxNode {
            switch syntaxNode {
                ...
                case .ifConfigCodeBlock: description += "if config code block "
            }
        }
...
fileprivate extension Finding.Message {
    static let placeAtTopOfFile: Finding.Message = "place imports at the top of the file"

    static func groupImports(before: LineType, after: LineType) -> Finding.Message {
        "place \(before) imports before \(after) imports"
    }

    static let removeDuplicateImport: Finding.Message = "remove this duplicate import"
...
package struct SortImportsConfiguration: SyntaxRuleValue {
    ...
    package init(from decoder: any Decoder) throws {
        ...
        includeConditionalImports = try container.decodeIfPresent(
            Bool.self, forKey: .includeConditionalImports) ?? false
        shouldGroupImports = try container.decodeIfPresent(Bool.self, forKey: .shouldGroupImports)
            ?? true
        sortOrder = try container.decodeIfPresent(SortOrder.self, forKey: .sortOrder)
            ?? .alphabetical
    }
}
