...
/// Doc comments (`///` and `/** */`) inside function, initializer, and accessor
...
final class NoLocalDocComments: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    override static var key: String { "noLocalDocComments" }
    ...
    private func scanBody(_ body: CodeBlockSyntax) {
        for token in body.tokens(viewMode: .sourceAccurate) {
            scanTrivia(token.leadingTrivia, on: token, isLeading: true, declToken: nestedDeclToken(for: token))
    ...
    private func scanTrivia(
        ...
        // If the token is the leading token of a nested declaration that may carry a
        // doc comment (func/init/deinit/subscript/var/let/...), skip its leading trivia.
            ...
            switch trivia[index] {
                case .docLineComment, .docBlockComment:
                    let anchor: FindingAnchor =
                        isLeading ? .leadingTrivia(index) : .trailingTrivia(index)
    ...
    /// Returns the token if it is the first token of a nested declaration whose
    /// doc comment should be preserved (currently: nested `func`).
...
extension Finding.Message {
    fileprivate static let localDocComment: Finding.Message =
