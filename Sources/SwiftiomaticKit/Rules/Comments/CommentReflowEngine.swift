...
package enum CommentReflowEngine {
    ...
    package static func reflow(
        ...
        var output: [String] = []
        for block in blocks {
            block.render(into: &output, width: availableWidth)
        }
    ...
    fileprivate enum Block {
        ...
        func render(into out: inout [String], width: Int) {
            ...
            case .blank:
                out.append("")
            case .codeFence(let open, let body, let close):
                out.append(open)
            ...
            case .list(let items, _):
                for item in items {
                    ...
                    if wrapped.isEmpty {
                        ...
                        for tail in wrapped.dropFirst() {
                            out.append(continuation + tail)
                        }
                    }
                    ...
                    for line in nestedOut {
                        if line.isEmpty { out.append("") } else { out.append(continuation + line) }
                    }
                }
    ...
    private static func listMarker(_ line: String) -> (marker: String, bodyOffset: Int)? {
        ...
        if ch == "-" || ch == "*" || ch == "+" {
            ...
            if next < line.endIndex, line[next] == " " {
                ...
                return (marker: marker, bodyOffset: line.distance(from: line.startIndex, to: markerEnd))
            }
            return nil
        }
        // Ordered: digits + "."
    ...
    private static func parseList(
        ...
        var firstItemMarkerIndent: Int? = nil
        ...
        while i < lines.count {
            ...
            let bodyText = String(line[line.index(line.startIndex, offsetBy: m.bodyOffset)...])
            var item = ListItem(
                marker: String(repeating: " ", count: leading) + String(m.marker.drop(while: { $0 == " " })),
    ...
    package static func tokenize(_ text: String) -> [String] {
        ...
        var pending = ""
        func flush() {
            if !pending.isEmpty { atoms.append(pending); pending = "" }
        }
