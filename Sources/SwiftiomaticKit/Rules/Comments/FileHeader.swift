...
/// configured text. When configured with an empty string, any existing file header is removed. File
/// header comments are line comments ( `//` ) or block comments ( `/* */` ) at the start of the
/// file, before any blank line, doc comment, or code. Doc comments ( `///` , `/** */` ) are not
/// considered file header comments.
///
...
final class FileHeader: StructuralFormatRule<FileHeaderConfiguration>, @unchecked Sendable {
    ...
    override class var defaultValue: FileHeaderConfiguration {
        var config = FileHeaderConfiguration()
    ...
    override func visit(_ node: SourceFileSyntax) -> SourceFileSyntax {
        ...
        guard let newTrivia = processHeader(trivia: trivia, text: text, hasCodeAfter: true) else {
            return node
        }
    ...
    /// Returns the new trivia if it differs from the current trivia, or `nil` if no change is
    /// needed.
...
fileprivate extension Finding.Message {
    static let updateFileHeader: Finding.Message = "update file header to match configured text"
}
...
package struct FileHeaderConfiguration: SyntaxRuleValue {
    ...
    /// The exact header text every file must begin with. May contain newlines for multi-line
    /// headers. When `nil` or empty, any existing file header is removed instead.
