...
/// `TODO` and `FIXME` comments with a bracketed date should be resolved by that
/// date.
///
...
final class ExpiringTodo: LintSyntaxRule<ExpiringTodoConfiguration>, @unchecked Sendable {
    override static var key: String { "expiringTodo" }
    override static var group: ConfigurationGroup? { .comments }

    private enum ExpiryLevel {
        case approaching, expired, badFormatting
    }
    ...
    private func scan(trivia: Trivia, baseOffset: Int) {
        ...
        for piece in trivia {
            ...
            let text: String
            switch piece {
                case .lineComment(let t), .blockComment(let t),
                    .docLineComment(let t), .docBlockComment(let t):
                    text = t
                default:
                    continue
            }
            ...
            for match in regex.matches(in: text, options: [], range: range) {
                ...
                guard
                    let level = expiryLevel(for: dateString, config: config),
                    let severity = severity(for: level, config: config)
                else {
                    continue
                }
    ...
    private func expiryLevel(
        ...
        guard
            let approachingDate = calendar.date(
                byAdding: .day,
        ...
        if calendar.compare(today, to: approachingDate, toGranularity: .day) != .orderedAscending {
            return .approaching
        }
        return nil
    }
    ...
    private func message(for level: ExpiryLevel) -> Finding.Message {
        switch level {
            case .approaching: return .todoApproachingExpiry
            case .expired: return .todoExpired
            case .badFormatting: return .todoBadFormatting
        }
    }
...
extension Finding.Message {
    fileprivate static let todoApproachingExpiry: Finding.Message =
        "TODO/FIXME is approaching its expiry and should be resolved soon"
    fileprivate static let todoExpired: Finding.Message =
        "TODO/FIXME has expired and must be resolved"
    fileprivate static let todoBadFormatting: Finding.Message =
...
package struct ExpiringTodoConfiguration: SyntaxRuleValue {
    ...
    package var dateFormat: String = "MM/dd/yyyy"
    /// Opening delimiter that introduces the date inside the TODO comment.
    package var dateDelimitersOpening: String = "["
    /// Closing delimiter that terminates the date inside the TODO comment.
    package var dateDelimitersClosing: String = "]"
    /// Separator between date components.
    package var dateSeparator: String = "/"
    /// Days before expiry at which the TODO starts being reported as
    /// approaching its deadline.
    package var approachingExpiryThreshold: Int = 15
    /// Severity reported for TODOs whose expiry is within
    /// `approachingExpiryThreshold` days.
    ...
    /// Severity reported for TODOs whose date couldn't be parsed using
    /// `dateFormat` and the configured delimiters.
