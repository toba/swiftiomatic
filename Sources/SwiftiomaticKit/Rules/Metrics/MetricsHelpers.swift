...
/// Picks the appropriate severity for a measured metric value against two-tier
/// thresholds. Returns `nil` when the value is at or below the warning threshold
...
/// Counts the number of source lines spanned by the given node, optionally
/// excluding lines that are blank or contain only comments.
func bodyLineCount(
    ...
    // Walk tokens within the node and collect line numbers that contain at
    // least one non-trivia, non-comment token.
    var meaningfulLines = Set<Int>()
    for token in node.tokens(viewMode: .sourceAccurate) {
        let line = token.startLocation(converter: converter).line
        if line > start && line < end {
            meaningfulLines.insert(line)
        }
    }
