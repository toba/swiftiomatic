...
final class RequireObserverRemovalInDeinit: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: FunctionCallExprSyntax) -> SyntaxVisitorContinueKind {
        guard node.isNotificationCenterRemoveObserverCall,
              ...
              ref.baseName.tokenKind == .keyword(.self) else { return .visitChildren }

        diagnose(.requireObserverRemovalInDeinit, on: node)
        return .visitChildren
    }
}

fileprivate extension Finding.Message {
    static let requireObserverRemovalInDeinit: Finding.Message =
...
fileprivate extension FunctionCallExprSyntax {
    ...
    var isNotificationCenterRemoveObserverCall: Bool {
        guard let outer = calledExpression.as(MemberAccessExprSyntax.self),
              ...
              root.baseName.text == "NotificationCenter" else { return false }
        return true
    }
}
