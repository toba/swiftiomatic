...
/// `NotificationCenter.addObserver(forName:object:queue:using:)` returns an
...
/// raised.
final class RetainNotificationObserver: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    private func isAddObserverCall(_ node: FunctionCallExprSyntax) -> Bool {
        guard let memberAccess = node.calledExpression.as(MemberAccessExprSyntax.self),
            memberAccess.declName.baseName.text == "addObserver"
        else {
            return false
        }
    ...
    /// Returns true if the call's value is captured by an enclosing
    /// expression: assigned, returned, used as a function argument, or
    /// included in an array/dictionary literal.
    private func isResultConsumed(_ node: FunctionCallExprSyntax) -> Bool {
        ...
        // `return ...` — but if the enclosing function is
        // `@discardableResult`, the caller may still drop the observer.
        if parent.is(ReturnStmtSyntax.self) {
            return !enclosingFunctionIsDiscardable(node)
        }
        ...
        // Subscript usage: `dict[key] = <call>` puts the call in an
        // ExprListSyntax with an AssignmentExpr.
        ...
        // `_ = nc.addObserver(...)` is intentionally an unsuppressed warning.
        // Other parents (CodeBlockItem) are statement positions: result is
        // discarded.
        return false
    }

    /// Walks up `node`'s parent chain to find the immediately enclosing
    /// `FunctionDeclSyntax` (stopping at closures so we don't cross
    /// boundaries) and returns true if it's marked `@discardableResult`.
...
extension Finding.Message {
    fileprivate static let discardedObserver: Finding.Message =
