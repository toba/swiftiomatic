...
final class RequireTestFnPrefixOrAttribute: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        var result = node
        result.memberBlock = rewriteMembers(
            result.memberBlock,
            framework: framework,
            state: state,
            context: context
        ...
        for member in memberBlock.members {
            ...
            switch framework {
                case .swiftTesting:
                    ...
                    Self.diagnose(.addTestAttribute, on: funcDecl.funcKeyword, context: context)
                case .xcTest:
                    modifiedFunc = addTestPrefix(to: modifiedFunc)
                    Self.diagnose(
                        .addTestPrefix(name: funcDecl.name.text),
                        on: funcDecl.name,
                        context: context
                    )
            }
    ...
    private static func shouldAddTestAnnotation(
        ...
        if framework == .xcTest {
            ...
            if (state.identifierCounts[name] ?? 0) > 1 { return false }
        }
    ...
    private static func addTestAttribute(to funcDecl: FunctionDeclSyntax) -> FunctionDeclSyntax {
        var result = funcDecl
    ...
    private static func addTestPrefix(to funcDecl: FunctionDeclSyntax) -> FunctionDeclSyntax {
        let name = funcDecl.name.text
