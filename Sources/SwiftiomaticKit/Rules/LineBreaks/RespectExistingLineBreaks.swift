...
extension TokenStream {
    ...
    /// example, an open curly brace on a non-continuation line should always be kept on the same
    /// line as the tokens before it unless the tokens before it are exactly the length of the line
    /// and a break must be inserted there to prevent the brace from going over the limit.
    func isDiscretionaryNewlineAllowed(before token: TokenSyntax) -> Bool {
        ...
        // First, check the pretty printer tokens that will be added before the text token. If we
        // find a break or newline before we find some other text, we allow a discretionary newline.
        // If we find some other content, we don't allow it.
        //
        // If there were no before tokens, then we do the same check the token stream created thus
    ...
    func shouldInsertNewline(basedOn semicolon: TokenSyntax?) -> Bool {
        config[RespectExistingLineBreaks.self] ? semicolon == nil : true
    }
}
