...
/// error is raised.
final class FlagAmbiguousTrailingClosure: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    private func diagnoseBadOverloads(_ overloads: [String: [FunctionDeclSyntax]]) {
        for (_, decls) in overloads where decls.count > 1 {
            let decl = decls[0]
            diagnose(
                .flagAmbiguousTrailingClosure(decl.fullDeclName),
                on: decl.name,
    ...
    override func visit(_ decls: MemberBlockSyntax) -> SyntaxVisitorContinueKind {
...
fileprivate extension Finding.Message {
    static func flagAmbiguousTrailingClosure(_ decl: String) -> Finding.Message {
