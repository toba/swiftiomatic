...
/// Listing a `var` by name in a closure capture list (`{ [counter] in ... }`) snapshots its current
/// value at closure-creation time. Subsequent mutations through the original binding are invisible
/// to the closure, which is almost always surprising.
///
/// This is the same rule SwiftLint emits as `capture_variable`: "Non-constant variables should not
/// be listed in a closure's capture list to avoid confusion about closures capturing variables at
/// creation time."
///
/// The rule is purely syntactic. It pre-scans the source file for `var` declarations (excluding
/// `lazy var`, IUOs, attributed property-wrapper bindings, and stored properties of types — those
/// are accessed through `self`). For each closure capture-list entry, it flags the name when it
/// matches a file-level mutable var. Skipped:
...
///
/// Lint: When a closure capture list lists a name that matches a mutable `var` declaration in the
/// same file, a warning is raised.
final class NoMutableInCaptureList: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: ClosureCaptureClauseSyntax) -> SyntaxVisitorContinueKind {
        ...
        for capture in node.items {
            // Skip `[weak x]`, `[unowned x]`, `[unowned(safe) x]`, etc. These are reference-type
            // lifetime captures, not value-snapshot captures.
...
/// Collects names of `var` declarations across a file that are candidates for the capture-list
/// footgun.
///
/// Excludes:
    ...
    override func visit(_ node: VariableDeclSyntax) -> SyntaxVisitorContinueKind {
        ...
        for binding in node.bindings {
            if let annotation = binding.typeAnnotation,
               annotation.type.is(ImplicitlyUnwrappedOptionalTypeSyntax.self) { continue }
