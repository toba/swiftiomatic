...
/// `Task { try ... }` silently swallows thrown errors when the error type is
...
final class UnhandledThrowingTask: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    /// True if `node` is `Task { ... }` or `Task<_, _> { ... }` (or any
    /// specialization where the last generic argument is `_`, meaning the
    ...
    private func isTaskWithImplicitErrorType(_ node: FunctionCallExprSyntax) -> Bool {
        ...
        if let specialized = node.calledExpression.as(GenericSpecializationExprSyntax.self),
    ...
    /// True if the result of the Task is captured: assigned to a binding,
    /// awaited via `.value`/`.result`, or returned.
    private func isConsumed(_ node: FunctionCallExprSyntax) -> Bool {
        ...
        // Implicit return: only statement in a function body that has a
        // return type.
    ...
    /// True if the trailing closure (or only argument) of `node` contains
    /// an unhandled `throw` or `try`.
...
///
/// `try?`/`try!` and complete `do`/`catch` (where the final catch binds via
...
private final class ThrowsVisitor: SyntaxVisitor {
    ...
    override func visit(_ node: DoStmtSyntax) -> SyntaxVisitorContinueKind {
        ...
        // If the final catch binds via a value-binding pattern that isn't a
        // bare identifier (e.g. `catch let x as Foo`), it doesn't catch all.
...
extension Finding.Message {
    fileprivate static let unhandledThrowingTask: Finding.Message =
