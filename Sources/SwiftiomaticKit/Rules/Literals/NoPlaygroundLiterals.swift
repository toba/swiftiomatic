...
final class NoPlaygroundLiterals: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    package override func visit(_ node: MacroExpansionExprSyntax) -> SyntaxVisitorContinueKind {
        switch node.macroName.text {
            case "colorLiteral": diagnosedColorLiteralMacroExpansion(node)
            case "fileLiteral": diagnosedFileLiteralMacroExpansion(node)
            case "imageLiteral": diagnosedImageLiteralMacroExpansion(node)
            default: break
        }
    ...
    private func diagnosedFileLiteralMacroExpansion(_ node: MacroExpansionExprSyntax) {
        guard isLiteralMacroCall(node, matchingLabels: ["resourceName"]) else { return }
    ...
    private func isLiteralMacroCall(
        ...
        guard node.genericArgumentClause == nil,
              node.trailingClosure == nil,
              node.additionalTrailingClosures.isEmpty,
              node.arguments.count == labels.count else { return false }
