...
extension TokenStream {
    ...
    func emitMultilineSegmentTextTokens(breakKind: BreakKind, segment: Substring) {
        ...
        func emitBreak() {
            if !currentBreak.isEmpty {
                // We append this as a syntax, instead of a `.space` , so that it is always included
                // in the output.
                var str = ""
        ...
        for scalar in segment.unicodeScalars {
            // We don't have to worry about newlines occurring in segments. Either a segment will
            // end in a newline character or the newline will be in trivia.
        ...
        // Only one of these will actually do anything based on whether our last char was whitespace
        // or not.
        emitWord()
        emitBreak()
    }
    ...
    func visitStringSegment(_ node: StringSegmentSyntax) -> SyntaxVisitorContinueKind {
        ...
        let stringLiteralParent = node.parent?
        ...
        let isMultiLineString = stringLiteralParent?.openingQuote.tokenKind == .multilineStringQuote
        ...
        let emitSegmentTextTokens =
            // If our configure reflow behavior is never, always use the single line emit segment
            // text tokens.
        ...
        if segmentText.hasSuffix("\n") {
            // If this is a multiline string segment, it will end in a newline. Remove the newline
            // and append the rest of the string, followed by a break if it's not the last line
            // before the closing quotes. (The `StringLiteralExpr` above does the closing break.)
            let remainder = node.content.text.dropLast()

            if !remainder.isEmpty { emitSegmentTextTokens(remainder) }
        ...
        {
            // Segments with trailing backslashes won't end with a literal newline; the backslash
            // and any `#` delimiters for raw strings are considered trivia. To preserve the
            // original text and wrapping, we need to manually render them break into the token
            // stream.
