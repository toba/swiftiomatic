...
/// Remove a hand-written `Equatable` implementation when the compiler-synthesized conformance would
/// be equivalent.
///
/// For structs conforming to `Equatable` (or `Hashable` ), if the `static func ==` compares exactly
/// the same stored instance properties that the compiler would synthesize, the hand-written
...
final class RedundantEquatable: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        guard let removal = findRemovableEquatable(in: visited.memberBlock.members) else {
            return DeclSyntax(visited)
        }
    ...
    private static func findRemovableEquatable(
        ...
        for (index, member) in members.enumerated() {
            ...
            guard let comparedProps = parseComparedProperties(from: funcDecl),
                  comparedProps == storedProps else { return nil }
    ...
    private static func isEquatableOperator(_ funcDecl: FunctionDeclSyntax) -> Bool {
        guard funcDecl.name.tokenKind == .binaryOperator("=="),
              funcDecl.modifiers.contains(anyOf: [.static]) else { return false }
        ...
        guard paramArray[0].type.trimmedDescription == paramArray[1].type.trimmedDescription else {
            return false
        }
    ...
    private static func parseComparedProperties(
        ...
        guard let body = funcDecl.body,
              let onlyItem = body.statements.firstAndOnly else { return nil }
    ...
    private static func parseComparisons(
        ...
        guard let infixExpr = expr.as(InfixOperatorExprSyntax.self),
              let binOp = infixExpr.operator.as(BinaryOperatorExprSyntax.self) else { return false }
        ...
        return binOp.operator.text == "&&"
            ? parseComparisons(infixExpr.leftOperand, into: &props)
...
fileprivate extension Finding.Message {
    static let removeRedundantEquatable: Finding.Message =
