...
///
/// When a `let` binding is followed immediately by a `return` of the same identifier, the binding
/// is unnecessary. The expression can be returned directly.
///
/// For example: `let result = expr; return result` → `return expr` .
///
/// This rule only fires when the variable is a simple `let` with one binding, no type annotation,
/// and the very next statement is `return <same identifier>` .
///
...
final class DropRedundantProperty: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    private static func tryMerge(
        ...
        // Build: `return value` — transfer declaration's leading trivia (may include preceding
        // comments) to the return keyword, and use the original return expression's trivia on the
        // inlined value.
...
fileprivate extension Finding.Message {
    static func removeRedundantProperty(name: String) -> Finding.Message {
