...
///
/// This rule only applies to `var` declarations with an explicit optional type annotation (e.g.
/// `T?` , `Optional<T>` ). It does not apply to `let` declarations, or to `var` declarations inside
/// protocols (where there is no stored property).
///
...
final class DropRedundantNilInit: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        guard node.bindingSpecifier.tokenKind == .keyword(.var) else { return DeclSyntax(node) }
        ...
        if parent?.parent?.is(ProtocolDeclSyntax.self) == true { return DeclSyntax(node) }
        ...
        for (index, binding) in bindings.enumerated() {
            guard let initializer = binding.initializer,
                  ...
                  isOptionalType(binding.typeAnnotation?.type) else { continue }
...
fileprivate extension Finding.Message {
    static let removeRedundantNilInit: Finding.Message = "remove redundant '= nil' initializer"
}
