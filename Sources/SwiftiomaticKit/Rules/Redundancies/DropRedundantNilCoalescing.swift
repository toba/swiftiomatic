...
final class DropRedundantNilCoalescing: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        // Without type info we can't tell whether the LHS is a single or double optional. `?? nil`
        // is a no-op for `T?` but flattens `T??` to `T?` — and stripping it in the latter case
        // breaks compilation. Conservatively skip any LHS that involves a function call, subscript,
        // or `try?`, where the result type plausibly comes from a signature we can't see.
        if lhsMayProduceDoubleOptional(node.leftOperand) { return ExprSyntax(node) }
        ...
        // computing a `SourceLocation` on a detached node yields the wrong line/column, because the
        // `SourceLocationConverter` is built from the original source bytes and the operator's
        // offset within a detached subtree starts near 0.
...
/// Returns true when the LHS contains a function call, subscript, or `try?`, any of which may yield
/// a doubly-optional value that `?? nil` is flattening.
