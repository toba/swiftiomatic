...
final class NoFallThroughOnlyCases: RewriteSyntaxRule<BasicRuleValue>, @unchecked Sendable {
    ...
    override func visit(_ node: SwitchCaseListSyntax) -> SwitchCaseListSyntax {
        ...
        func flushViolations() {
            for node in fallThroughOnlyCases { newChildren.append(.switchCase(visit(node))) }
        ...
        for element in node {
            ...
            if isMergeableFallThroughOnly(switchCase) {
                ...
                if canMergeWithPreviousCases(switchCase) {
                    ...
                    newChildren.append(
                        .switchCase(visit(mergedCases(fallThroughOnlyCases + [newSwitchCase]))))
                } else {
    ...
    /// Returns true if this node or one of its descendants is a `ValueBindingPatternSyntax`.
    private func containsValueBindingPattern(_ node: SwitchCaseSyntax.Label) -> Bool {
        switch node {
            case let .case(label):
                containsValueBindingPattern(Syntax(label))
            case .default:
                false
        }
    }
    ...
    /// Returns whether the given `SwitchCaseSyntax` contains only a fall-through statement and is
    ...
    private func isMergeableFallThroughOnly(_ switchCase: SwitchCaseSyntax) -> Bool {
        ...
        // When there are any additional or non-fall-through statements, it isn't only a fall-through.
        ...
        // Check for any comments that are adjacent to the case or fall-through statement.
        ...
        // Check for any comments that are inline on the fall-through statement.
    ...
    private func mergedCases(_ cases: [SwitchCaseSyntax]) -> SwitchCaseSyntax {
        ...
        lastCase
            .leadingTrivia = cases.first!.leadingTrivia.withoutLastLine() + lastCase.leadingTrivia
        return lastCase
    }
}
