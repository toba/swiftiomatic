...
/// unnecessary. Removing it simplifies the API and removes the requirement for callers to use
/// `await` .
///
/// Some functions are intentionally async for protocol conformance or future-proofing even if they
/// don't currently await.
///
/// Lint: If an `async` function has no `await` in its body, a lint warning is raised.
///
/// Rewrite: The `async` specifier is removed.
final class RedundantAsync: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    private static func containsAwait(_ node: some SyntaxProtocol) -> Bool {
        for child in node.children(viewMode: .sourceAccurate) {
            ...
            if child.is(AwaitExprSyntax.self) { return true }
...
fileprivate extension Finding.Message {
    static let removeRedundantAsync: Finding.Message =
