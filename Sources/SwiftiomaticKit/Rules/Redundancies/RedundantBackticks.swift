...
final class RedundantBackticks: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    override class var group: ConfigurationGroup? { .redundancies }

    /// Swift reserved keywords that always require backticks when used as identifiers (unless in a
    /// special context like after `.` or as argument labels).
    ...
    /// Keywords that need backticks even as argument labels (expression keywords + binding
    /// specifiers).
    ...
    /// Contextual keywords that need backticks in declaration name position (variable binding,
    /// function name, type name) because the parser would interpret them as declaration
    /// introducers.
...
fileprivate extension Finding.Message {
    static func removeRedundantBackticks(name: String) -> Finding.Message {
