...
///
/// At statement level, `let _ = expr` can be simplified to `_ = expr` since the `let` keyword is
/// unnecessary when the result is discarded.
///
/// In case patterns, `if case .foo(let _)` can be simplified to `if case .foo(_)` since the `let`
/// binding of a wildcard is redundant.
///
...
final class RedundantLet: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    private static func isRedundantLetDecl(_ node: VariableDeclSyntax) -> Bool {
        ...
        // Not `async let` .
        guard !node.modifiers.contains(.async) else { return false }
    ...
    private static func isLikelyResultBuilderContext(parent: Syntax?) -> Bool {
        ...
        while let p = current {
            ...
            if p.is(FunctionDeclSyntax.self) { return false }
            current = p.parent
        }
        return false
    }
    ...
    private static func isResultBuilderClosure(_ closure: ClosureExprSyntax) -> Bool {
        ...
        if parent.is(MultipleTrailingClosureElementSyntax.self) { return true }
        ...
        return parent.is(MacroExpansionExprSyntax.self) || parent.is(MacroExpansionDeclSyntax.self)
            ? true
            : false
    }
    ...
    private static func isResultBuilderAccessor(_ accessor: AccessorDeclSyntax) -> Bool {
        guard let accessorBlock = accessor.parent?.as(AccessorBlockSyntax.self),
              let property = accessorBlock.parent?.as(VariableDeclSyntax.self) else { return false }
    ...
    static func transform(
        ...
        // In case patterns like `case .foo(let _)` , the AST has: LabeledExprSyntax →
        // PatternExprSyntax → ValueBindingPatternSyntax(let, WildcardPatternSyntax) The `let` /
        // `var` is redundant when the inner pattern is just `_` .
        //
...
fileprivate extension Finding.Message {
    static let removeRedundantLet: Finding.Message =
        "remove 'let' from 'let _ = ...'; use '_ = ...' instead"

    static let removeRedundantLetInCasePattern: Finding.Message =
