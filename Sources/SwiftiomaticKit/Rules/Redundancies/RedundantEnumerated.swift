...
///
/// The rule only rewrites when the call is exactly `seq.enumerated()` with no further chaining, no
/// arguments, and no trailing closure. Closure-based usages ( `seq.enumerated().map { ... }` ) are
/// not handled because $0/$1 reference analysis is intricate; lint a separate rule when that case
/// becomes important.
///
...
final class RedundantEnumerated: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        guard firstUnused != secondUnused else { return StmtSyntax(node) }
        ...
        if firstUnused {
            ...
            result.pattern = PatternSyntax(
                second.pattern.with(\.leadingTrivia, tuple.leadingTrivia)
                    .with(\.trailingTrivia, tuple.trailingTrivia))
            result.sequence = ExprSyntax(
                base.with(\.leadingTrivia, call.leadingTrivia)
                    .with(\.trailingTrivia, call.trailingTrivia))
        } else {
            // `for (i, _) in seq.enumerated()` → `for i in seq.indices`
            Self.diagnose(.useIndicesItemUnused, on: second.pattern, context: context)
            result.pattern = PatternSyntax(
                first.pattern.with(\.leadingTrivia, tuple.leadingTrivia)
                    .with(\.trailingTrivia, tuple.trailingTrivia))
...
fileprivate extension Finding.Message {
    static let dropEnumeratedIndexUnused: Finding.Message =
        "drop '.enumerated()'; the index is unused"

    static let useIndicesItemUnused: Finding.Message =
