...
///
/// A type annotation is redundant when it exactly matches what the compiler would infer, such as
/// `let x: Foo = Foo(...)` or `let x: Bool = true` .
///
/// This rule fires for:
...
final class RedundantType: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    ...
    static func transform(
        ...
        for (index, binding) in bindings.enumerated() {
            ...
            guard isRedundant(typeName: typeName, initializer: initializer.value) else { continue }

            Self.diagnose(
                .removeRedundantType(typeName: typeName), on: typeAnnotation, context: context)

            var newBinding = binding
            newBinding.typeAnnotation = nil

            // Transfer any comments from the type annotation's trailing trivia to the `=` token's
            // leading trivia. Without this, `var x: T /* c */ = val` would lose the comment when
    ...
    private static func isRedundant(typeName: String, initializer: ExprSyntax) -> Bool {
        ...
        if initializer.is(BooleanLiteralExprSyntax.self) { return typeName == "Bool" }

        // `let x: String = "..."`
        if initializer.is(StringLiteralExprSyntax.self) { return typeName == "String" }
        ...
        for caseItem in switchExpr.cases {
            switch caseItem {
                case let .switchCase(switchCase):
                    guard allStatementsMatch(typeName: typeName, statements: switchCase.statements)
                    else { return false }
    ...
    private static func allStatementsMatch(
        typeName: String,
        statements: CodeBlockItemListSyntax
    ) -> Bool {
        guard let lastItem = statements.last else { return false }

        // Extract the expression from the code block item. In statement position, if/switch may not
        // be directly accessible via .as(ExprSyntax.self), so check specific types too.
    ...
    private static func simpleTypeName(from expr: ExprSyntax) -> String? {
        ...
        if let array = expr.as(ArrayExprSyntax.self) { return array.trimmedDescription }
...
fileprivate extension Finding.Message {
    static func removeRedundantType(typeName: String) -> Finding.Message {
