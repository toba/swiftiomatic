...
/// Lint nested `<receiver>.withLock { ... <receiver>.withLock { ... } ... }` on the same receiver.
/// Re-entering a non-recursive lock (e.g. `Mutex` ) is a guaranteed deadlock.
final class NoNestedWithLock: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: FunctionCallExprSyntax) -> SyntaxVisitorContinueKind {
        guard let outerReceiver = Self.withLockReceiver(of: node),
              let outerClosure = Self.closureArgument(of: node) else { return .visitChildren }

        let collector = NestedWithLockCollector(
            receiver: outerReceiver.trimmedDescription, viewMode: .sourceAccurate)
        collector.walk(outerClosure.statements)
        for inner in collector.matches { diagnose(.nestedWithLock, on: inner) }
...
fileprivate extension Finding.Message {
    static let nestedWithLock: Finding.Message =
