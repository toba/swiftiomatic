...
/// Lint mutation of the loop subject inside a `for-in` loop. Mutating the iterated collection while
/// the iterator is live is undefined behavior in Swift's `Sequence` model and a frequent source of
/// crashes for `Array` .
///
/// Detection: the loop subject must be either a `DeclReferenceExprSyntax` (e.g. `for x in items` )
/// or a `MemberAccessExprSyntax` (e.g. `for x in self.items` ). Inside the body, any call of the
/// form `<subject>.<mutator>(...)` where `<mutator>` is a known Array/Dictionary/ Set mutating
/// method is flagged.
final class NoMutationDuringIteration: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
...
fileprivate extension Finding.Message {
    static func mutatingIteratedCollection(_ method: String) -> Finding.Message {
