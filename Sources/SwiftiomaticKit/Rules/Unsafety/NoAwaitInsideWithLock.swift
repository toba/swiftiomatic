...
/// Lint `withLock { ... await ... }` — holding a lock across suspension is a blocking/deadlock
/// hazard.
///
/// The rule fires on any function call whose callee is `<receiver>.withLock` when the trailing
/// closure (or first closure argument) contains an `await` expression at any nesting level
/// *outside* of nested closures (a nested `Task { }` body is a separate isolation context and
...
final class NoAwaitInsideWithLock: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: FunctionCallExprSyntax) -> SyntaxVisitorContinueKind {
        ...
        if let firstAwait = collector.firstAwait { diagnose(.awaitInsideWithLock, on: firstAwait) }
...
private final class AwaitCollector: SyntaxVisitor {
    ...
    // Don't descend into nested closures — those run in a separate isolation context and don't hold
    // the outer lock.
    override func visit(_: ClosureExprSyntax) -> SyntaxVisitorContinueKind { .skipChildren }
}

fileprivate extension Finding.Message {
    static let awaitInsideWithLock: Finding.Message =
