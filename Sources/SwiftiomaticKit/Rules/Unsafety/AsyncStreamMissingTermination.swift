...
/// Lint `AsyncStream { continuation in ... }` (and `AsyncThrowingStream`)
/// initializer bodies that call `continuation.yield(...)` but never call
/// `continuation.finish(...)` and never set `continuation.onTermination`.
///
/// Without termination handling the stream can leak when the consumer is
/// cancelled — the producer keeps holding resources for a continuation that
...
final class AsyncStreamMissingTermination: LintSyntaxRule<LintOnlyValue>, @unchecked Sendable {
    ...
    override func visit(_ node: FunctionCallExprSyntax) -> SyntaxVisitorContinueKind {
        ...
        if let trailing = node.trailingClosure {
            closure = trailing
        } else if let firstClosure = node.arguments.lazy.compactMap({ $0.expression.as(ClosureExprSyntax.self) }).first {
...
extension Finding.Message {
    fileprivate static let missingTermination: Finding.Message =
