...
/// Wrap each branch of a ternary expression onto its own line when the expression would exceed the
/// configured line length.
///
/// The pretty printer no longer makes wrapping decisions for ternaries — instead, this rule inserts
/// discretionary newlines into the leading trivia of `?` and `:` whenever the ternary's last column
/// would exceed `LineLength`. The pretty printer respects those newlines (see
/// `RespectExistingLineBreaks`) and applies a continuation indent to each wrapped branch,
/// producing:
///
/// ```swift
...
/// ```
///
/// If either operator already has a leading newline, the rule normalizes the other to match so the
/// ternary always has both branches on their own lines once it wraps.
final class WrapTernaryBranches: StaticFormatRule<BasicRuleValue>, @unchecked Sendable {
    static let rewriteOrder = 1220
    ...
    static func transform(
        ...
        // Inserting newlines around `?`/`:` inside a single-line string interpolation produces
        // invalid Swift (newlines aren't allowed inside `"\(...)"`).
        ...
        if questionHasNewline || colonHasNewline {
            ...
            // the ternary's operators are already on lines that don't overflow — no need to force a
            // wrap, even if the *collapsed-to-one-line* description of the ternary is long. This
            // matters when an operand is consumed by a lower-precedence operator that wraps
        ...
        if !questionHasNewline {
            ...
            result.questionMark.leadingTrivia = .newline
                + dropLeadingSpaces(result.questionMark.leadingTrivia)
        }
        if !colonHasNewline {
    ...
    /// Strips a single leading run of horizontal whitespace from a trivia sequence so we don't end
    /// up with a redundant space after the inserted newline. The pretty printer recomputes
    ...
    private static func dropLeadingSpaces(_ trivia: Trivia) -> Trivia {
        ...
        switch first {
            case .spaces, .tabs: return Trivia(pieces: Array(trivia.dropFirst()))
    ...
    /// True if `node` is contained within another `TernaryExprSyntax`. Walks the captured
    /// pre-recursion parent chain since the post-recursion node is detached.
...
fileprivate extension Finding.Message {
    static let wrapTernaryBranch: Finding.Message = "wrap ternary branch onto a new line"
}
