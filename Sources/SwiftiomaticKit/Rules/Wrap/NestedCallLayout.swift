...
final class NestedCallLayout: StaticFormatRule<NestedCallLayoutConfiguration>, @unchecked Sendable {
    ...
    /// Rewrites a call to the configured layout.
    ///
    /// Every position read goes through `original` , never through `node` . The rewriter hands this
    /// method the result of visiting the children, so `node` is a fresh detached tree whenever any
    /// child changed. A detached node has no preceding token, so a column read on it returns 0, an
    /// indentation read returns the empty string, and a parent read returns nil. The rule then
    /// believes the call starts at the left margin and collapses it far past the line length.
    ///
    /// - Parameters:
    ///   - node: the call with its children already rewritten
    ///   - original: the same call as it sits in the source file
    static func transform(
        ...
        guard !isInnerNestedCall(original) else { return ExprSyntax(node) }
        ...
        if calleeSpansMultipleLines(node) {
            if mode == .inline,
               let inlined = tryInlineModifierCallArgument(
                   node, original: original, context: context
               ) { return inlined }
        ...
        if let chain = collectChain(node), chain.count >= 2 {
            switch mode {
                case .inline:
                    ...
                    // internal whitespace. That only works when the input is in canonical
                    // fully-nested form. For non-canonical inputs (e.g., extra indent), hug
                    // instead.
                    if isCanonicalFullyNested(node, original: original) {
                        if let result = inlineLayout(
                            node, original: original, chain: chain, context: context
                        ) { return result }
                        return ExprSyntax(node)
                    }
                case .wrap:
                    return wrapLayout(node, original: original, chain: chain, context: context)
            }
        }

        // Fallback A: collapse a wrapped multi-arg single-level call onto one line if it fits. This
        // also covers wrapped single-arg calls whose inlined form fits the line.
        if mode == .inline,
           let collapsed = tryCollapseCallToOneLine(node, original: original, context: context) {
            return collapsed
        }

        // Fallback B: collapse the leading newline so the sole arg hugs the opening paren,
        // re-indenting any continuation lines.
        if mode == .inline, let hugged = tryHugSingleArg(node, original: original, context: context)
        { return hugged }
    ...
    static func transform(
        ...
        original: MacroExpansionExprSyntax,
        parent _: Syntax?,
        context: Context
    ) -> ExprSyntax {
        ...
        return tryCollapseMacroExpansionToOneLine(node, original: original, context: context)
            ?? ExprSyntax(node)
    }
    ...
    ///
    /// - Parameters:
    ///   - original: the same call as it sits in the source file, which is what carries the
    ///     indentation
    private static func isCanonicalFullyNested(
        _ node: FunctionCallExprSyntax,
        original: FunctionCallExprSyntax
    ) -> Bool {
    ...
    ///
    /// The call must still be attached to the tree, because the answer comes from its parent.
    ...
    /// - Parameters:
    ///   - original: the same call as it sits in the source file, which is what carries the column
    ///     and the indentation
    private static func inlineLayout(
        ...
        if fullyInlineLength <= maxLength {
            Self.diagnose(.collapseNestedCall, on: original, context: context)
    ...
    private static func rebuildFullyInline(
        ...
        for level in chain.dropLast().reversed() {
            ...
            result = ExprSyntax(FunctionCallExprSyntax(
                calledExpression: level.call.calledExpression.trimmed, leftParen: .leftParenToken(),
                arguments: [arg], rightParen: .rightParenToken()))
        }
    ...
    private static func rebuildSingleCallInline(
        ...
        return ExprSyntax(FunctionCallExprSyntax(
            calledExpression: call.calledExpression.trimmed, leftParen: .leftParenToken(),
            arguments: LabeledExprListSyntax(args), rightParen: .rightParenToken()))
    }
    ...
    private static func rebuildFullyWrappedInnerInline(
        ...
        for level in chain.dropFirst().dropLast().reversed() {
            ...
            innerExpr = ExprSyntax(FunctionCallExprSyntax(
                calledExpression: level.call.calledExpression.trimmed, leftParen: .leftParenToken(),
                arguments: [arg], rightParen: .rightParenToken()))
        }
        ...
        if let label = outerOriginal.label {
            ...
            arg = LabeledExprSyntax(expression:
                    innerExpr
                    .with(\.leadingTrivia, .newline + Trivia(stringLiteral: innerIndent)))
        }
        let result = ExprSyntax(FunctionCallExprSyntax(
            calledExpression: outermost.calledExpression.trimmed, leftParen: .leftParenToken(),
            arguments: [arg],
    ...
    ///
    /// - Parameters:
    ///   - original: the same call as it sits in the source file, whose period token carries the
    ///     real rendered column
    private static func tryInlineModifierCallArgument(
        ...
        guard let memberAccess = node.calledExpression.as(MemberAccessExprSyntax.self),
            let originalMember = original.calledExpression.as(MemberAccessExprSyntax.self),
            node.leftParen != nil,
        ...
        let prefix = columnOffset(of: originalMember.period)
        let maxLength = context.configuration[LineLength.self]
        guard prefix + segment.count <= maxLength else { return nil }

        Self.diagnose(.collapseNestedCall, on: originalMember.declName, context: context)
    ...
    /// - Parameters:
    ///   - original: the same call as it sits in the source file, which is what carries the
    ///     indentation that the rebuilt lines anchor on
    private static func tryHugSingleArg(
        ...
        newRightParen = rightParen.leadingTrivia.containsNewlines
            ? rightParen.with(\.leadingTrivia, .newline + Trivia(stringLiteral: baseIndent))
            : rightParen
    ...
    private static func wrapLayout(
        ...
        Self.diagnose(.expandNestedCall, on: original, context: context)
    ...
    /// - Parameters:
    ///   - original: the same expansion as it sits in the source file, which is what carries the
    ///     column
    ...
    private static func argumentsHaveComments(
        ...
        return rightParen?.leadingTrivia.hasAnyComments == true
            ? true
            : false
    }
    ...
    ///
    /// The node must still be attached to the tree. The walk runs backwards over the preceding
    /// tokens, and a detached node has none, so a detached node always reports 0.
    private static func columnOffset(of node: some SyntaxProtocol) -> Int {
        ...
        // Accumulate the source text on the current line that precedes the node, walking backwards
        // and stopping at the most recent newline.
        var lineFragment = ""
        ...
        while let t = token {
            ...
            if prependStoppingAtNewline(t.leadingTrivia.description) { return lineFragment.count }
    ...
    ///
    /// The node must still be attached to the tree. The walk runs backwards over the preceding
    /// tokens, and a detached node has none, so a detached node always reports the empty string.
