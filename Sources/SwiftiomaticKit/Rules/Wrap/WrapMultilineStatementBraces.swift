...
final class WrapMultilineStatementBraces: RewriteSyntaxRule<BasicRuleValue>, @unchecked Sendable {
    ...
    override func visit(_ node: ForStmtSyntax) -> StmtSyntax {
        ...
        var result = forNode
        result.body.leftBrace = newBrace
        // Strip trailing whitespace from the token before `{` For a for loop, this is typically the
        // sequence expression or where clause
    ...
    override func visit(_ node: FunctionDeclSyntax) -> DeclSyntax {
        ...
        guard let funcNode = visited.as(FunctionDeclSyntax.self),
              let body = funcNode.body else { return visited }
        guard let newBrace = wrappedBrace(
    ...
    override func visit(_ node: InitializerDeclSyntax) -> DeclSyntax {
        ...
        guard let initNode = visited.as(InitializerDeclSyntax.self),
              let body = initNode.body else { return visited }
        guard let newBrace = wrappedBrace(
            leftBrace: body.leftBrace,
            rightBrace: body.rightBrace
        ) else { return visited }
    ...
    override func visit(_ node: DeinitializerDeclSyntax) -> DeclSyntax {
        ...
        guard let deinitNode = visited.as(DeinitializerDeclSyntax.self),
              let body = deinitNode.body else { return visited }
        guard let newBrace = wrappedBrace(
            leftBrace: body.leftBrace,
            rightBrace: body.rightBrace
        ) else { return visited }
    ...
    private func stripTrailingOnLastSigToken<D: DeclSyntaxProtocol>(_ decl: inout D) {
        // The leftBrace is now on its own line. The preceding token may have trailing whitespace
        // from the original `... {` layout. We find the token before the member block left brace
        // via the tree.
        ...
        // Walk the decl children to find which property contains this token and strip its trailing
        // whitespace
    ...
    static func transform(
        ...
        _ = parent; return WrapMultilineStatementBraces(context: context).visit(node)
    }
    static func transform(
