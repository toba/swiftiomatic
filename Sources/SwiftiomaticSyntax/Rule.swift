...
extension Rule {
  package static var rationale: String? { nil }
  package static var scope: Scope { .lint }
  public static var isCorrectable: Bool { false }
  package static var isOptIn: Bool { false }
  package static var isDeprecated: Bool { false }
  package static var deprecationMessage: String? { nil }
  package static var requiresSourceKit: Bool { false }
  ...
  package static var minSwiftVersion: SwiftVersion { .v6 }
  package static var requiresFileOnDisk: Bool { false }
  package static var nonTriggeringExamples: [Example] { [] }
  package static var triggeringExamples: [Example] { [] }
  package static var corrections: [Example: Example] { [:] }

  /// All identifiers (current + deprecated) this rule responds to
  package static var allIdentifiers: [String] {
    Array(deprecatedAliases) + [id]
  }

  /// Structured examples for this rule
  package static var examples: RuleExamples {
    RuleExamples(
...
extension Rule {
  package var shouldLintEmptyFiles: Bool {
    false
  }

  package init(configuration: Any) throws {
    self.init()
  ...
  package func validate(file: SwiftSource, using _: RuleStorage, compilerArguments: [String])
    -> [RuleViolation]
  {
    validate(file: file, compilerArguments: compilerArguments)
  }

  package func validate(file _: SwiftSource) -> [RuleViolation] {
    []
  }

  package func validate(file: SwiftSource, compilerArguments _: [String]) -> [RuleViolation] {
    validate(file: file)
  }

  package func isEqualTo(_ rule: some Rule) -> Bool {
  ...
  package func collectInfo(for _: SwiftSource, into _: RuleStorage, compilerArguments _: [String]) {
    // no-op: only CollectingRules mutate their storage
  }

  package func correct(file: SwiftSource, compilerArguments _: [String]) -> Int {
    correct(file: file)
  }

  package func correct(file _: SwiftSource) -> Int {
    0
  }

  package func correct(file: SwiftSource, using _: RuleStorage, compilerArguments: [String]) -> Int {
  ...
  package func createConfigurationDescription(exclusiveOptions: Set<String> = [])
    -> RuleOptionsDescription
  {
  ...
  package func canBeDisabled(violation: RuleViolation, by ruleID: RuleIdentifier) -> Bool {
    switch ruleID {
    case .all:
      true
...
extension Rule {
  ...
  package static var identifier: String { id }

  /// Human-readable display name (alias for ``name``)
  package static var ruleName: String { name }

  /// Brief description (alias for ``summary``)
  package static var ruleSummary: String { summary }

  /// Detailed rationale in Markdown (alias for ``rationale``)
  package static var ruleRationale: String? { rationale }

  /// Previous identifiers (alias for ``deprecatedAliases``)
  package static var ruleDeprecatedAliases: Set<String> { deprecatedAliases }

  /// Whether this rule requires compiler arguments (alias for ``requiresCompilerArguments``)
  package static var runsWithCompilerArguments: Bool { requiresCompilerArguments }
  ...
  package static var ruleMinSwiftVersion: SwiftVersion { minSwiftVersion }

  /// Non-triggering examples (alias for ``nonTriggeringExamples``)
  package static var ruleNonTriggeringExamples: [Example] { nonTriggeringExamples }

  /// Triggering examples (alias for ``triggeringExamples``)
  package static var ruleTriggeringExamples: [Example] { triggeringExamples }

  /// Correction pairs (alias for ``corrections``)
  package static var ruleCorrections: [Example: Example] { corrections }
}
...
extension Rule {
  ...
  package var requiresSourceKit: Bool {
...
package protocol FormatAwareRule: Rule {
...
package protocol AsyncEnrichableRule: Rule {
...
extension AsyncEnrichableRule {
  package static var canEnrichAsync: Bool { true }
}
