...
public struct Example: Sendable {
  ...
  package private(set) var code: String
  ...
  package private(set) var configuration: [String: any Sendable]?
  /// Whether the example should be tested by prepending multibyte grapheme clusters
  ///
  /// - SeeAlso: addEmoji(_:)
  package private(set) var shouldTestMultiByteOffsets: Bool
  ...
  package private(set) var file: StaticString
...
extension Example {
  ...
  package func with(code: String) -> Example {
    var new = self
    new.code = code
    return new
  }
  ...
  package func removingViolationMarkers() -> Example {
