...
/// aggregated results.
package protocol CollectingRule: Rule {
...
extension CollectingRule {
  ...
  package func validate(file: SwiftSource, using storage: RuleStorage, compilerArguments: [String])
    -> [RuleViolation]
  {
  ...
  package func collectInfo(for _: SwiftSource) -> FileInfo {
    Console.fatalError(
      "Must call `collectInfo(for:compilerArguments:)` for CollectingRule requiring compiler arguments",
    )
  }

  package func collectInfo(for file: SwiftSource, compilerArguments _: [String]) -> FileInfo {
    collectInfo(for: file)
  }

  package func validate(file _: SwiftSource, collectedInfo _: [SwiftSource: FileInfo]) -> [RuleViolation] {
    Console.fatalError(
  ...
  package func validate(file _: SwiftSource, compilerArguments _: [String]) -> [RuleViolation] {
    Console.fatalError(
