...
public struct Region: Equatable, Sendable {
  ...
  package let end: Location
  /// All rule identifiers that are disabled in this region.
  package let disabledRuleIdentifiers: Set<RuleIdentifier>

  package init(start: Location, end: Location, disabledRuleIdentifiers: Set<RuleIdentifier>) {
    self.start = start
    self.end = end
  ...
  package func contains(_ location: Location) -> Bool {
  ...
  package func isRuleEnabled(_ rule: some Rule) -> Bool {
    !isRuleDisabled(rule)
  }
  ...
  package func isRuleDisabled(_ rule: some Rule) -> Bool {
  ...
  package func areRulesDisabled(ruleIDs: [String]) -> Bool {
  ...
  package func deprecatedAliasesDisabling(rule: some Rule) -> Set<String> {
  ...
  package func toSourceRange(locationConverter: SourceLocationConverter) -> SourceRange? {
