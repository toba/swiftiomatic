...
extension String {
  package var hasTrailingWhitespace: Bool {
  ...
  package var isUppercase: Bool { self == uppercased() }

  package var isLowercase: Bool { self == lowercased() }
  ...
  package func substring(from: Int, length: Int? = nil) -> String {
    if let length {
  ...
  package func lastIndex(of search: String) -> Int? {
  ...
  package func nsRangeToIndexRange(_ nsrange: NSRange) -> Range<Index>? {
  ...
  package var fullNSRange: NSRange {
  ...
  package func absolutePathStandardized() -> String {
    let standardized = URL(fileURLWithPath: self).standardizedFileURL.path
    return URL(fileURLWithPath: standardized.absolutePathRepresentation()).path
  }
  ...
  package func countOccurrences(of character: Character) -> Int {
  ...
  package func path(relativeTo rootDirectory: String) -> String {
  ...
  package func deletingPrefix(_ prefix: String) -> String {
  ...
  package func indent(by spaces: Int, skipFirst: Bool = false, skipEmptyLines: Bool = true) -> String {
  ...
  package func linesPrefixed(with prefix: Self) -> Self {
    split(separator: "\n").joined(separator: "\n\(prefix)")
  }
  ...
  package func characterPosition(of utf8Offset: Int) -> Int? {
