...
struct ChildOptionSeverityConfiguration<Parent: Rule>: RuleConfiguration,
    ...
    static var error: Self { Self(optionSeverity: .error) }
    /// Configuration with an error severity.
    static var warning: Self { Self(optionSeverity: .warning) }
    /// Configuration disabling an option.
    static var off: Self { Self(optionSeverity: .off) }
    ...
    func asOption() -> OptionType { .symbol(optionSeverity.rawValue) }
}
