...
extension SeverityLevelsConfiguration: SeverityLevelsBasedRuleConfiguration {
    var severityConfiguration: SeverityLevelsConfiguration<Parent> { self }
}

/// Violation visitor customized to collect violations of code blocks that exceed a specified number of lines.
class BodyLengthVisitor<LevelConfig: SeverityLevelsBasedRuleConfiguration>: ViolationsSyntaxVisitor<LevelConfig> {
    @inlinable
    ...
    func registerViolations(leftBrace: TokenSyntax,
