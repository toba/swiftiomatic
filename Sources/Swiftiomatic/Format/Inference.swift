...
private struct Inference {
    ...
    let closingParenPosition = OptionInferrer { formatter, options in
        var functionCallSameLine = 0
        var functionCallBalanced = 0
        var functionDeclarationSameLine = 0
        var functionDeclarationBalanced = 0
        ...
        formatter.forEach(.startOfScope("(")) { i, _ in
            ...
            let isClosingParenOnSameLine =
                (formatter.last(.nonSpaceOrComment, before: closingBraceIndex)?.isLinebreak != true)
        ...
        if functionCallSameLine > functionCallBalanced,
           functionDeclarationBalanced > functionDeclarationSameLine
        {
        ...
        if options.callSiteClosingParenPosition == .sameLine {
            options.closingParenPosition =
                functionDeclarationSameLine > functionDeclarationBalanced ? .sameLine : .balanced
        } else {
    ...
    let stripUnusedArguments = OptionInferrer { formatter, options in
        var functionArgsRemoved = 0
        var functionArgsKept = 0
        var unnamedFunctionArgsRemoved = 0
        var unnamedFunctionArgsKept = 0
        ...
        formatter.forEachToken { i, token in
            ...
            while index < endIndex {
                guard
                    let externalNameIndex = formatter.index(
                ...
                guard
                    let nextIndex =
                    formatter.index(of: .nonSpaceOrCommentOrLinebreak, after: externalNameIndex)
                else { return }
            ...
            guard !argNames.isEmpty,
                  let bodyStartIndex = formatter.index(
                      after: endIndex,
                      where: {
                          switch $0 {
    ...
    let explicitSelf = OptionInferrer { formatter, options in
        ...
        func processAccessors(
            ...
            var localNames = localNames
            while let nextIndex = formatter.index(
                of: .nonSpaceOrCommentOrLinebreak, after: index,
                if: {
                ...
                foundAccessors = true
                index = startIndex + 1
                if let parenStart = formatter.index(
...
private extension Formatter {
    ...
    func grouping(for numberType: NumberType) -> Grouping {
        var grouping = [(group: Int, threshold: Int, count: Int)]()
        var lowest = Int.max
...
extension Inference {
    ...
    nonisolated(unsafe) static let byName: [String: OptionInferrer] = {
