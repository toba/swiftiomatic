...
package final class Formatter {
    ...
    init(
        _ tokens: [Token], options: FormatOptions = FormatOptions(),
        ...
        for (i, token) in tokens.enumerated() {
            switch token {
                ...
                case let .commentBody(comment):
                    ...
                    switch directive {
                        case "options":
                            // Inline options directives are not supported in this build
                            continue
                        case "disable":
...
extension Formatter {
    ...
    /// Replaces the tokens in the specified range with new tokens
    @discardableResult
    private func replaceTokens(in range: Range<Int>, with tokens: ArraySlice<Token>, isMove: Bool)
        -> Int
    {
        let max = min(range.count, tokens.count)
        for i in 0 ..< max {
            replaceToken(
                at: range.lowerBound + i,
                with: tokens[tokens.startIndex + i],
                isMove: isMove,
            )
        }
        if range.count > max {
            ...
            switch step {
                case let .insert(lineIndex, line, movedFromLineIndex):
                    ...
                    if lineIndex >= lineRanges.count {
                        ...
                        insert(
                            line,
                            at: lineRanges[lineIndex].lowerBound,
                            isMove: movedFromLineIndex != nil,
                        )
                    }
    ...
    func index(
        of type: TokenType, in range: CountableRange<Int>,
        if matches: (Token) -> Bool = { _ in true },
    ) -> Int? {
    ...
    func nextToken(after index: AnyIndex, where matches: (Token) -> Bool = { _ in
        true
    }) -> Token? {
    ...
    func next(_ type: TokenType, after index: AnyIndex, if matches: (Token) -> Bool = { _ in true })
        -> Token?
    {
    ...
    func next(
        _ type: TokenType, in range: CountableRange<Int>,
        if matches: (Token) -> Bool = { _ in true },
    ) -> Token? {
    ...
    func lastIndex(in range: CountableRange<Int>, where matches: (Token) -> Bool) -> Int? {
        ...
        for i in range.reversed() {
            ...
            switch token {
                case .startOfScope(":")
                where [.endOfScope("#endif"), .endOfScope("}")].contains(scopeStack.last):
                    break
    ...
    func index(
        of type: TokenType, before index: AnyIndex, if matches: (Token) -> Bool = { _ in true },
    ) -> Int? {
    ...
    /// is deallocated.
    func registerAutoUpdatingReference(_ reference: AutoUpdatingReference) {
    ...
    func unregisterAutoUpdatingReference(_ reference: AutoUpdatingReference) {
...
extension String {
    ...
    func ranges(of string: some StringProtocol,
                options: String.CompareOptions = []) -> [Range<Index>]
    {
        var result: [Range<Index>] = []
        var startIndex = startIndex
        while startIndex < endIndex, let range = self[startIndex...].range(
            of: string,
            options: options,
        ) {
            result.append(range)
            startIndex =
                range.lowerBound < range.upperBound
                    ? range
                    .upperBound : index(range.lowerBound, offsetBy: 1, limitedBy: endIndex) ??
                    endIndex
        }
        return result
    }
...
final class AutoUpdatingRange: AutoUpdatingReference, AnyClosedRange, Equatable,
    CustomStringConvertible
{
    var range: ClosedRange<Int>
    private weak let formatter: Formatter?
