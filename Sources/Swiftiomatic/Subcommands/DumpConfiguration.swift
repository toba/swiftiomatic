        ...
        func validate() throws {
            if configurationOptions.configuration != nil, !effective {
                throw ValidationError(
                    "'--configuration' is only valid in combination with '--effective'")
            }
        }

        func run() throws {
            let diagnosticPrinter = StderrDiagnosticPrinter(colorMode: .auto)
            let diagnosticsEngine = DiagnosticsEngine(diagnosticsHandlers: [
                diagnosticPrinter.printDiagnostic
            ])

            var configuration: Configuration

            if effective {
                let configurationProvider = Frontend.ConfigurationProvider(
                    diagnosticsEngine: diagnosticsEngine)

                guard let effectiveConfiguration = configurationProvider.provide(
            ...
            do {
                try print(configuration.asJSONString(sortBy: sort))
            } catch {
