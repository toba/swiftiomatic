...
extension Configuration {
    final class RulesWrapper {
        // MARK: - Properties

        private static let isOptInRuleCache = Mutex([String: Bool]())
        ...
        private func mergedAllRulesWrapped(with child: RulesWrapper) -> [ConfigurationRuleWrapper] {
            ...
            return
                childConfigRulesWithConfig // Include, if rule is configured in child
        ...
        private func mergedCustomRules(
            ...
            configuration.customRuleConfigurations =
                childCustomRulesRule.configuration.customRuleConfigurations
                    ...
                    .filter { parentConfig in
                        !childCustomRulesRule.configuration.customRuleConfigurations
                            .contains { childConfig in
            ...
            return newAllRulesWrapped.filter { !($0.rule is CustomRules) } + [(
                newCustomRulesRule,
                true,
            )]
        }
            ...
            switch mode { // Switch parent's mode. Child is in default mode.
                case var .defaultConfiguration(disabled, optIn):
                    ...
                    return .defaultConfiguration(
                        disabled: Set(childDisabled)
                            .union(Set(disabled.filter { !childOptIn.contains($0) })),
                        optIn: Set(childOptIn)
                            .union(Set(optIn.filter { !childDisabled.contains($0) }))
                            .filter {
                ...
                case .allCommandLine:
                    ...
                    return .defaultConfiguration(
                        ...
                        optIn: Set(
                            newAllRulesWrapped.map { type(of: $0.rule).identifier }
                                .filter {
        ...
        private func isOptInRule(
            ...
            let cachedIsOptInRule = Self.isOptInRuleCache.withLock { $0[identifier] }
            ...
            Self.isOptInRuleCache.withLock { $0[identifier] = isOptInRule }
            return isOptInRule
        }
    }
}
