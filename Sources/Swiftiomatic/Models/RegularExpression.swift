...
// MARK: - RegularExpression (Swift Regex)
...
struct RegularExpression: Hashable, Comparable, ExpressibleByStringLiteral, @unchecked Sendable {
    ...
    private static let regexCache = Mutex([RegexCacheKey: RegularExpression]())
        ...
        -> RegularExpression
    {
        ...
        return try regexCache.withLock { cache in
            ...
            let result = try RegularExpression(pattern: pattern, options: opts)
            cache[key] = result
            return result
        }
    }
