...
private struct BaselineViolation: Codable, Hashable, Comparable {
    ...
    init(violation: RuleViolation, text: String) {
        let location = violation.location
        self.violation = violation.with(
            location: Location(
...
struct Baseline: Equatable {
    ...
    func compare(_ otherBaseline: Self) -> [RuleViolation] {
        otherBaseline.baseline
            .flatMap { relativePath, otherBaselineViolations -> Set<RuleViolation> in
...
private struct LineCache {
    ...
    private mutating func cached(file: String) -> [String]? {
        ...
        if let fileLines = SwiftSource(path: file)?.lines.map(\.content) {
...
private extension Sequence<BaselineViolation> {
    init(_ violations: [RuleViolation]) where Self == BaselineViolations {
        var lineCache = LineCache()
        self = violations.map { $0.baselineViolation(text: lineCache.text(at: $0.location)) }
    }

    var violationsWithAbsolutePaths: [RuleViolation] {
...
private extension RuleViolation {
    var withAbsolutePath: RuleViolation {
