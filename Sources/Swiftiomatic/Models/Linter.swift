...
private extension Rule {
    ...
    private func performLint(
        ...
        let (disabledViolationsAndRegions, enabledViolationsAndRegions) = violations
            .map { violation in
...
struct Linter: @unchecked Sendable {
    ...
    ///
    /// - returns: A linter capable of checking for violations after running each rule's collection step.
    func collect(into storage: RuleStorage) -> CollectedLinter {
...
struct CollectedLinter: @unchecked Sendable {
    ...
    func styleViolations(using storage: RuleStorage) -> [StyleViolation] {
    ...
    private func getStyleViolations(
        ...
        let violations = validationResults
        ...
        return regions.flatMap { region in
            region.disabledRuleIdentifiers.filter {
                ...
                StyleViolation(
                    ...
                    severity: superfluousDisableCommandRule.configuration.severity,
                    location: region.start,
                    reason: superfluousDisableCommandRule
