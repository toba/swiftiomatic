...
struct ShorthandArgumentConfiguration: SeverityBasedRuleConfiguration {
    ...
    mutating func apply(configuration: Any) throws(Issue) {
        if $allowUntilLineAfterOpeningBrace.key.isEmpty {
            $allowUntilLineAfterOpeningBrace.key = "allow_until_line_after_opening_brace"
        }
        if $alwaysDisallowMoreThanOne.key.isEmpty {
            $alwaysDisallowMoreThanOne.key = "always_disallow_more_than_one"
        }
        if $alwaysDisallowMemberAccess.key.isEmpty {
            $alwaysDisallowMemberAccess.key = "always_disallow_member_access"
        }
        do {
