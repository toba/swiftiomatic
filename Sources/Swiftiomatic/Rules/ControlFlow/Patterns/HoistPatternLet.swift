  ...
  static let hoistPatternLet = FormatRule(
    ...
    formatter.forEach(.startOfScope("(")) { i, _ in
      ...
      let startIndex =
        ...
        ?? (prevIndex + 1)
      if case .keyword(let keyword) = formatter.tokens[startIndex],
        ["let", "var"].contains(keyword)
      {
        ...
        while index < endIndex {
          ...
          case .identifier(let name) where wasParenOrCommaOrLabel:
            wasParenOrCommaOrLabel = false
        ...
        // Remove keyword
        let lower = (formatter.index(of: .nonSpace, before: startIndex) ?? (prevIndex - 1)) + 1
        formatter.removeTokens(in: lower...startIndex)
      } else if hoist {
  ...
  func indicesOf(_ keyword: String, in range: CountableRange<Int>) -> [Int]? {
    ...
    for index in range {
      ...
      case .identifier("_"):
        break
      case .identifier
      where last(.nonSpaceOrComment, before: index)
        != .operator(
          ".",
          .prefix,
        ):
