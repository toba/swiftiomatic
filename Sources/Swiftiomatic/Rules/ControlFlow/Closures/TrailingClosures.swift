    ...
    formatter.forEach(.startOfScope("(")) { functionOpenParen, _ in
      guard
        let identifierIndex = formatter.parseFunctionIdentifier(
          beforeStartOfScope: functionOpenParen,
        )
      else { return }
      let name = formatter.tokens[identifierIndex].string

      guard !nonTrailing.contains(name),
        !formatter.isConditionalStatement(at: functionOpenParen),
      ...
      let trailingClosures = arguments.suffix(while: { arg in
        ...
        guard
          let first = formatter.index(
            of: .nonSpaceOrCommentOrLineBreak,
            in: range.lowerBound..<range.upperBound + 1,
          ),
          let last = formatter.index(
            of: .nonSpaceOrCommentOrLineBreak, before: range.upperBound + 1,
            if: { _ in true },
          ),
      ...
      guard let functionCallClosingParen = formatter.endOfScope(at: functionOpenParen),
        formatter.next(.nonSpaceOrCommentOrLineBreak, after: functionCallClosingParen)
          != .startOfScope("{")
      else { return }
      ...
      if trailingClosures.count == 1 {
        ...
        guard
          let closingBraceIndex = formatter.index(
            of: .nonSpaceOrComment, before: functionCallClosingParen,
            if: { $0 == .endOfScope("}") },
          ),
          let openingBraceIndex = formatter.index(
            of: .startOfScope("{"), before: closingBraceIndex,
          ),
          formatter.index(of: .endOfScope("}"), before: openingBraceIndex) == nil,
          var startIndex = formatter.index(
            of: .nonSpaceOrLineBreak,
            before: openingBraceIndex,
          )
        else { return }
        ...
        case .delimiter(","), .startOfScope("("):
          break
        case .delimiter(":"):
          if let commaIndex = formatter.index(
            of: .delimiter(","),
            before: openingBraceIndex,
          ) {
            startIndex = commaIndex
          } else if formatter.index(of: .startOfScope("("), before: openingBraceIndex)
            == functionOpenParen
          {
        ...
        formatter.replaceTokens(
          in: startIndex..<openingBraceIndex,
          with:
        ...
        let lineWithClosingParen =
          formatter.startOfLine(
            at: functionCallClosingParen,
          )...formatter.endOfLine(at: functionCallClosingParen)
        let tokenBeforeClosingParen = formatter.index(
          of: .nonSpaceOrCommentOrLineBreak, before: functionCallClosingParen,
        )

        if formatter.tokens(in: lineWithClosingParen)?.string.trimmingCharacters(
        ...
        if let tokenBeforeClosingParen,
          formatter.tokens[tokenBeforeClosingParen] == .delimiter(",")
        {
        ...
        for (index, closure) in trailingClosures.enumerated().reversed() {
          ...
          let closureOnOneLine = formatter.onSameLine(
            closureRange.lowerBound, closureRange.upperBound,
          )

          if index == trailingClosures.indices.first,
            let indexBeforeFirstClosure = formatter.index(
              of: .nonSpaceOrCommentOrLineBreak, before: closure.valueRange.lowerBound,
            )
          {
            ...
            // Otherwise, the previous comma becomes the new closing paren
              ...
              formatter.replaceToken(
                at: indexBeforeFirstClosure,
                with: .endOfScope(")"),
              )
            }

            // Unwrap the line so the trailing closure label immediately follows the closing paren
            if !formatter.onSameLine(
              indexBeforeFirstClosure,
              closure.valueRange.lowerBound,
            ) {
              formatter.unwrapLine(
                before: closureRange.lowerBound,
                preservingComments: true,
              )
            }
          }

          else if let closureLabelIndex = closure.labelIndex?.autoUpdating(in: formatter),
            let commaIndex = formatter.index(
              of: .nonSpaceOrCommentOrLineBreak, before: closureLabelIndex,
            ),
            formatter.tokens[commaIndex] == .delimiter(",")
          {
            let commaWasOnPreviousLine = !formatter.onSameLine(
              commaIndex,
              closureLabelIndex.index,
            )
            ...
            if commaWasOnPreviousLine {
              formatter.unwrapLine(
                before: closureLabelIndex.index,
                preservingComments: true,
              )
            }
          }
