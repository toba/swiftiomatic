  ...
  static let indent = FormatRule(
    ...
    formatter.forEachToken(onlyWhereEnabled: false) { i, token in
      ...
      case .startOfScope(let string):
        ...
        case "/*":
          if scopeStack
            .count < 2 || scopeStack[scopeStack.count - 2] != .startOfScope("/*")
          {
        ...
        case ":":
          ...
          if formatter.options.indentCase,
            scopeStack
              .count < 2 || scopeStack[scopeStack.count - 2] != .startOfScope("#if")
          {
            indent += formatter.options.indent
          }
        case "#if":
        ...
        case "[", "(":
          ...
          if formatter
            .last(.nonSpaceOrComment, before: linebreakIndex) != .delimiter(","),
            formatter
      ...
      default:
        ...
        if let scope = scopeStack.last, token.isEndOfScope(scope) {
          ...
          let start = formatter.startOfLine(at: i)
          guard
            let firstIndex = formatter.index(
      ...
      case .endOfScope("default"), .keyword("@unknown"),
        ...
        if index == i || index == i - 1 {
          let indent: String
          if case .space(let space) = formatter.tokens[index] {
            indent = space
          } else {
            indent = ""
          }
      ...
      case .lineBreak:
            ...
            || !(nextTokenIndex == nil
              ...
              || (((_nextToken.isIdentifier
                && !(_nextToken == .identifier("async")
                  && formatter
                    .currentScope(at: nextTokenIndex!) != .startOfScope("(")))
                || _nextToken
                  .isMacro
              ...
              || (_nextToken == .delimiter(",")
                ...
                .contains(
                  formatter.currentScope(at: nextTokenIndex!)?
                    .string ?? ""))))
        ...
        if linewrapStack.last == true {
          if !linewrapped {
                ...
                .count < indent.count + formatter.options.indent.count
            {
              indent =
                formatter
                .currentIndentForLine(at: conditionBeginIndex)
                + formatter
                .options.indent
              indentStack[indentStack.count - 1] = indent
            }
          ...
          } else if formatter.token(at: nextTokenIndex ?? -1)
            == .operator(
              ".",
              .infix,
            )
        ...
        default:
            ...
            )
          {
            switch lastToken {
            case .endOfScope("}"):
              if formatter.token(at: nextNonSpaceIndex)
                == .operator(
                  ".",
                  .infix,
                )
    ...
    if formatter.options.indentStrings {
      formatter.forEach(.startOfScope("\"\"\"")) { stringStartIndex, _ in
        ...
        guard let stringEndIndex = formatter.endOfScope(at: stringStartIndex),
          ...
          formatter
            .startOfLine(at: stringStartIndex, excludingIndent: true) != stringStartIndex
        else { return }
...
extension Formatter {
  ...
  func stringBodyIndent(at i: Int) -> String {
    ...
    if let index = index(of: .nonSpace, in: start..<i),
      case .stringBody(let string) = tokens[index],
  ...
  func isWrappedDeclaration(at i: Int) -> Bool {
    guard
      ...
      case .keyword(let keyword) = tokens[keywordIndex],
      [
