...
extension StatementPositionRule {
  ...
  private func defaultValidate(file: SwiftSource) -> [RuleViolation] {
...
/// Uncuddled Behaviors
extension StatementPositionRule {
  private func uncuddledValidate(file: SwiftSource) -> [RuleViolation] {
  ...
  private static func uncuddledMatches(in file: SwiftSource) -> [UncuddledMatch] {
    uncuddledRegex.matches(in: file).compactMap { match in
  ...
  private func uncuddledViolationRanges(in file: SwiftSource) -> [Range<String.Index>] {
  ...
  private func uncuddledCorrect(file: SwiftSource) -> Int {
    var contents = file.contents
    let syntaxMap = file.syntaxMap
    let matches = Self.uncuddledMatches(in: file)
