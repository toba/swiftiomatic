...
struct UnusedDeclarationRule: AnalyzerRule, CollectingRule {
    ...
    private func violationOffsets(declaredUSRs: Set<DeclaredUSR>, allReferencedUSRs: Set<String>)
        -> [ByteCount]
    {
...
extension SwiftSource {
    ...
    fileprivate func referencedUSRs(index: SourceKitDictionary, editorOpen: SourceKitDictionary)
        -> Set<String>
    {
        Set(
            index.traverseEntitiesDepthFirst { parent, entity -> String? in
                if let usr = entity.usr,
                   ...
                   editorOpen.propertyAtOffset(nameOffset, property: \.kind)
    ...
    private func declaredUSR(
        ...
        if !configuration.includePublicAndOpen,
           [.public, .open].contains(
               editorOpen.propertyAtOffset(
                   nameOffset,
    ...
    private func cursorInfo(at byteOffset: ByteCount, compilerArguments: [String])
        -> SourceKitDictionary?
    {
    ...
    private func shouldIgnoreEntity(
        ...
        if indexEntity.shouldSkipIndexEntityToWorkAroundSR11985()
            ...
            || indexEntity.isImplicit || indexEntity.value["key.is_test_candidate"]?
            .boolValue == true
        ...
        if !Set(indexEntity.enclosedSwiftAttributes).isDisjoint(with: [.ibinspectable, .iboutlet]) {
            if let getter = indexEntity.entities.first(where: {
                $0.declarationKind == .functionAccessorGetter
            }),
                !getter.isImplicit
            {
                return true
            ...
            if let setter = indexEntity.entities.first(where: {
                $0.declarationKind == .functionAccessorSetter
            }),
                !setter.isImplicit
            {
                return true
...
extension SourceKitDictionary {
    ...
    fileprivate var annotatedDeclaration: String? {
        value["key.annotated_decl"]?.stringValue
    }

    fileprivate var isImplicit: Bool {
        value["key.is_implicit"]?.boolValue == true
    }

    fileprivate func propertyAtOffset<T>(_ offset: ByteCount, property: KeyPath<Self, T?>) -> T? {
        if let nameOffset,
           nameOffset == offset,
    ...
    fileprivate func shouldSkipRelated(relatedUSRsToSkip: Set<String>) -> Bool {
    ...
    fileprivate func shouldSkipIndexEntityToWorkAroundSR11985() -> Bool {
    ...
    fileprivate func shouldSkipResultBuilder() -> Bool {
    ...
    fileprivate func extends(reference other: Self) -> Bool {
