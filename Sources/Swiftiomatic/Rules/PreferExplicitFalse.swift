...
@_spi(Rules)
public final class PreferExplicitFalse: SyntaxFormatRule {
  public override class var isOptIn: Bool { true }

  public override func visit(_ node: PrefixOperatorExprSyntax) -> ExprSyntax {
    ...
    // Skip #if conditions
    if isInsideIfConfigCondition(node) {
      return super.visit(node)
    }

    // Skip if adjacent to comparison or casting operators
    if isAdjacentToComparisonOrCasting(node) {
      return super.visit(node)
    }

    let visited = super.visit(node)
    guard let prefixNode = visited.as(PrefixOperatorExprSyntax.self) else {
      return visited
    }
  ...
  private func isInsideIfConfigCondition(_ node: some SyntaxProtocol) -> Bool {
    ...
    while let parent = current.parent {
      if let ifConfig = parent.as(IfConfigClauseSyntax.self) {
        if let condition = ifConfig.condition, condition.id == current.id {
          return true
        }
        return false
      }
  ...
  private func isAdjacentToComparisonOrCasting(_ node: PrefixOperatorExprSyntax) -> Bool {
    ...
    if let infix = parent.as(InfixOperatorExprSyntax.self) {
      if let binOp = infix.operator.as(BinaryOperatorExprSyntax.self) {
        if Self.comparisonOperators.contains(binOp.operator.text) {
          return true
        }
      }
    }

    if parent.is(IsExprSyntax.self) || parent.is(AsExprSyntax.self) {
      return true
    }
