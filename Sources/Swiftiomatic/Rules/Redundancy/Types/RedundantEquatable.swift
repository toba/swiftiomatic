extension FormatRule {
  static let redundantEquatable = FormatRule(
    ...
    let typesManuallyImplementingEquatableConformance =
      formatter
      .manuallyImplementedEquatableTypes(
        in: declarations,
      )
    ...
    for equatableType in typesManuallyImplementingEquatableConformance {
      ...
      case "class":
        ...
        isEligibleForAutoEquatableConformance =
          formatter.options
          .equatableMacro != .none
      default:
      ...
      // can be generated by that macro instead of written manually.
      else if case .macro(let macro, module: let module) = formatter.options.equatableMacro {
        let declarationWithEquatableConformance = equatableType
          .declarationWithEquatableConformance
        ...
        if let startOfExtensionTypeBody = formatter.index(
          ...
          formatter.index(
            of: .keyword("where"),
            in: equatableConformance.index..<startOfExtensionTypeBody,
          ) != nil
        {
          continue
        }
        ...
        formatter.insert(
          [.keyword(macro), .space(" ")],
          at: equatableType.typeDeclaration
  ...
  func manuallyImplementedEquatableTypes(in declarations: [Declaration]) -> [EquatableType] {
    ...
    declarations.forEachRecursiveDeclaration { declaration in
        ...
        let startOfArguments = index(of: .startOfScope("("), after: declaration.keywordIndex)
      {
        let functionArguments =
          parseFunctionDeclarationArguments(startOfScope: startOfArguments)
    ...
    return typesWithEquatableConformances.compactMap {
      ...
      guard let typeDeclaration = typeDeclarationsByFullyQualifiedName[typeName],
        let equatableImplementation =
          equatableImplementationsByFullyQualifiedName[typeName]
      else { return nil }
