...
extension FormatRule {
  ...
  static let redundantType = FormatRule(
    ...
    formatter.forEach(.operator("=", .infix)) { i, _ in
      ...
      case .explicit:
        isInferred = false
        declarationKeywordIndex =
          formatter.declarationIndexAndScope(at: equalsIndex)
          .index
...
extension Formatter {
    ...
    -> (matches: Bool, i: Int, j: Int, wasValue: Bool)
  {
      ...
      let valueIndex = index(of: .nonSpaceOrCommentOrLineBreak, after: j)
    {
      ...
      if options.propertyTypes == .inferred,
        typeToken == .identifier("Void")
