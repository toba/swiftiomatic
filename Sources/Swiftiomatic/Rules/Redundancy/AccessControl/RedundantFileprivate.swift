  ...
  static let redundantFileprivate = FormatRule(
    ...
    formatter.forEach(.keyword("fileprivate")) { i, _ in
      ...
      guard formatter.options.swiftVersion >= "4",
        ...
        formatter.next(.nonSpaceOrCommentOrLinebreak, after: nameIndex)?
          .isOperator(".") == false,
        case .identifier(let typeName) = formatter.tokens[nameIndex],
      ...
      if memberType == "init" {
        ...
        if !formatter.isTypeInitialized(typeName, in: 0..<startIndex),
          !formatter.isTypeInitialized(
            typeName,
            in: endIndex + 1..<formatter.tokens.count,
          ),
          !formatter.isInitOverridden(for: typeName, in: 0..<startIndex),
          !formatter.isInitOverridden(
            for: typeName,
            in: endIndex + 1..<formatter.tokens.count,
          )
        {
        ...
        !formatter.areMembers(
          names, of: typeName,
          referencedIn: endIndex + 1..<formatter.tokens.count,
        )
      {
        formatter.replaceToken(at: i, with: .keyword("private"))
      }
  ...
  func areMembers(
    ...
    while i < range.upperBound {
      ...
      case .keyword("struct"), .keyword("extension"), .keyword("enum"), .keyword("actor"),
        ...
        for case .identifier(let name) in tokens[startIndex..<endIndex]
