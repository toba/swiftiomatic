  ...
  fileprivate final class Visitor: ViolationCollectingVisitor<OptionsType> {
    override func visitPost(_ node: ExtensionDeclSyntax) {
      ...
      for member in node.memberBlock.members {
        guard let modifiers = member.decl.asProtocol(WithModifiersSyntax.self)?.modifiers else {
          continue
        }
    ...
    override func visit(_ node: ExtensionDeclSyntax) -> DeclSyntax {
      ...
      let newMembers = MemberBlockItemListSyntax(
        node.memberBlock.members.map { member in
          guard let withModifiers = member.decl.asProtocol(WithModifiersSyntax.self) else {
            return member
          }
          ...
          let newModifiers = modifiers.filter { $0.name.tokenKind != .keyword(memberACL) || $0.detail != nil }
          let newDecl = member.decl.removingACLModifier(memberACL, replacingWith: newModifiers)
...
extension DeclSyntax {
  fileprivate func removingACLModifier(
    _ keyword: Keyword, replacingWith newModifiers: DeclModifierListSyntax
  ) -> DeclSyntax {
