...
extension UnneededEscapingRule {
  fileprivate final class Visitor: ViolationCollectingVisitor<OptionsType> {
    ...
    override func visitPost(_ node: SubscriptDeclSyntax) {
      if case .getter(let items) = node.accessorBlock?.accessors {
    ...
    private func validate(
      ...
      let correctionEndPosition =
        if case .spaces(let count) = attr.trailingTrivia.first, count > 0 {
...
private final class EscapeChecker: SyntaxVisitor {
  ...
  override func visitPost(_ node: CodeBlockItemListSyntax) {
    if case .expr(let returnExpr) = node.onlyElement?.item, isTainted(returnExpr) {
      doesEscape = true
    }
  }
