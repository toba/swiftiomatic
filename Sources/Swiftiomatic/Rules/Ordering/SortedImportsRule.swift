...
extension Trivia {
  fileprivate var skippingLeadingNewline: Self {
  ...
  fileprivate var splitBlocks: (first: Trivia, second: Trivia, foundSplit: Bool) {
    var leading = [TriviaPiece]()
    var trailing = [TriviaPiece]()
    var foundSplit = false

    for piece in pieces {
      if case .newlines(let count) = piece, count > 1 {
...
extension ImportDeclSyntax {
  fileprivate var asCodeBlockItem: CodeBlockItemSyntax {
