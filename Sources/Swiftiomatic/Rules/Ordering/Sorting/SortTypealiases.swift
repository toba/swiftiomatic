...
extension FormatRule {
  static let sortTypealiases = FormatRule(
    ...
    formatter.forEach(.keyword("typealias")) { typealiasIndex, _ in
      guard
        let (equalsIndex, andTokenIndices, endIndex) =
          formatter
      ...
      for elementIndex in sortedElements.indices {
        ...
        let delimiterIndexInTokens =
          sortedElements[elementIndex].delimiterIndex
          - sortedElements[elementIndex]
          .startIndex
