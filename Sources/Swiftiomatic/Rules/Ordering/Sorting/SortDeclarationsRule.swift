...
extension SortDeclarationsRule {
    fileprivate final class Visitor: ViolationCollectingVisitor<OptionsType> {
        ...
        private func hasSortDirective(_ trivia: Trivia) -> Bool {
            for piece in trivia {
                if case let .lineComment(text) = piece, text.contains("sm:sort") {
                    return true
                }
            }
            return false
        ...
        private func checkSorted(members: MemberBlockItemListSyntax,
                                 at position: AbsolutePosition)
        {
            let names = members.compactMap { member -> String? in
                ...
                if let varDecl = member.decl.as(VariableDeclSyntax.self) {
                    return varDecl.bindings.first?.pattern.as(IdentifierPatternSyntax.self)?
                        .identifier.text
                }
