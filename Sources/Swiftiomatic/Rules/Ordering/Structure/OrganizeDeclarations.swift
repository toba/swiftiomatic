...
extension Formatter {
    ...
    for (index, (declaration, category)) in sortedDeclarations.enumerated() {
        ...
        category.shouldBeMarked(in: Set(formattedCategories), for: options.organizationMode)
      {
        ...
        removeExistingCategorySeparators(
          from: declaration,
          previousDeclaration: index == 0
            ? nil
            : sortedDeclarations[index - 1]
              .declaration,
          order: order,
        ...
        if index == 0 {
          var tokensBetweenStartOfScopeAndFirstDeclaration: ArraySlice<Token> {
            tokens[
              typeDeclaration
                .openBraceIndex..<typeDeclaration.body[0].range
                .lowerBound]
          }
  ...
  func matchingCategorySeparatorComments(
    in range: Range<Int>,
    order: ParsedOrder
  ) -> [ClosedRange<
    Int,
    ...
    for consecutiveGroup in consecutiveGroups {
        ...
        declarationIndex != typeDeclaration.body.indices.last
      {
        ...
        if options.blankLineAfterSubgroups {
          ...
          switch options.organizationMode {
          case .visibility:
            mustPreserveBlankLine =
              thisCategory.visibility
              != followingCategory
              .visibility
          case .type:
...
enum VisibilityCategory: CaseIterable, Hashable, RawRepresentable {
  ...
  var rawValue: String {
    ...
    case .explicit(let declarationType):
  ...
  func categoryOrder(for mode: DeclarationOrganizationMode) -> ParsedOrder {
    ...
    case .visibility:
      let categoryPairings = VisibilityCategorys.flatMap {
        ...
        case .explicit(let explicitDeclarationType):
...
extension Array where Element: Equatable & Hashable {
  ...
  func firstElementAppearanceOrder() -> [Element] {
    ...
    for element in self where !appeared.contains(element) {
