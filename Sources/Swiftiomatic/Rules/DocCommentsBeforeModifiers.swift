...
@_spi(Rules)
public final class DocCommentsBeforeModifiers: SyntaxFormatRule {
  ...
  private func moveDocComments<D: SyntaxProtocol>(
    ...
    if hasAttributes {
      ...
      attrs[idx].leadingTrivia = insertDocBlock(
        collectedDoc, into: attrs[idx].leadingTrivia
      )
      ...
      mods[idx].leadingTrivia = insertDocBlock(
        collectedDoc, into: mods[idx].leadingTrivia
      )
    ...
    for piece in pieces {
      if piece.isDocComment {
        if !foundDoc {
          ...
          while splitIdx > 0, whitespaceBuffer[splitIdx - 1].isSpaceOrTab { splitIdx -= 1 }
