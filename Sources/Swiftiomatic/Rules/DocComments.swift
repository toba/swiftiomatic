...
@_spi(Rules)
public final class DocComments: SyntaxFormatRule {
  ...
  public override func visit(_ node: MemberBlockItemSyntax) -> MemberBlockItemSyntax {
    ...
    return processTrivia(visited, toDocComment: true, preserveRegular: isConsecutive, original: node)
  }
  ...
  public override func visit(_ node: CodeBlockItemSyntax) -> CodeBlockItemSyntax {
    ...
    if isAtFileScope(node) {
      ...
      return processTrivia(visited, toDocComment: true, preserveRegular: false, original: node)
    } else {
      // Local scope: convert /// to //
      return processTrivia(visited, toDocComment: false, preserveRegular: false, original: node)
    }
  }
  ...
  private func convertRegularToDoc<N: SyntaxProtocol>(
    _ node: N, original: some SyntaxProtocol
  ) -> N {
    var modified = false
