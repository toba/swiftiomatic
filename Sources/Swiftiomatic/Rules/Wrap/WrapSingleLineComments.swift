...
public final class WrapSingleLineComments: SyntaxFormatRule {
  ...
  public override func visit(_ token: TokenSyntax) -> TokenSyntax {
    ...
    while i < pieces.count {
      ...
      case .lineComment(let text):
        let result = tryWrap(
          text: text, prefix: "//", triviaKind: .lineComment,
          index: i, pieces: &pieces, originalIndexMap: &originalIndexMap,
          maxWidth: maxWidth)
        if result.didChange {
          changed = true
      ...
      case .docLineComment(let text):
        let result = tryWrap(
          text: text, prefix: "///", triviaKind: .docLineComment,
          index: i, pieces: &pieces, originalIndexMap: &originalIndexMap,
          maxWidth: maxWidth)
        if result.didChange {
          changed = true
    ...
    let triviaIdx = originalTrivia.index(
      originalTrivia.startIndex, offsetBy: anchorOrigIdx)
  ...
  private func tryWrap(
    ...
    let wrapped = wrapComment(
      text: text, prefix: prefix, column: column, maxWidth: maxWidth)
    ...
    for (j, line) in wrapped.enumerated() {
      ...
      let piece: TriviaPiece = switch triviaKind {
