...
extension IdentifierPatternSyntax {
  fileprivate var enclosingVarDecl: VariableDeclSyntax? {
    let identifierDecl =
...
private enum NamedDeclType: CustomStringConvertible {
  ...
  var description: String {
    switch self {
    case .function(_, let resolvedName, _): "Function name '\(resolvedName)'"
    case .enumElement(let name): "Enum element name '\(name)'"
    case .variable(let name, _, _): "Variable name '\(name)'"
    }
  }
  ...
  var isStatic: Bool {
    ...
    case .variable(_, let isStatic, _): isStatic
    }
  }

  var isPrivate: Bool {
    switch self {
    case .function(_, _, let isPrivate): isPrivate
    case .enumElement: false
    case .variable(_, _, let isPrivate): isPrivate
    }
  }
  ...
  var name: String {
    let name =
      switch self {
      case .function(let name, _, _): name
      case .enumElement(let name): name
      case .variable(let name, _, _): name
      }
    return
      name
