...
extension FormatRule {
  ...
  static let modifierOrder = FormatRule(
    ...
    formatter.forEach(.keyword) { i, token in
      ...
      loop: while let index = formatter.index(
        ...
        case let token where formatter.isModifier(at: index):
          ...
          lastIndex = index
        case .endOfScope(")"):
          if case .identifier(let param)? = formatter.last(
    ...
    **NOTE:** If the `--modifier-order` option isn't set, the default order will be:
    `\(FormatRuleCatalog.defaultModifierOrder.flatMap(\.self).joined(separator: "`, `"))`
    """
  }
}
...
extension Formatter {
  ...
  var preferredModifierOrder: [String] {
    ...
    for (i, modifiers) in FormatRuleCatalog.defaultModifierOrder.enumerated() {
      for modifier in modifiers {
        priorities[modifier] = i
      }
    }
    var order = options.modifierOrder.flatMap { FormatRuleCatalog.mapModifiers($0) ?? [] }
    for (i, modifiers) in FormatRuleCatalog.defaultModifierOrder.enumerated() {
      let insertionPoint =
