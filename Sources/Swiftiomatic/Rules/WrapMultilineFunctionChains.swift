...
public final class WrapMultilineFunctionChains: SyntaxFormatRule {
  ...
  public override func visit(_ node: FunctionCallExprSyntax) -> ExprSyntax {
    ...
    let indent: String = periods.first { $0.leadingTrivia.containsNewlines }?
  ...
  private func collectChain(
    ...
    if let callExpr = expr.as(FunctionCallExprSyntax.self) {
      ...
      hasFunctionCall = true
      collectChain(subscriptExpr.calledExpression, periods: &periods, hasFunctionCall: &hasFunctionCall)
      ...
      collectChain(optionalChain.expression, periods: &periods, hasFunctionCall: &hasFunctionCall)
    } else if let forceUnwrap = expr.as(ForceUnwrapExprSyntax.self) {
      collectChain(forceUnwrap.expression, periods: &periods, hasFunctionCall: &hasFunctionCall)
    }
  }
  ...
  private func isInnerChainCall(_ expr: ExprSyntax) -> Bool {
    ...
    if parent.as(MemberAccessExprSyntax.self) != nil {
      if let grandparent = parent.parent,
         grandparent.is(FunctionCallExprSyntax.self) || grandparent.is(SubscriptCallExprSyntax.self)
      {
        return true
      }
    }
  ...
  private func nextPeriodInChain(after period: TokenSyntax, in periods: [TokenSyntax]) -> TokenSyntax? {
