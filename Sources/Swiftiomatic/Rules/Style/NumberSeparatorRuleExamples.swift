enum NumberSeparatorRuleExamples {
    static let nonTriggeringExamples: [Example] = ["-", "+", ""].flatMap { (sign: String) -> [Example] in
        [
    ...
    static let triggeringExamples =
        makeTriggeringExamples(signs: ["-↓", "+↓", "↓"]) + makeTriggeringExamplesWithParentheses()
    ...
    private static func makeCorrections(signs: [(String, String)]) -> [Example: Example] {
        ...
        for (violation, sign) in signs {
            ...
            result[
                Example("let foo = \(violation)1.0001", configuration: ["minimum_fraction_length": 3])
            ] =
                Example("let foo = \(sign)1.000_1")
            result[
                Example(
                    "let foo = \(violation)1_000_000.000000_1", configuration: ["minimum_fraction_length": 3]
                )
            ] =
                Example("let foo = \(sign)1_000_000.000_000_1")
            result[
                Example(
                    "let foo = \(violation)1000000.000000_1", configuration: ["minimum_fraction_length": 3]
                )
            ] =
                Example("let foo = \(sign)1_000_000.000_000_1")
