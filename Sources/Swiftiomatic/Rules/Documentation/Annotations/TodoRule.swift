...
extension TodoRule {
    fileprivate final class Visitor: ViolationCollectingVisitor<OptionsType> {
        ...
        private func violations(in piece: TriviaPiece,
                                offset: AbsolutePosition) -> [SyntaxViolation]
        {
            let comment: String
            switch piece {
                case let .blockComment(text), let .lineComment(text),
                     let .docBlockComment(text), let .docLineComment(text):
                    comment = text
                default:
                    return []
            }
