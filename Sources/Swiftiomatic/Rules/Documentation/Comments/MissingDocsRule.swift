...
extension MissingDocsRule {
  fileprivate final class Visitor: ViolationCollectingVisitor<OptionsType> {
    ...
    override func visitPost(_ node: EnumCaseDeclSyntax) {
      guard !node.hasDocComment, case .enum(let enumAcl) = aclScope.peek() else {
        return
      }
    ...
    override func visit(_ node: InitializerDeclSyntax) -> SyntaxVisitorContinueKind {
      if node.signature.parameterClause.parameters.isNotEmpty
        || !configuration
          .excludesTrivialInit
      {
...
extension SyntaxProtocol {
  fileprivate var hasDocComment: Bool {
...
private enum AccessControlBehavior {
  ...
  var explicitAcl: AccessControlLevel? {
    switch self {
    case .actor(let acl): acl
    case .local: nil
    case .class(let acl): acl
    case .enum(let acl): acl
    case .extension(let acl): acl
    case .protocol(let acl): acl
    case .struct(let acl): acl
    }
  }
...
extension Stack<AccessControlBehavior> {
  fileprivate mutating func push(behavior: AccessControlBehavior, evalEffectiveAcl: Bool) {
  ...
  fileprivate func computeAcl(givenExplicitAcl acl: AccessControlLevel?, evalEffectiveAcl: Bool)
    -> AccessControlLevel
  {
    if let parentBehavior = peek() {
      switch parentBehavior {
      ...
      case .actor, .class, .struct, .enum:
        if let acl {
          acl
            < parentBehavior
            .effectiveAcl || !evalEffectiveAcl ? acl : parentBehavior.effectiveAcl
        } else {
          parentBehavior.effectiveAcl >= .internal
            ? .internal
            : parentBehavior
              .effectiveAcl
        }
