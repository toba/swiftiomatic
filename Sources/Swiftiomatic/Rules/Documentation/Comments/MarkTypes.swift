...
extension FormatRule {
  static let markTypes = FormatRule(
    ...
    for (index, declaration) in declarations.enumerated() {
      ...
      let potentialCommentRange =
        markInsertIndex..<typeDeclaration.leadingCommentRange
        .upperBound
      ...
      let commentsToRemove = formatter.singleLineComments(
        ...
        matching: { comment in
          ...
          where comment.lowercased().hasPrefix(commentPrefix.lowercased()) {
            return true
          }
