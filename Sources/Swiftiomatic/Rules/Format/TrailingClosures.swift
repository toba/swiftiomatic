            ...
            let trailingClosures = arguments.suffix(while: { arg in
                ...
                guard
                    let first = formatter.index(
                        of: .nonSpaceOrCommentOrLinebreak, in: range.lowerBound ..< range.upperBound + 1
                    ),
                    let last = formatter.index(
                        of: .nonSpaceOrCommentOrLinebreak, before: range.upperBound + 1, if: { _ in true }
                    ),
            ...
            if trailingClosures.count == 1 {
                ...
                guard
                    let closingBraceIndex = formatter.index(
                        of: .nonSpaceOrComment, before: functionCallClosingParen, if: { $0 == .endOfScope("}") }
                    ),
