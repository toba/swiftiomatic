...
extension FormatRule {
  ...
  static let spaceAroundOperators = FormatRule(
    ...
    formatter.forEachToken { i, token in
      ...
      case .operator(let name, .infix)
      where formatter.options.noSpaceOperators.contains(name)
        || (formatter.options.spaceAroundRangeOperators == .remove
          && token
            .isRangeOperator):
        if formatter.token(at: i + 1)?.isSpace == true,
          ...
          !prevToken.isCommentOrLinebreak,
      ...
      case .operator(_, .infix):
        if token.isRangeOperator,
          formatter.options.spaceAroundRangeOperators != .insert
        {
          break
        }
