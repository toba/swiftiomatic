        ...
        formatter.forEach(.endOfScope) { i, token in
            ...
            switch token {
                case .endOfScope(")"):
                    ...
                    if formatter.options.swiftVersion >= "6.1",
                       let identifierIndex = formatter
                       .parseFunctionIdentifier(beforeStartOfScope: startOfScope),
                    ...
                    // then we know it's a tuple.
                            ...
                            formatter.commaSeparatedElementsInScope(startOfScope: startOfScope)
                            .count > 1
                    {
                        trailingCommaSupported = true
                            ...
                            )
                    {
                        ...
                        let tokensPrecedingValuesNotTypes: Set<Token> = [
                            .startOfScope("{"), .keyword("return"), .keyword("throw"),
                            .keyword("switch"),
                            .endOfScope("case"),
                        ]
                        if tokensPrecedingValuesNotTypes
                            .contains(formatter.tokens[tokenBeforeStartOfScope])
                        {
                            trailingCommaSupported = true
                        }
                ...
                case .endOfScope(">"):
                    ...
                    if formatter.options.swiftVersion >= "6.2" {
                        ...
                        if let identifierIndex = formatter.index(
                            ...
                            keyword.isKeyword,
                            ["class", "actor", "struct", "enum", "typealias", "func"]
    ...
    func addOrRemoveTrailingComma(
        ...
        switch options.trailingCommas {
            ...
            case .multiElementLists:
                switch trailingCommaSupported {
                    case _
                    where commaSeparatedElementsInScope(startOfScope: startOfScope).count <= 1,
                         false?:
                        trailingCommaMode = .remove
                    case true?:
                        trailingCommaMode = .insert
