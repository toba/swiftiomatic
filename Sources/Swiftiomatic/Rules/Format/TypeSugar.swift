            ...
            switch formatter.tokens[typeIndex] {
                case .identifier("Array"):
                    formatter.replaceTokens(
                        in: typeIndex ... endIndex,
                        with: [.startOfScope("[")] + formatter
                            .tokens[typeStart ... typeEnd] + [.endOfScope("]")],
                    )

                case .identifier("Dictionary"):
                    guard let commaIndex = formatter.index(
                        of: .delimiter(","),
                        in: typeStart ..< typeEnd,
                    ) else {
                        return
                    }
                ...
                case .identifier("Optional"):
                    switch formatter.options.shortOptionals {
                        ...
                        case .preserveStructInits, .exceptPropertiesDeprecated:
                            ...
                            {
                                let inStructWithSynthesizedMemberwiseInit =
                                    parentType.keyword == "struct"
                                        && !parentType.body
                                        .contains(where: { $0.keyword == "init" })

                                let isStoredInstanceProperty = property.isStoredInstanceProperty
                                let hasNoDefaultValue = property.parsePropertyDeclaration()?
                                    .value == nil

                                if inStructWithSynthesizedMemberwiseInit, isStoredInstanceProperty,
                                   hasNoDefaultValue
                                {
                                    return
                                }
                    ...
                    ].contains(where: typeTokens.contains) {
