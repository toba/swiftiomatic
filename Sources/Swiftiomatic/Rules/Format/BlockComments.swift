  ...
  static let blockComments = FormatRule(
    ...
    formatter.forEachToken { i, token in
      ...
      case .startOfScope("/*"):
        ...
        if case .commentBody(let body) = formatter.tokens[i + 1] {
          ...
          nextIndex > i + 2
        {
          ...
          endIndex += formatter.replaceCommentBody(
            at: startIndex, isDocComment: isDocComment,
            stripLeadingStars: &stripLeadingStars,
          )
        }
          ...
          case .linebreak:
            ...
            var delta =
              ...
              + formatter.replaceCommentBody(
                at: index + 1, isDocComment: isDocComment,
                stripLeadingStars: &stripLeadingStars,
              )
            index += delta
            endIndex += delta
...
extension Formatter {
  func replaceCommentBody(
    ...
    var delta = 0
    var space = ""
    if case .space(let s) = tokens[index] {
      removeToken(at: index)
      space = s
      delta -= 1
    }
    if case .commentBody(let body)? = token(at: index) {
