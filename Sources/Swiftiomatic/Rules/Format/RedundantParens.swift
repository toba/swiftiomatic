...
extension FormatRule {
    ...
    static let redundantParens = FormatRule(
        ...
        formatter.forEach(.startOfScope("(")) { i, _ in
                ...
                case .operator("->", .infix), .keyword("throws"), .keyword("rethrows"),
                    ...
                    if prevToken != .keyword("throws"),
                       formatter.index(
                           before: i,
                           where: {
                               [.endOfScope(")"), .operator("->", .infix), .keyword("for")]
                                   .contains($0)
                           },
                       ) == nil,
                    ...
                    {
                        isClosure =
                            formatter.isStartOfClosure(at: scopeIndex) && formatter
                                .isInClosureArguments(at: i)
                    }
            ...
            switch prevToken {
                case .stringBody, .operator("?", .postfix), .operator("!", .postfix), .operator(
                "->",
                .infix,
            ):
                    return
                ...
                case _ where isClosure:
                    ...
                    if let index = formatter.tokens[i + 1 ..< closingIndex]
                        .firstIndex(of: .identifier("_")),
                ...
                case .delimiter(","), .endOfScope, .keyword:
                    ...
                    if ![.startOfScope("{"), .delimiter(","), .startOfScope(":")]
                        .contains(nextToken),
                ...
                default:
                    ...
                    if prevToken == .startOfScope("("),
                       formatter
                       .last(.nonSpaceOrComment, before: previousIndex) == .identifier("Selector")
                    {
                        return
                    }
                       ...
                       case .operator = formatter.tokens[index]
                    {
                        if nextToken.isOperator(".")
                            ||
                            (index == i + 1 && formatter.token(at: i - 1)?
                    ...
                    guard formatter
                        ...
                        formatter.index(
                            ...
                            where: {
                                switch $0 {
                                    case .operator(_, .infix), .identifier("any"),
                                         .identifier("some"),
                                         .identifier("each"),
                                        ...
                                        switch prevToken {
                                            ...
                                            case .operator("=", .infix) where $0 == .operator(
                                            "->",
                                            .infix,
                                        ):
                                                break
                                    ...
                                    case .operator(_, .postfix):
                                        switch prevToken {
                                            case .operator(_, .prefix), .keyword("as"),
                                                 .keyword("is"):
                                                return true
                                            default:
                                                return false
                        ...
                        formatter.index(
                            in: i + 1 ..< closingIndex,
                            where: { $0.isUnwrapOperator },
                        ) ?? closingIndex
                        >= formatter
                        .index(of: .nonSpace, before: closingIndex) ?? closingIndex - 1
                    else {
                        return
                    }
                    if formatter
