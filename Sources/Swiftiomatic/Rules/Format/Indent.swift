...
extension FormatRule {
    ...
    static let indent = FormatRule(
        ...
        options: [
            "indent", "tab-width", "smart-tabs", "indent-case", "ifdef", "xcode-indentation",
            "indent-strings",
        ],
        ...
        if formatter.options.fragment,
           ...
           case let .space(string) = indentToken
        {
            ...
            switch token {
                case let .startOfScope(string):
                    switch string {
                        case ":" where scopeStack.last == .endOfScope("case"):
                            popScope()
                        case "{"
                        where !formatter.isStartOfClosure(at: i) && linewrapStack.last == true:
                            indentStack.removeLast()
                    ...
                    if lineIndex > scopeStartLineIndexes.last ?? -1 {
                        indentCount = 1
                    } else if token.isMultilineStringDelimiter,
                              ...
                              formatter.next(.linebreak, in: endIndex + 1 ..< closingIndex) != nil
                    {
                        indentCount = 1
                              ...
                              scopeStack[scopeStack.count - 2] == .startOfScope(":")
                    {
                        indentCount = 1
                    } else {
                    ...
                    switch string {
                        case "/*":
                            if scopeStack
                                .count < 2 || scopeStack[scopeStack.count - 2] !=
                                .startOfScope("/*")
                            {
                        ...
                        case "{" where formatter.isFirstStackedClosureArgument(at: i):
                            ...
                            if formatter.tokens[prevIndex] == .delimiter(":") {
                                guard formatter.token(at: prevIndex - 1)?.isIdentifier == true,
                                      let parenIndex = formatter.index(
                                          of: .nonSpace, before: prevIndex - 1,
                                          if: {
                                              $0 == .startOfScope("(")
                                          },
                                      )
                                    ...
                                    stringBodyIndentStack[stringBodyIndentStack.count - 1] =
                                        stringIndent
                            ...
                            indent = formatter.spaceEquivalentToTokens(
                                from: startIndex,
                                upTo: prevIndex + 1,
                            )
                        ...
                        case "[", "(":
                            ...
                            if formatter
                                .last(.nonSpaceOrComment, before: linebreakIndex) !=
                                .delimiter(","),
                                formatter
                                .next(.nonSpaceOrComment, after: linebreakIndex) != .delimiter(",")
                            {
                                fallthrough
                            }
                ...
                case .keyword("in") where scopeStack.last == .startOfScope("{"):
                    if let startIndex = formatter.index(of: .startOfScope("{"), before: i),
                       ...
                       let paramsIndex = formatter.index(
                           of: .startOfScope,
                           in: startIndex + 1 ..< i,
                       ),
                ...
                case .operator("=", .infix):
                    ...
                    guard let nextKeyword = formatter.index(
                ...
                default:
                    ...
                    defer {
                           ...
                           let startOfScope = formatter.startOfScope(at: i)
                        {
                            ...
                            var previousAssignmentIndex = formatter.index(
                                of: .operator("=", .infix), before: startOfScope,
                            )
                            while let currentPreviousAssignmentIndex = previousAssignmentIndex,
                                  formatter
                                  .isConditionalStatement(at: currentPreviousAssignmentIndex),
                                  let nextPreviousAssignmentIndex = formatter.index(
                                      of: .operator("=", .infix),
                            ...
                            if scopeStack.last == .operator("=", .infix),
                               ...
                               .conditionalBranches(at: nextTokenAfterAssignment),
                    ...
                    if let scope = scopeStack.last, token.isEndOfScope(scope) {
                        ...
                        if token == .endOfScope(")"),
                           ...
                           formatter
                           .isTrailingClosureOnLineAfterMultilineMethodCall(at: nextTokenIndex),
                           let functionCallStartOfScope = formatter.startOfScope(at: i)
                        {
                            let indent = formatter
                                .currentIndentForLine(at: functionCallStartOfScope)
                            indentStack.append(indent)
                            stringBodyIndentStack.append("")
                            indentCounts.append(indentCount)
                            scopeStartLineIndexes.append(lineIndex)
                        ...
                        if token == .endOfScope("}"),
                           let startOfScope = formatter.startOfScope(at: i),
                           formatter
                           .isTrailingClosureOnLineAfterMultilineMethodCall(at: startOfScope)
                        {
                            popScope()
                        }
                        ...
                        let start = formatter.startOfLine(at: i)
                        guard let firstIndex = formatter.index(
                        ...
                        if token == .endOfScope("#endif"),
                           formatter.options.ifdefIndent == .outdent
                        {
                            i += applyIndent("", at: start)
                        } else {
                            var indent = indentStack.last ?? ""
                            if token.isSwitchCaseOrDefault,
                               formatter.options.indentCase, !formatter.isInIfdef(
                                   at: i,
                ...
                case .linebreak:
                    ...
                    let linewrapped =
                        lastNonSpaceOrLinebreakIndex > -1
                            && (!formatter.isEndOfStatement(
                                at: lastNonSpaceOrLinebreakIndex,
                                in: scopeStack.last,
                            )
                                ||
                                (nextTokenIndex
                                    .map { formatter.isTrailingClosureLabel(at: $0) } == true)
                                || !(nextTokenIndex == nil
                                    || [
                                        .endOfScope("}"), .endOfScope("]"), .endOfScope(")"),
                                    ].contains(_nextToken) || _nextToken.isStringBody
                                    || formatter.isStartOfStatement(
                                        at: nextTokenIndex!,
                                        in: scopeStack.last,
                                    )
                                    || (((_nextToken.isIdentifier
                                            && !(_nextToken == .identifier("async")
                                                && formatter
                                                .currentScope(at: nextTokenIndex!) !=
                                                .startOfScope("(")))
                                        || _nextToken
                                        .isMacro || [.keyword("try"), .keyword("await")]
                                        .contains(_nextToken))
                                        && formatter.last(
                                            .nonSpaceOrCommentOrLinebreak,
                                            ...
                                            $0 != .keyword("return") && !$0
                                                .isOperator(ofType: .infix)
                                        } ?? false)
                                    || (_nextToken == .delimiter(",")
                                        && [
                                            "<", "[", "(", "case",
                                        ]
                                        .contains(formatter.currentScope(at: nextTokenIndex!)?
                                            .string ?? ""))))
                    ...
                    if linewrapStack.last == true {
                        if !linewrapped {
                               ...
                               .count < indent.count + formatter.options.indent.count
                            {
                                indent =
                                    formatter
                                        .currentIndentForLine(at: conditionBeginIndex) + formatter
                                        .options.indent
                                indentStack[indentStack.count - 1] = indent
                            }

                            let startOfLineIndex = formatter.startOfLine(
                                at: i,
                            ...
                            if formatter.options.wrapTernaryOperators == .beforeOperators,
                               startOfLine == .operator(":", .infix) || startOfLine == .operator(
                                   "?",
                                   .infix,
                               )
                            {
                            ...
                            if formatter.options.wrapTernaryOperators == .beforeOperators,
                               ...
                               let previousOperatorIndex = scope.string.components(separatedBy: "-")
                               .last.flatMap({
                                   Int($0)
                               })
                            {
                        ...
                        } else if formatter.token(at: nextTokenIndex ?? -1) == .operator(
                            ".",
                            .infix,
                        )
                            || formatter.isLabel(at: nextTokenIndex ?? -1)
                        {
                            var lineStart = formatter.startOfLine(
                                at: lastNonSpaceOrLinebreakIndex, excludingIndent: true,
                            )
                               ...
                               ].contains(startToken)
                            {
                                if let index = formatter.index(
                                    ...
                                    lineStart = formatter.startOfLine(
                                        at: lastNonSpaceOrLinebreakIndex, excludingIndent: true,
                                    )
                                }
                            }
                            ...
                            } else if formatter.tokens[lineStart ..< lastNonSpaceOrLinebreakIndex]
                                .allSatisfy({
                                ...
                                })
                            {
                                if lastToken.isEndOfScope {
                                    indent = formatter
                                        .currentIndentForLine(at: lastNonSpaceOrLinebreakIndex)
                                }
                                if formatter.options.ifdefIndent == .preserve,
                                   preserveIfdefDepth > 0
                                {
                                    // keep relative indentation unchanged
                                } else if !lastToken
                                    .isEndOfScope || lastToken == .endOfScope("case")
                            ...
                            } else if !formatter.options.xcodeIndentation || !formatter
                                .isWrappedDeclaration(at: i)
                            {
                    ...
                    switch nextToken {
                        ...
                        case .startOfScope("/*"), .commentBody, .endOfScope("*/"):
                            nextNonSpaceIndex = formatter
                                .endOfScope(at: nextNonSpaceIndex) ?? nextNonSpaceIndex
                            fallthrough
                        case .startOfScope("//"):
                            nextNonSpaceIndex =
                                formatter.index(
                        ...
                        default:
                            ...
                            if formatter.options.fragment, lastToken == .delimiter(","),
                               formatter.startOfScope(at: i) == nil
                            {
                                break // Can't reliably indent
                            }
                            if lastIndex == formatter.startOfLine(
                                at: lastIndex,
                                excludingIndent: true,
                            ) {
                                switch lastToken {
                                    case .endOfScope("}"):
                                        if formatter.token(at: nextNonSpaceIndex) == .operator(
                                            ".",
                                            .infix,
                                        )
                                        ...
                                        } else if [.keyword("else"), .startOfScope("{")]
                                            .contains(nextToken),
                                            let scopeStart = formatter
                                            .startOfScope(at: lastIndex),
                                            let startIndex = formatter
                                            .indexOfLastSignificantKeyword(at: scopeStart),
                                            [.keyword("else"), .keyword("switch")]
                                            .contains(formatter.tokens[startIndex]),
                                            let equalsIndex = formatter.index(
                                                of: .operator("=", .infix),
                                                before: startIndex,
                                            ),
                                            let outerIndex = formatter
                                            .startOfConditionalStatement(at: equalsIndex)
                                        {
        ...
        if formatter.options.indentStrings {
            formatter.forEach(.startOfScope("\"\"\"")) { stringStartIndex, _ in
                ...
                guard let stringEndIndex = formatter.endOfScope(at: stringStartIndex),
                      ...
                      formatter
                      .startOfLine(at: stringStartIndex, excludingIndent: true) !=
                      stringStartIndex
                else { return }
