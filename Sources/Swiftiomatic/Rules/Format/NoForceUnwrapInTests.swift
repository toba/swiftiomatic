      ...
      for forceUnwrapOperator in forceUnwrapOperators {
        ...
        if needsUnwrapMethod {
          ...
          let startsWithPrefixOperator =
            ...
            && formatter.tokens[expressionRange.lowerBound]
              != .operator(
                ".",
                .prefix,
              )
          ...
          let insertionIndex =
            startsWithPrefixOperator
            ? expressionRange.lowerBound + 1
            : expressionRange
              .lowerBound
...
extension Formatter {
    ...
    -> AutoUpdatingRange?
  {
    ...
    while let asIndexNeedingExpansion = expressionRange.range.first(where: {
      guard
        let tokenAfterForceCastParenScope =
