...
extension Formatter {
  ...
  func validateTestFunctionAccessControl(
    ...
    if modifiers.contains("override") || modifiers.contains("@objc")
      || modifiers
        .contains("static")
    {
      return
  ...
  func removeACLModifiers(from declaration: Declaration, except exceptions: [String]) {
    for aclModifier in FormatRuleCatalog.aclModifiers where !exceptions.contains(aclModifier) {
  ...
  func ensurePrivateAccessControl(_ declaration: Declaration) {
    ...
    for aclModifier in FormatRuleCatalog.aclModifiers {
      if let modifierIndex = indexOfModifier(
        aclModifier, forDeclarationAt: declaration.keywordIndex,
      ) {
