...
extension ValidateTestCasesRule {
  fileprivate final class Visitor: ViolationCollectingVisitor<OptionsType> {
    ...
    override func visitPost(_ node: FunctionDeclSyntax) {
      ...
      guard !["setUp", "setUpWithError", "tearDown", "tearDownWithError"].contains(name) else {
        return
      }

      // Skip private/fileprivate methods — these are helpers
      if node.modifiers.contains(keyword: .private) || node.modifiers.contains(keyword: .fileprivate) {
        return
      }
      ...
      if node.modifiers.contains(keyword: .static) || node.modifiers.contains(keyword: .class) {
        return
      }
