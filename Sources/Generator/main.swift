...
// Content-based skip: hash all input files. If the hash matches the stamp from
// the previous run, the generator output would be byte-identical (FileGenerator
// already does this at the per-file level), so skip the entire swift-syntax
// parse + collection pass. Saves several seconds on incremental builds when
