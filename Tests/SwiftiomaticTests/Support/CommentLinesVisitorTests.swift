...
@Suite(.rulesRegistered) struct CommentLinesVisitorTests {
  ...
  @Test func multipleSingleLineComments() {
    let contents = """
      // First comment
      // Second comment
      // Third comment
  ...
  @Test func mixedCommentsAndCode() {
    let contents = """
      // Comment at the top
      import Foundation
  ...
  @Test func commentsWithWhitespace() {
    let contents = """
  ...
  @Test func inlineCommentsNotCounted() {
    let contents = """
      let x = 5 // This comment is on the same line as code
      print("test") /* inline block comment */
      """
    #expect(commentOnlyLines(in: contents) == [])
  }

  @Test func commentBlockStartedOnCodeLine() {
    let contents = """
      print("test") /* block
                       comment */
      """
