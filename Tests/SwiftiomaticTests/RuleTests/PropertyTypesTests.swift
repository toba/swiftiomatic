...
@Suite struct PropertyTypesTests {
    @Test func convertsExplicitTypeToInferredType() {
        let input = """
        let foo: Foo = .init()
    ...
    @Test func convertsInferredTypeToExplicitType() {
        let input = """
        let foo = Foo()
        let bar = Bar.staticBar
        let quux = Quux.quuxBulder(foo: .foo, bar: .bar)
    ...
    @Test func convertsTypeMembersToExplicitType() {
        let input = """
        struct Foo {
            let foo = Foo()
            let bar = Bar.staticBar
    ...
    @Test func convertsLocalsToImplicitType() {
        let input = """
        struct Foo {
            let foo = Foo()
    ...
    @Test func preservesInferredTypeFollowingTypeWithDots() {
        let input = """
        let baaz = Baaz.Example.default
    ...
    @Test func preservesExplicitTypeIfNoRHS() {
        let input = """
        let foo: Foo
        let bar: Bar
        """
    ...
    @Test func preservesImplicitTypeIfNoRHSType() {
        let input = """
        let foo = foo()
        let bar = bar
        let int = 24
    ...
    @Test func preservesImplicitForVoidAndTuples() {
        let input = """
        let foo = Void()
    ...
    @Test func preservesExplicitTypeIfUsingLocalValueOrLiteral() {
        let input = """
    ...
    @Test func compatibleWithRedundantTypeInferred() {
        let input = """
        let foo: Foo = Foo()
        """
        ...
        testFormatting(
            for: input,
            [output],
            rules: [.redundantType, .propertyTypes],
            options: options,
        )
    }

    @Test func compatibleWithRedundantTypeExplicit() {
        let input = """
        let foo: Foo = Foo()
        """
    ...
    @Test func compatibleWithRedundantTypeInferLocalsOnly() {
        let input = """
    ...
    @Test func propertyTypeWithIfExpressionDisabledByDefault() {
        let input = """
    ...
    @Test func propertyTypeWithIfExpression() {
        let input = """
        let foo: Foo =
            if condition {
                .init(bar)
        ...
        let options = FormatOptions(
            propertyTypes: .inferred, inferredTypesInConditionalExpressions: true,
        )
        testFormatting(
            for: input,
            [output],
    ...
    @Test func propertyTypeWithSwitchExpression() {
        let input = """
        let foo: Foo =
            switch condition {
            case true:
    ...
    @Test func preservesNonMatchingIfExpression() {
        let input = """
        let foo: Foo =
            if condition {
                .init(bar)
    ...
    @Test func preservesExplicitOptionalType() {
    ...
    @Test func preservesTypeWithSeparateDeclarationAndProperty() {
        let input = """
        var foo: Foo!
        foo = Foo(afterDelay: {
            print(foo)
    ...
    @Test func preservesTypeWithExistentialAny() {
        let input = """
        protocol ShapeStyle {}
        ...
        let options = FormatOptions(propertyTypes: .inferred)
        testFormatting(
            for: input, rule: .propertyTypes, options: options,
            exclude: [.wrapFunctionBodies, .wrapPropertyBodies],
        )
    }

    @Test func preservesTypeWithSomeKeyword() {
        let input = """
        /// This compiles
    ...
    @Test func preservesExplicitRightHandSideWithOperator() {
        let input = """
    ...
    @Test func preservesInferredRightHandSideWithOperators() {
        let input = """
        let foo = Foo().bar
    ...
    @Test func nonUppercaseSymbolsNotTreatedAsTypes() {
        let input = """
        let foo = bar()
        let foo = _bar()
        """
    ...
    @Test func preservesUserProvidedSymbolTypes() {
        let input = """
        class Foo {
            let foo = Foo()
            let bar = Bar()
        ...
        let options = FormatOptions(
            propertyTypes: .inferLocalsOnly, preservedPropertyTypes: ["Foo", "Baaz", "quux"],
        )
        testFormatting(for: input, output, rule: .propertyTypes, options: options)
    }

    @Test func preserveInitIfExplicitlyExcluded() {
        let input = """
        class Foo {
            let foo = Foo()
            let bar = Bar.init()
    ...
    @Test func closureBodyIsConsideredLocal() {
        let input = """
        foo {
            let bar = Bar()
            let baaz: Baaz = .init()
    ...
    @Test func ifGuardConditionsPreserved() {
        let input = """
        if let foo = Foo(bar) {
            let foo = Foo(bar)
    ...
    @Test func propertyObserversConsideredLocal() {
        let input = """
        class Foo {
            var foo: Foo {
                get {
    ...
    @Test func handlesEscapedIdentifiers() {
        let input = """
    ...
    @Test func emptyArrayLiteralToInferredType() {
        let input = """
        let array: [Int] = []
        var numbers: [Double] = []
        let matrix: [[String]] = []
    ...
    @Test func emptyDictionaryLiteralToInferredType() {
        let input = """
        let dict: [String: Int] = [:]
    ...
    @Test func emptyCollectionInitToCollectionLiteral() {
        let input = """
        let array = [Foo]()
        var mapping = [Int: String]()
        let nested = [String: [Int: Bool]]()
    ...
    @Test func emptyCollectionLiteralWithSpaces() {
        let input = """
        let array: [Int] = [ ]
        let dict: [String: Int] = [ : ]
        """
        ...
        testFormatting(
            ...
            exclude: [.spaceInsideBrackets, .spaceAroundOperators],
        )
    }

    @Test func nonEmptyCollectionLiteralsNotChanged() {
        let input = """
        let array: [Int] = [1, 2, 3]
        ...
        let options = FormatOptions(propertyTypes: .inferred)
        testFormatting(
            for: input, output, rule: .propertyTypes, options: options, exclude: [.redundantType],
        )
    }

    @Test func emptyCollectionLiteralWithComplexTypes() {
        let input = """
        let array: [Int?] = []
        let dict: [String: Int?] = [:]
    ...
    @Test func emptySetLiteralToInferredType() {
        let input = """
        let set: Set<Int> = []
    ...
    @Test func emptySetInitToSetLiteral() {
        let input = """
        let set = Set<Foo>()
