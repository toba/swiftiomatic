...
@Suite struct RuleConfigurationDescriptionTests {
  ...
  private struct MockConfiguration: RuleConfiguration {
    ...
    @ConfigurationElement(
      key: "set", deprecationNotice: .suggestAlternative(ruleID: "my_rule", name: "other_opt"))
    ...
    mutating func apply(configuration: Any) throws(Swiftiomatic.Issue) {
      ...
      } catch let issue where issue == Swiftiomatic.Issue.nothingApplied(ruleID: Parent.identifier)
      {
        // Acceptable
      }
      ...
      if !supportedKeys.isSuperset(of: configuration.keys) {
        ...
        Swiftiomatic.Issue.invalidConfigurationKeys(ruleID: Parent.identifier, keys: unknownKeys)
          .print()
      }
    }
