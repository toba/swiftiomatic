...
@Suite(.rulesRegistered) struct YamlSwiftLintTests {
    @Test func flattenYaml() {
        do {
            ...
            #expect(dict1["string"] as? String == "string" && dict2["string"] as? String ==
                "string")
            ...
            #expect(dictFromArray1["bool"] as? Bool == true && dictFromArray2["bool"] as? Bool ==
                true)
            #expect(dictFromArray1["int"] as? Int == 1 && dictFromArray2["int"] as? Int == 1)
            #expect(
                dictFromArray1["double"] as? Double == 1.0 && dictFromArray2["double"] as? Double ==
                    1.0,
            )
            #expect(
            ...
            Issue.record(Comment(rawValue: error.localizedDescription))
        }
    }
