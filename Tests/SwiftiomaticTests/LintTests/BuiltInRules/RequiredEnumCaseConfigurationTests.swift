...
@Suite struct RequiredEnumCaseConfigurationTests {
    ...
    private func validateRulesExistForProtocol1() {
        #expect(config.protocols[Self.protocol1]?.contains(Self.rule1) ?? false)
        #expect(config.protocols[Self.protocol1]?.contains(Self.rule2) ?? false)
    }
    ...
    private func validateRulesExistForProtocol3() {
        #expect(config.protocols[Self.protocol3]?.contains(Self.rule3) ?? false)
        #expect(config.protocols[Self.protocol3]?.contains(Self.rule2) ?? false)
    }
