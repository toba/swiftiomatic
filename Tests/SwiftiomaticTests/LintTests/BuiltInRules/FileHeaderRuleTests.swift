...
@Suite struct FileHeaderRuleTests {
  ...
  @Test func fileHeaderWithForbiddenPatternUsingFilenamePlaceholder() throws {
    ...
    // Triggering tests
    #expect(
      try validate(fileName: "FileNameMatchingSimple.swift", using: configuration1).count == 1)
    #expect(
      try validate(fileName: "FileNameMatchingComplex.swift", using: configuration2).count == 1)
  }
