...
@Suite struct ExplicitTypeInterfaceConfigurationTests {
    ...
    @Test func invalidKeyInCustomConfiguration() async throws {
        ...
        #expect(console == "warning: Configuration for 'explicit_type_interface' rule contains the invalid key(s) 'invalidKey'.")
    }
    ...
    @Test func consoleDescription() throws {
        ...
        #expect(RuleConfigurationDescription.from(configuration: config).oneLiner() == "severity: warning; excluded: [class, instance]; allow_redundancy: false")
    }
}
