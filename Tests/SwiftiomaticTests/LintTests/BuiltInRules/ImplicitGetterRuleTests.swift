...
@Suite struct ImplicitGetterRuleTests {
  ...
  @Test func propertyReason() throws {
    ...
    #expect(
      violations.first?.reason == "Computed read-only properties should avoid using the get keyword"
    )
  }
  ...
  @Test func subscriptReason() throws {
    ...
    #expect(violations.count == 1)
    #expect(
      violations.first?.reason == "Computed read-only subscripts should avoid using the get keyword"
    )
  }
}
