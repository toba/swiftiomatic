...
@Suite struct CommandTests {
  ...
  @Test func enableThis() {
    ...
    let file = SwiftLintFile(contents: input)
    let expected = Command(
      action: .enable, ruleIdentifiers: ["rule_id"], line: 1, range: 4..<33, modifier: .this)
    #expect(file.commands() == expected.expand())
    #expect(Command(string: input) == expected)
  }
  ...
  @Test func enableNext() {
    ...
    let file = SwiftLintFile(contents: input)
    let expected = Command(
      action: .enable, ruleIdentifiers: ["rule_id"], line: 1, range: 4..<33, modifier: .next)
    #expect(file.commands() == expected.expand())
    #expect(Command(string: input) == expected)
  }
  ...
  @Test func superfluousDisableCommandsIgnoreDelimiter() {
    ...
    #expect(
      violations(Example("// swiftlint:disable nesting - \(longComment)\nprint(123)\n")).map(
        \.ruleIdentifier) == ["blanket_disable_command", "superfluous_disable_command"])
    #expect(
  ...
  @Test func invalidDisableCommands() {
    ...
    #expect(
      multipleViolations.filter({ $0.ruleIdentifier == "superfluous_disable_command" }).count == 9)
    #expect(
      multipleViolations.filter({ $0.ruleIdentifier == "blanket_disable_command" }).count == 4)

    let onlyNonExistentRulesViolations = violations(
      Example("// swiftlint:disable this is a comment\n"))
    #expect(
      onlyNonExistentRulesViolations.filter({ $0.ruleIdentifier == "superfluous_disable_command" })
        .count == 4)
    #expect(
