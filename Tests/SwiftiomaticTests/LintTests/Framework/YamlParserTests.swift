...
@Suite struct YamlParserTests {
  ...
  @Test func parseEmptyString() throws {
    #expect(
      (try YamlParser.parse("", env: [:])).count == 0, "Parsing empty YAML string should succeed")
  }

  @Test func parseValidString() throws {
    #expect(
      try YamlParser.parse("a: 1\nb: 2", env: [:]).count == 2,
      "Parsing valid YAML string should succeed")
  }

  @Test func parseReplacesEnvVar() throws {
  ...
  @Test func parseTreatNoAsString() throws {
    let string = "excluded:\n  - no"
  ...
  @Test func parseTreatYesAsString() throws {
    let string = "excluded:\n  - yes"
    let result = try YamlParser.parse(string, env: [:])
  ...
  @Test func parseTreatOnAsString() throws {
    let string = "excluded:\n  - on"
    let result = try YamlParser.parse(string, env: [:])
  ...
  @Test func parseTreatOffAsString() throws {
    let string = "excluded:\n  - off"
    let result = try YamlParser.parse(string, env: [:])
  ...
  @Test func treatAllEnvVarsAsStringsWithoutCasting() throws {
    let env = [
      "INT": "1",
      "FLOAT": "1.0",
      "BOOL": "true",
  ...
  @Test func respectCastsOnEnvVars() throws {
    let env = [
      "INT": "1",
      "FLOAT": "1.0",
      "BOOL": "true",
