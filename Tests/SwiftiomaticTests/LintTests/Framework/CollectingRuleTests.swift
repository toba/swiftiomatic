...
@Suite struct CollectingRuleTests {
  ...
  @Test func collectsAnalyzerFiles() {
    struct Spec: MockCollectingRule, AnalyzerRule {
      ...
      func validate(
        file: SwiftLintFile, collectedInfo: [SwiftLintFile: [String]], compilerArguments: [String]
      )
        -> [StyleViolation]
      {
    ...
    #expect(
      !(violations(Example("_ = 0"), config: Spec.configuration!, requiresFileOnDisk: true).isEmpty)
    )
  }
  ...
  @Test func corrects() {
    ...
    struct AnalyzerSpec: MockCollectingRule, AnalyzerRule, CorrectableRule {
      ...
      func validate(
        file: SwiftLintFile, collectedInfo: [SwiftLintFile: String], compilerArguments _: [String]
      )
        -> [StyleViolation]
      {
    ...
    #expect(
      inputs.corrections(config: AnalyzerSpec.configuration!, requiresFileOnDisk: true).count == 1)
  }
}
...
extension MockCollectingRule {
  ...
  static var description: RuleDescription {
    RuleDescription(
      identifier: "mock_test_rule_for_swiftlint_tests", name: "", description: "", kind: .lint)
  }
