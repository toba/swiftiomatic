// Adapted from SwiftLint 0.63.2 TestHelpers.swift (MIT license)
...
nonisolated(unsafe) let allRuleIdentifiers = Set(RuleRegistry.shared.list.list.keys)
...
func violations(_ example: Example,
...
func makeConfig(_ ruleConfiguration: Any?,
                _ identifier: String,
...
private func assertCorrection(_ before: Example, expected: Example, config: Configuration,
    ...
    do {
        ...
        Issue.record("couldn't read file at path '\(path)': \(error)",
                     sourceLocation: sourceLocation)
    }
}
...
func verifyRule(_ ruleDescription: RuleDescription,
    ...
    guard let config = makeConfig(
        ...
        Issue.record("Failed to create configuration", sourceLocation: sourceLocation)
        return
    }
...
func verifyLint(_ ruleDescription: RuleDescription,
    ...
    for command in disableCommands {
        ...
        for trigger in disabledTriggers {
            ...
            #expect(violationsPartitionedByType.first.isEmpty,
                    "Violation(s) still triggered although rule was disabled")
            #expect(violationsPartitionedByType.second.isEmpty,
...
private func verifyExamples(triggers: [Example],
    ...
    for nonTrigger in nonTriggers {
        ...
        Issue.record("nonTriggeringExample violated: \n\(nonTriggerWithViolations)")
    }
    ...
    for trigger in triggers {
        ...
        if markerOffsets.isEmpty {
            if triggerViolations.isEmpty {
                Issue.record("triggeringExample did not violate: \n```\n\(trigger.code)\n```")
            }
            continue
        }
        ...
        if !violationsAtUnexpectedLocation.isEmpty {
            Issue.record("triggeringExample violated at unexpected location: \n" +
        ...
        if !locationsWithoutViolation.isEmpty {
            Issue.record("triggeringExample did not violate at expected location: \n" +
...
func checkError<T: Error & Equatable>(
    ...
    do {
        try closure()
        Issue.record("No error caught", sourceLocation: sourceLocation)
    } catch let rError as T {
        if error != rError {
            Issue.record("Wrong error caught. Got \(rError) but was expecting \(error)",
                         sourceLocation: sourceLocation)
        }
    } catch {
        Issue.record("Wrong error caught", sourceLocation: sourceLocation)
    }
}
