...
@Suite struct UnusedPrivateDeclarationsTests {
  ...
  @Test func removeUnusedFilePrivateInNestedStruct() {
    ...
    testFormatting(
      for: input,
      output,
      rule: .unusedPrivateDeclarations,
      exclude: [.emptyBraces],
    )
  }
  ...
  @Test func removeUnusedFileprivateFunction() {
    ...
    let output = """
      ...
      """
    testFormatting(
      for: input, [output], rules: [.unusedPrivateDeclarations, .blankLinesAtEndOfScope],
    )
  }
  ...
  @Test func doesNotRemoveFileprivateInit() {
    let input = """
      struct Foo {
          fileprivate init() {}
          static let foo = Foo()
