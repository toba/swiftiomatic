...
@Suite
struct UseSomeForGenericParametersTests: RuleTesting {
    @Test func basicNoConstraint() {
        assertFormatting(
            UseSomeForGenericParameters.self,
            input: """
            ...
            findings: [
                FindingSpec(
                    "1️⃣", message: "use 'some' opaque parameter instead of named generic parameter")
            ]
        )
    }
    ...
    @Test func preservesGenericBoundToNestedTypeIdentity() {
        assertFormatting(
            UseSomeForGenericParameters.self,
            input: """
                func b<T: Equatable>(_ value: Container<T>.Nested) {
                    print(value)
                }
                """,
            expected: """
    ...
    @Test func preservesTwoGenericsBoundToNestedTypeIdentity() {
        assertFormatting(
            UseSomeForGenericParameters.self,
            input: """
                func c<A, B>(_ value: Pair<A, B>.Columns) {
                    print(value)
                }
                """,
            expected: """
    ...
    @Test func preservesGenericUsedAsGenericArgument() {
        assertFormatting(
            UseSomeForGenericParameters.self,
            input: """
                func d<T: Equatable>(_ values: Array<T>) {
                    print(values)
                }
                """,
            expected: """
