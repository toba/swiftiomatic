...
struct BlockCommentsRuleTests {
    @Test func noViolationForLineComments() async {
        await assertNoViolation(BlockCommentsRule.self, """
            // A comment
            // on multiple lines
            """)
    }
    ...
    @Test func detectsBlockComment() async {
        await assertViolates(BlockCommentsRule.self, """
            /* A comment
               on multiple lines */
            """)
    }
...
struct DocCommentsBeforeModifiersRuleTests {
    @Test func noViolationForDocBeforeModifiers() async {
        await assertNoViolation(DocCommentsBeforeModifiersRule.self, """
            /// Doc comment
            @MainActor
            func foo() {}
            """)
...
struct DocCommentsRuleTests {
    @Test func noViolationForDocComment() async {
        await assertNoViolation(DocCommentsRule.self, """
            /// A placeholder type
            class Foo {}
            """)
    }
    ...
    @Test func detectsRegularCommentOnDeclaration() async {
        await assertViolates(DocCommentsRule.self, """
            // A placeholder type
            class Foo {}
            """)
    }
...
struct MarkTypesRuleTests {
    @Test func noViolationWithMark() async {
        await assertNoViolation(MarkTypesRule.self, """
            // MARK: - Foo
    ...
    @Test func detectsMissingMarkForMultipleTypes() async {
        await assertViolates(MarkTypesRule.self, """
            class Foo {}
            class Bar {}
            """)
    }
