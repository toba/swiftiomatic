...
@Suite struct FileHeaderTests {
    ...
    @Test func replaceHeaderWhenFileContainsNoCode() {
        ...
        testFormatting(
            for: input, rule: .fileHeader, options: options,
    ...
    @Test func fileHeaderAuthorReplacement() {
        ...
        let foo = bar
        """
        let fileInfo = FileInfo(replacements: [
            .authorName: .constant(name), .authorEmail: .constant(email),
        ])
        let options = FormatOptions(
            fileHeader: "// Created by {author.name} {author.email}", fileInfo: fileInfo,
        )
        testFormatting(for: input, output, rule: .fileHeader, options: options)
    }
    ...
    @Test func fileHeaderMultipleReplacement() {
        ...
        let options = FormatOptions(
            fileHeader: "// Copyright © {author.name}\n// Created by {author.name}",
            fileInfo: fileInfo,
        )
        testFormatting(for: input, output, rule: .fileHeader, options: options)
    }
    ...
    @Test func fileHeaderCreationDateReplacement() {
        ...
        let fileInfo = FileInfo(creationDate: date)
        let options = FormatOptions(
            fileHeader: "// Created by Nick Lockwood on {created}.", fileInfo: fileInfo,
        )
        testFormatting(for: input, output, rule: .fileHeader, options: options)
    }
    ...
    @Test func fileHeaderDateFormattingDayMonthYear() {
        ...
        let fileInfo = FileInfo(creationDate: date)
        let options = FormatOptions(
            fileHeader: "// {created}", dateFormat: .dayMonthYear, fileInfo: fileInfo,
        )
        testFormatting(for: input, output, rule: .fileHeader, options: options)
    }
    ...
    @Test func fileHeaderDateTimeZoneAbbreviations() throws {
        // GMT+0530
        try testTimeZone(
            timeZone: #require(FormatTimeZone(rawValue: "IST")),
            tests: [
                "15:00Z": "20:30",
                "16:00+1": "20:30",
                "01:00+10": "20:30",
    ...
    @Test func fileHeaderDateTimeZoneIdentifiers() throws {
        // GMT+0845
        try testTimeZone(
            timeZone: #require(FormatTimeZone(rawValue: "Australia/Eucla")),
            tests: [
                "15:00Z": "23:45",
                "16:00+1": "23:45",
                "01:00+10": "23:45",
    ...
    @Test func fileHeaderRuleThrowsIfCreationDateUnavailable() {
        ...
        #expect(throws: (any Error).self) {
            try format(input, rules: [.fileHeader], options: options)
        }
    }
