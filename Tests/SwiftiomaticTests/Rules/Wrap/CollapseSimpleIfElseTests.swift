...
struct CollapseSimpleIfElseTests: RuleTesting {
    @Test func collapsesIfLetElse() {
        assertFormatting(
            ...
            expected: """
                ...
                """,
            findings: [FindingSpec("1️⃣", message: "collapse simple if/else onto a single line")])
    }
    ...
    @Test func skipsCommentOnLastCondition() {
        // The collapse clears the trailing trivia of the last condition, so the comment goes with
        // it.
        assertFormatting(
            CollapseSimpleIfElse.self,
            input: """
    ...
    @Test func skipsCommentOnEarlierCondition() {
        // One line would put every later condition after the comment marker.
        assertFormatting(
            CollapseSimpleIfElse.self,
            input: """
                if a, // one
    ...
    @Test func skipsCommentBeforeOpeningBrace() {
        assertFormatting(
            CollapseSimpleIfElse.self,
            input: """
                if a,
    ...
    @Test func skipsCommentOnElseIfCondition() {
        assertFormatting(
            CollapseSimpleIfElse.self,
            input: """
                if a {
