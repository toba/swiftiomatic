...
struct CollapseSimpleEnumsTests: RuleTesting {
    @Test func collapsesSimpleEnum() {
        assertFormatting(
            ...
            expected: """
                ...
                """,
            findings: [FindingSpec("1️⃣", message: "collapse simple enum onto a single line")])
    }
    ...
    @Test func collapsesRawValueTypeEnumWithoutAssignments() {
        // A raw-value-typed enum collapses as long as no case assigns an explicit raw value — the
        // bare cases carry implicit raw values whether expanded or on one line.
        assertFormatting(
            CollapseSimpleEnums.self,
            input: """
    ...
    @Test func collapsesNestedEnumInsideNonCollapsibleEnum() {
        // The outer enum has methods so it can't collapse, but the inner CodingKeys enum should
        // still be visited and collapsed.
        assertFormatting(
            CollapseSimpleEnums.self,
            input: """
                enum Indent: Codable {
    ...
    // MARK: - Comments block the collapse

    @Test func keepsCaseLineComments() {
        assertFormatting(
            CollapseSimpleEnums.self,
            input: """
                enum PoolCompletion {
    ...
    @Test func keepsCaseDocComments() {
        assertFormatting(
            CollapseSimpleEnums.self,
            input: """
    ...
    @Test func keepsCommentAfterLeftBrace() {
        assertFormatting(
            CollapseSimpleEnums.self,
            input: """
                enum Direction {  // two of them
                    case up
                    case down
                }
                """,
    ...
    @Test func keepsTrailingCommentOnACase() {
        assertFormatting(
            CollapseSimpleEnums.self,
            input: """
                enum Direction {
                    case up  // toward the sky
                    case down
                }
                """,
            expected: """
    ...
    @Test func keepsCommentBeforeRightBrace() {
        assertFormatting(
            CollapseSimpleEnums.self,
            input: """
                enum Direction {
