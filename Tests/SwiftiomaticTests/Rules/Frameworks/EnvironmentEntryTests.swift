...
@Suite struct EnvironmentEntryTests {
  @Test func replaceEnvironmentKeyDefinitionForEntryMacro() {
    ...
    let output = """
      ...
      """
    testFormatting(
      for: input, output, rule: .environmentEntry,
  ...
  @Test func environmentKeyIsNotRemovedWhenPropertyAndKeyDontMatch() {
    let input = """
      ...
      """
    testFormatting(
      for: input, rule: .environmentEntry, options: FormatOptions(swiftVersion: "6.0"),
  ...
  @Test func entryMacroReplacementWhenKeyDoesNotHaveEnvironmentKeySuffix() {
    let input = """
