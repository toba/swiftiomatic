...
@Suite(.rulesRegistered) struct DeploymentTargetRuleTests {
    ...
    func macOSAttributeReason() async throws {
        ...
        let violations = try await ruleViolations(
            example,
            rule: DeploymentTargetRule.identifier,
            configuration: ["macOS_deployment_target": "10.14.0"],
        )
    ...
    func watchOSConditionReason() async throws {
        ...
        let violations = try await ruleViolations(
            example,
            rule: DeploymentTargetRule.identifier,
            configuration: ["watchOS_deployment_target": "5.0.1"],
        )
    ...
    func iOSNegativeAttributeReason() async throws {
        ...
        let violations = try await ruleViolations(
            example,
            rule: DeploymentTargetRule.identifier,
            configuration: ["iOS_deployment_target": "15.0"],
        )
