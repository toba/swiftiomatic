...
@Suite struct SpaceAroundParensTests {
  static let cases: [FormatCase] = [
    ...
    FormatCase(
      "spaceBetweenParenAndFoo",
      input: "func foo ()",
      output: "func foo()"),
    FormatCase(
      "spaceBetweenParenAndAny",
      input: "func any ()",
      output: "func any()"),
    FormatCase(
      "spaceBetweenParenAndAnyType",
      input: "let foo: any(A & B).Type",
      output: "let foo: any (A & B).Type"),
    FormatCase(
      "spaceBetweenParenAndSomeType",
      input: "func foo() -> some(A & B).Type",
      output: "func foo() -> some (A & B).Type"),
    FormatCase(
      "noSpaceBetweenParenAndInit",
      input: "init ()",
      output: "init()"),
    FormatCase(
      "noSpaceBetweenObjcAndSelector",
      input: "@objc (XYZFoo) class foo",
      output: "@objc(XYZFoo) class foo"),
    FormatCase(
      "noSpaceBetweenHashSelectorAndBrace",
      input: "#selector(foo)"),
    FormatCase(
      "noSpaceBetweenHashKeyPathAndBrace",
      input: "#keyPath (foo.bar)",
      output: "#keyPath(foo.bar)"),
    FormatCase(
      "noSpaceBetweenHashAvailableAndBrace",
      input: "#available (iOS 9.0, *)",
      output: "#available(iOS 9.0, *)"),
    FormatCase(
      "noSpaceBetweenPrivateAndSet",
      input: "private (set) var foo: Int",
      output: "private(set) var foo: Int"),
    FormatCase(
      "spaceBetweenLetAndTuple",
      input: "if let (foo, bar) = baz {}"),
    FormatCase(
      "spaceBetweenIfAndCondition",
      input: "if(a || b) == true {}",
      output: "if (a || b) == true {}"),
    FormatCase(
      "noSpaceBetweenArrayLiteralAndParen",
      input: "[String] ()",
      output: "[String]()"),

    // Capture list + arguments
    FormatCase(
      "addSpaceBetweenCaptureListAndArguments4",
      input: "{ [weak self](foo: @escaping(Bar?) -> Void) -> Baz? in foo }",
      output: "{ [weak self] (foo: @escaping (Bar?) -> Void) -> Baz? in foo }"),
    FormatCase(
      "addSpaceBetweenCaptureListAndArguments5",
      input: "{ [weak self](foo: @autoclosure() -> String) -> Baz? in foo() }",
      output: "{ [weak self] (foo: @autoclosure () -> String) -> Baz? in foo() }"),
    FormatCase(
      "addSpaceBetweenCaptureListAndArguments6",
