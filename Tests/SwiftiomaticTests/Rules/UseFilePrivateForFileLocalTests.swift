...
@Suite
struct UseFilePrivateForFileLocalTests: RuleTesting {
  ...
  private func runWithMultipleConfigurations(
    ...
    for testConfig in testConfigurations {
      var configuration = Configuration.forTesting(enabledRule: UseFilePrivateForFileLocal.self.key)
      configuration[UseFilePrivateForFileLocal.self].accessLevel = testConfig.desired
      ...
      assertFormatting(
        UseFilePrivateForFileLocal.self,
