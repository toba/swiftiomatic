// Adapted from SwiftFormat 0.59.1 XCTestCase+testFormatting.swift (MIT license)
...
func testFormatting(for input: String, _ outputs: [String] = [], rules: [FormatRule],
    ...
    do {
        ...
        Issue.record("Encountered error generating rule registry: \(error.localizedDescription)",
    ...
    if options.swiftVersion != .undefined {
        ...
        #expect(options.swiftVersion <= maxVersion,
    ...
    if input != output, formatResult.output == output {
        #expect(!formatResult.changes.isEmpty, """
    ...
    do {
        #expect(try format(input, rules: FormatRules.all(except: exclude), options: options).output
    ...
    if input != output {
        do {
            #expect(try format(output, rules: rules, options: options).output
        ...
        if !input.hasPrefix("#!") {
            for rule in rules {
                ...
                do {
                    #expect(try format(disabled, rules: [rule], options: options).output
                                == disabled, "Failed to disable \(rule) rule", sourceLocation: sourceLocation)
                } catch {
                    Issue.record("Failed to format with disabled rule: \(error)", sourceLocation: sourceLocation)
                }
            }
        }
    }
    if input != output2, output != output2 {
        do {
            #expect(try format(output2, rules: FormatRules.all(except: exclude), options: options).output
                        == output2, sourceLocation: sourceLocation)
        } catch {
    ...
    #if os(macOS)
        do {
            #expect(try lint(output, rules: rules, options: options) == [], sourceLocation: sourceLocation)
            #expect(try lint(output2, rules: FormatRules.all(except: exclude), options: options)
                        == [], sourceLocation: sourceLocation)
        } catch {
