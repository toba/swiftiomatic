...
@Suite struct MetadataTests {
    ...
    @Test func generateRulesDocumentation() throws {
        ...
        for rule in FormatRules.all {
            ...
            if !rule.options.isEmpty {
                ...
                for option in rule.options {
                    let descriptor = try #require(Descriptors.byName[option])
    ...
    @Test func noInvalidRulesInRulesFile() {
        ...
        while let match = rulesFile.range(of: "\\*[a-zA-Z]+\\* - ", options: .regularExpression, range: range, locale: nil) {
            ...
            #expect(ruleNames.contains(ruleName), ruleName)
            range = match.upperBound ..< range.upperBound
        }
    }

    @Test func ruleHelpLinesEndInPeriod() {
        for rule in FormatRules.all {
            #expect(rule.help.hasSuffix(".") || rule.help.hasSuffix(".)"),
                      "\(rule.name) rule help does not end in a period")
        }
    }

    @Test func ruleExampleDiffsAreValid() throws {
        for rule in FormatRules.all {
            ...
            do {
                ...
                Issue.record("Error parsing ```diff code blocks in \(rule.name) rule examples: \(error)")
                continue
            }
            ...
            for codeBlock in codeBlocks {
                ...
                for (lineIndex, line) in lines.enumerated() {
                    ...
                    let firstChar = try #require(line.first)
    ...
    @Test func rulesOptions() throws {
        ...
        for descriptor in Descriptors.formatting.reversed() {
            ...
            #expect(!(descriptor.help.hasSuffix(".")), 
            ...
            formatter.forEach(.identifier("FormatRule")) { i, _ in
                ...
                for option in rule.options where !rule.isDeprecated {
                    if let oldName = rulesByOption[option] {
                        Issue.record("\(option) set as (non-shared) option for both \(name) and \(oldName)")
                    }
                ...
                for option in referencedOptions {
                    #expect(ruleOptions.contains(option.argumentName) || option.isDeprecated,
                              "\(option.argumentName) not listed in \(name) rule")
                }
                for argName in ruleOptions {
                    #expect(referencedOptions.contains { $0.argumentName == argName },
                              "\(argName) not used in \(name) rule")
                }
            }
        }

        #expect(allSharedOptions.isEmpty, "Options \(allSharedOptions.joined(separator: ",")) not shared by any rule)")
        #expect(allOptions.isEmpty, "Options \(allSharedOptions.joined(separator: ",")) not owned by any rule)")
    }

    @Test func allOptionsInRulesFile() {
        let arguments = Set(formattingArguments).subtracting(deprecatedArguments)
        for argument in arguments {
            #expect(rulesFile.contains("`--\(argument)`") || rulesFile.contains("`--\(argument) "), argument)
        }
    }

    @Test func noInvalidOptionsInRulesFile() {
        ...
        while let match = rulesFile.range(of: "`--[a-zA-Z-]+[` ]", options: .regularExpression, range: range, locale: nil) {
            ...
            #expect(arguments.contains(argument), argument)
            range = match.upperBound ..< range.upperBound
        }
    }

    @Test func argumentNamesAreLowercase() {
        let arguments = Set(commandLineArguments).subtracting(deprecatedArguments)
        for argument in arguments {
            #expect(argument == argument.lowercased())
        }
    }

    // MARK: keywords

    @Test func contextualKeywordsReferencedCorrectly() throws {
        ...
        for sourceFile in filesToVerify {
            ...
            formatter.forEach(.identifier("keyword")) { i, _ in
                ...
                guard keywords.contains(keyword) || keyword.isMacroOrCompilerDirective || keyword.isAttribute else {
                    ...
                    Issue.record("'\(keyword)' referenced on line \(line) of '\(sourceFile)' is not a valid Swift keyword. "
    ...
    @Test func latestVersionInChangelog() throws {
        let changelog = try String(contentsOf: changeLogURL, encoding: .utf8)
        #expect(changelog.contains("[\(SwiftFormat.version)]"), "CHANGELOG.md does not mention latest release")
        #expect(changelog.contains("(https://github.com/nicklockwood/SwiftFormat/releases/tag/\(SwiftFormat.version))"), 
                      "CHANGELOG.md does not include correct link for latest release")
    }

    @Test func latestVersionInPodspec() throws {
        let podspec = try String(contentsOf: podspecURL, encoding: .utf8)
        #expect(podspec.contains("\"version\": \"\(SwiftFormat.version)\""), "Podspec version does not match latest release")
        #expect(podspec.contains("\"tag\": \"\(SwiftFormat.version)\""), "Podspec tag does not match latest release")
    }

    @Test func versionConstantUpdated() {
        #expect(SwiftFormat.version == swiftFormatVersion)
    }

    @Test func changelogDatesAreAscending() throws {
        var lastDate: Date?
        ...
        for title in changelogTitles {
            let dateRange = try #require(title.range(of: " \\([^)]+\\)$", options: .regularExpression))
            let dateString = String(title[dateRange])
            let date = try #require(dateParser.date(from: dateString))
            if let lastDate, date > lastDate {
                Issue.record("\(title) has newer date than subsequent version (\(date) vs \(lastDate))")
                return
            }
            lastDate = date
        }
