...
@Suite struct BlankLinesAfterGuardStatementsTests {
  @Test func spacesBetweenMultiLineGuards() {
    ...
    testFormatting(
      for: input, output, rule: .blankLinesAfterGuardStatements, exclude: [.blankLinesBetweenScopes]
    )
  }
  ...
  @Test
  func spacesBetweenSingleLineAndMultiLineGuardsWhenBlankLineInsertedBetweenConsecutiveGuards() {
    let input = """
      guard let one = test.one else { return }
      guard let two = test.two else { return }
  ...
  @Test func multiLineGuardAndComments() {
    ...
    testFormatting(
      for: input, output, rule: .blankLinesAfterGuardStatements, exclude: [.docComments])
  }
  ...
  @Test func notInsertLineBreakWhenInlineFunction() {
    let input = """
      ...
      """
    testFormatting(
      for: input, rule: .blankLinesAfterGuardStatements, exclude: [.wrapConditionalBodies])
  }
