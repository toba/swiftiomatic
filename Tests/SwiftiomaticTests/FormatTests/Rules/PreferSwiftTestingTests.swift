...
@Suite struct PreferSwiftTestingTests {
  @Test func convertsSimpleTestSuite() {
    ...
    testFormatting(
      for: input, [output], rules: [.preferSwiftTesting, .sortImports, .isEmpty], options: options)
  }

  @Test func convertsTestSuiteWithSetUpTearDown() {
    let input = """
      @testable import MyFeatureLib
      import XCTest
  ...
  @Test func convertsSimpleXCTestHelpers() throws {
    ...
    testFormatting(
      ...
      options: options, exclude: [.noForceTryInTests])
  }

  @Test func convertsMultilineXCTestHelpers() {
    let input = """
      import XCTest
    ...
    testFormatting(
      for: input, [output], rules: [.preferSwiftTesting, .wrapArguments, .indent, .trailingSpace],
      options: options)
  }
  ...
  @Test func preservesUnsupportedUITestHelpers() {
    let input = """
      @testable import MyFeatureLib
      import XCTest
  ...
  @Test func preservesUnsupportedPerformanceTestHelpers() {
    let input = """
      @testable import MyFeatureLib
      import XCTest
  ...
  @Test func preservesAsyncOrThrowsTearDown() {
    let input = """
      @testable import MyFeatureLib
      import XCTest
  ...
  @Test func convertsHelpersInHelperMethods() {
    let input = """
      @testable import MyFeatureLib
      import XCTest
    ...
    let options = FormatOptions(swiftVersion: "6.0")
    testFormatting(
      for: input, [output], rules: [.preferSwiftTesting, .sortImports], options: options)
  }

  @Test func preservesHelpersWithLineFileParams() {
    let input = """
      @testable import MyFeatureLib
      import XCTest
  ...
  @Test func doesntUpdateNameToIdentifierRequiringBackTicks() {
    let input = """
      import XCTest
  ...
  @Test func doesntUpdateTestNameToExistingFunctionName() {
    let input = """
      import XCTest
  ...
  @Test func addsThrowingEffectIfNeeded() throws {
  ...
  @Test func preservesAdditionalXCTestSymbols() {
    let input = """
      @testable import MyFeatureLib
      import XCTest
    ...
    let options = FormatOptions(
      additionalXCTestSymbols: ["myCustomTestHelper"], swiftVersion: "6.0")
    testFormatting(for: input, rule: .preferSwiftTesting, options: options)
  }

  @Test func preservesXCTestCaseExtension() {
    let input = """
      @testable import MyFeatureLib
      import XCTest
  ...
  @Test func addsUIKitImportIfNeeded() {
    let input = """
      import XCTest
  ...
  @Test func appliesCustomTestSuiteAttribute() {
    let input = """
      import XCTest
  ...
  @Test func appliesMultipleTestSuiteAttributes() {
    let input = """
      import XCTest
    ...
    let options = FormatOptions(
      defaultTestSuiteAttributes: ["@MainActor", "@Suite"], swiftVersion: "6.0")
    testFormatting(
      for: input, [output], rules: [.preferSwiftTesting, .sortImports], options: options)
  }
}
