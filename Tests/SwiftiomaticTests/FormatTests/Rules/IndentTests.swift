...
@Suite struct IndentTests {
    ...
    @Test func indentMultipleSingleLineSwitchCaseCommentsWithCommentsIgnoredCorrectlyWhenIndentCaseTrue() {
        let input = """
        switch x {
            // bar
            case .y: return 1
    ...
    @Test func ifDefPreserveWithMultiplePlatformBranches() {
        let input = """
        import SwiftUI
    ...
    @Test func truncateBlankLineBeforeIndenting() {
        let input = """
        func foo() {
        ...
        #expect(try lint(input, rules: [.indent, .trailingSpace], options: options) == [
