...
@Suite struct IndentTests {
  ...
  @Test func indentMultilineStatementDoesntFailToTerminate() {
    let input = """
      foo(one: 1,
          two: 2).bar { _ in
          "one"
  ...
  @Test func wrappedClassWhereDeclarationLikeXcode() {
    ...
    let options = FormatOptions(xcodeIndentation: true)
    testFormatting(
      for: input, output, rule: .indent, options: options, exclude: [.simplifyGenericConstraints])
  }
  ...
  @Test func wrappedBeforeComma() {
    ...
    let output = """
      ...
      """
    testFormatting(
      for: input, output, rule: .indent, exclude: [.leadingDelimiters, .singlePropertyPerLine])
  }
  ...
  @Test func singleIndentTrailingClosureBody2() {
    ...
    testFormatting(
      for: input, rule: .indent, options: options,
      exclude: [
        .wrapConditionalBodies, .wrapMultilineStatementBraces, .blankLinesAfterGuardStatements,
      ])
  }
  ...
  @Test func indentUnderIndentedMultilineStringDoesntAddIndent() {
    let input = #"""
      class Main {
          func main() {
              print("""
  ...
  @Test func indentSwitchCaseWhere() {
    ...
    let options = FormatOptions(indentCase: true)
    testFormatting(
      for: input, rule: .indent, options: options, exclude: [.wrap, .wrapMultilineFunctionChains])
  }
