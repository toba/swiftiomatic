...
@Suite struct HoistTryTests {
  ...
  @Test func hoistTryInsideStringInterpolation2() {
    ...
    let output = """
      ...
      """
    testFormatting(
      for: input, output, rule: .hoistTry,
  ...
  @Test(.disabled("Rule hoists try in #expect differently from upstream"))
  func noHoistTryInsideExpect() {
    let input = """
      #expect(!(try foo()))
      """
  ...
  @Test func noMergeTrysInsideExpect() throws {
    let input = """
