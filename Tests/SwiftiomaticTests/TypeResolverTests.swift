...
struct MockResolver: TypeResolver {
    ...
    func resolveType(inFile _: String, offset: Int) async -> ResolvedType? {
...
struct TypeResolverTests {
    ...
    @Test func concurrencyCheckUpgradesConfidenceWithResolver() async {
        let source = """
        import Dispatch
        func doWork() {
        ...
        let checkWith = ConcurrencyModernizationCheck(filePath: "test.swift", typeResolver: resolver)
        checkWith.walk(tree)
