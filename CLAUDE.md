...
sm format --style compact Sources/     # override the configured style
...
The format pipeline is style-driven — pick a `style` (currently only `compact`), not individual rule toggles. Lints are still per-rule (`"lint": "no" | "warn" | "error"`).
...
| `StaticFormatRule<V>` | `SyntaxRule` (no rewriter) | Node-local format rule with `static transform(_:parent:context:)` (and optional `willEnter` / `didExit` hooks). Dispatched from `CompactStageOneRewriter` in a single tree walk. |
...
Lint rules are interleaved in a single `LintPipeline` tree walk. Format runs as stage 1 (one combined `CompactStageOneRewriter` walk dispatching every `StaticFormatRule`) plus stage 2 (a small ordered list of `StructuralFormatRule` passes), then the pretty printer. Pipelines are auto-generated by the build plugin.

The format pipeline is selected by the configuration's `style` value. `compact` is the default and only fully-implemented style; `roomy` is reserved.
...
- `LintPipeline` interleaves lint rules per node; `CompactStageOneRewriter` dispatches stage-1 format hooks per node in a single walk; `StructuralFormatRule`s run sequentially after
- `RuleMask` disables rules via `// sm:ignore` comments
- Configuration is JSON5-decoded; the format side is style-driven (`style: "compact"` plus universal layout settings); the lint side is per-rule (`"lint": "no" | "warn" | "error"`)
...
- `CompactStageOneRewriter+Generated.swift` — per-node-type dispatch (`willEnter` → `super.visit` → `rewrite<NodeType>` → `didExit`) for the stage-1 rewriter
